...
**REQUIREMENTS:**
1. **REINITIALIZE AGENT FIRST** - Always reinitialize TaskManager agent before any other action (even if already initialized)
...
**🚨 CRITICAL: ALWAYS REINITIALIZE AGENT FIRST:**
```bash
timeout 10s node "/Users/jeremyparker/Desktop/Claude Coding Projects/infinite-continue-stop-hook/taskmanager-api.js" init
```
This reinitializes the TaskManager agent for all task operations (even if already initialized).
...
**ERROR HANDLING:**
- "No agent ID provided/initialized" → Always reinitialize regardless of current state
- Agent expiration leads to task system failures - never skip reinitialization
- **ANY TASKMANAGER API ERROR** → Reinitialize agent: `timeout 10s node "/Users/jeremyparker/Desktop/Claude Coding Projects/infinite-continue-stop-hook/taskmanager-api.js" init`
- **ALWAYS REINITIALIZE** → Even if agent appears initialized, always run init before task operations
...
2. **Initialize Agent** - Always run: `timeout 10s node "/Users/jeremyparker/Desktop/Claude Coding Projects/infinite-continue-stop-hook/taskmanager-api.js" init`
