...
async function testTasksApi(_category = 'general') {
  loggers.stopHook.info('🧪 Testing TASKS.json API system...\n');
  try {
    ...
    loggers.stopHook.info(`✅ Schema version: ${tasksData.schema_version}`);
    loggers.stopHook.info(`✅ Total tasks: ${tasksData.metadata.total_tasks}`);
    loggers.stopHook.info(`✅ Tasks by type:`, tasksData.metadata.tasks_by_type);
    ...
    loggers.stopHook.info(`✅ Available task types: ${taskTypes.join(', ')}`);
    ...
    loggers.stopHook.info(`✅ Adapted ${adaptedData.tasks.length} other tasks`);
    ...
    loggers.stopHook.info(`✅ Test tasks: ${testTasks.length}`);
    loggers.stopHook.info(`✅ Audit tasks: ${auditTasks.length}`);
    ...
    loggers.stopHook.info(`✅ Task relationships defined: ${relationshipCount}`);
    ...
    loggers.stopHook.info(`✅ Priority order: ${priorityOrder.join(' → ')}`);
    ...
    loggers.stopHook.info(`   • Task types supported: ${taskTypes.length}`);
    loggers.app.info(
      `   • Auto-generation enabled: ${workflowConfig.auto_generation_enabled}`,
    );
    loggers.stopHook.info(`   • CLAUDE.md compliance: ✅`);
