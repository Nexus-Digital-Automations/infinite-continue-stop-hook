...
async function testTasksApi() {
  ...
  try {
    ...
    const relationshipCount = Object.keys(tasksData.task_relationships || {}).length;
    ...
    const taskTypes7 = sortedTasks.slice(0, 10).map(t => `${t.type}(${t.priority})`);
    ...
    console.log(`✅ Auto-generation enabled: ${workflowConfig.auto_generation_enabled}`);
    console.log(`✅ Mandatory test gate: ${workflowConfig.mandatory_test_gate}`);
    console.log(`✅ Security validation required: ${workflowConfig.security_validation_required}`);
    ...
    console.log(`✅ Auto-task types: ${autoTasks.map(t => t.type).join(', ')}`);
    ...
    console.log(`   • Schema version: ${tasksData.schema_version}`);
    console.log(`   • Total tasks: ${tasksData.metadata.total_tasks}`);
    console.log(`   • Features migrated: ${tasksData.metadata.migration_stats.features_migrated}`);
    console.log(`   • Auto-generated tasks: ${tasksData.metadata.migration_stats.auto_generated_tasks}`);
