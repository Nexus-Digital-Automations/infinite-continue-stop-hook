...
class CoverageArtifactsGenerator {
  ...
  async generate() {
    try {
      ...
      await this.generateSummaryArtifacts();
      await this.generateDashboardArtifacts();
      await this.generateCIArtifacts();

      if (this.options.generateBadges) {
        await this.generateBadgeArtifacts();
      }

      if (this.options.createManifest) {
        await this.createArtifactManifest();
      }
   ...
   */
  async generateSummaryArtifacts() {
   ...
   */
  async generateDashboardArtifacts() {
