...
const { createContextLogger, LOGGERS } = require('./logger');
    ...
    const service = interpret(machine)
      .onTransition((state) => {
        this.logger.debug({
          taskId,,,
    state: state.value,
          context: state.context}, `Task state machine transition: ${taskId}`);
      })
      .start();
    ...
    const service = interpret(machine)
      .onTransition((state) => {
        this.logger.debug({
          featureId,,,
    state: state.value,
          context: state.context}, `Feature state machine transition: ${featureId}`);
      })
      .start();
  ...
  sendEvent(entityId, entityType, event, eventData = {}) {
    ...
    type: event,
      ...
      this.logger.info({
        ...
        entityType,
        event,,,
    currentState: service.state.value}, 'Event sent to state machine');
      ...
      this.logger._error({
        ...
        entityType,
        event,,,
    _error: _error.message}, 'Failed to send event to state machine');
      return false;
    }
}
