...
class RAGWorkflowIntegration {
   ...
   */
  _generateRagProtocolsSection(category = 'general') {
    return `
...
class LegacyLessonsBridge {
   ...
   */
  searchLessons(query, category = null) {
    await this._ensureInitialized();
   ...
   */
  findSimilarErrors(errorDescription) {
    await this._ensureInitialized();
