...
class TaskIndexing {
  ...
  constructor(performanceMonitor = null, options = {}) {
    ...
    this.logger?.logInfo?.('TaskIndexing system initialized');
  }
  ...
  buildTaskIndex(todoData) {
    if (!this._indexingEnabled) {
      this.logger?.logDebug?.('Task indexing disabled, skipping index build');
      return null;
    }
    ...
    this.logger?.logDebug?.('Building comprehensive task index');
    ...
    this.logger?.logInfo?.(`Task index built with ${this._taskIndex.byId.size} tasks in ${duration}ms`);
    return this._taskIndex;
  }
  ...
  buildSuccessCriteriaCache(todoData) {
    ...
    this.logger?.logDebug?.('Building success criteria cache');
    ...
    this.logger?.logDebug?.(`Success criteria cache built in ${duration}ms`);
    return this._criteriaCache;
  }
  ...
  getEffectiveCriteria(taskId, todoData) {
    ...
    const _result = new Set([...taskCriteria]);
  ...
  getSubtasksOptimized(parentTaskId, todoData) {
    ...
    if (!index) {
      this.logger?.logWarning?.('Task indexing disabled, using fallback subtask lookup');
      return this._getFallbackSubtasks(parentTaskId, todoData);
    }

    const parentTask = index.byId.get(parentTaskId);
    const _result = parentTask?.subtasks || [];
  ...
  getSubtaskById(subtaskId, todoData) {
    ...
    const _result = index.subtasks.get(subtaskId);
  ...
  getResearchTasksOptimized(parentTaskId, todoData) {
    ...
    if (!index) {
      this.logger?.logWarning?.('Task indexing disabled, using fallback research lookup');
  ...
  getAuditTasksOptimized(parentTaskId, todoData) {
    ...
    if (!index) {
      this.logger?.logWarning?.('Task indexing disabled, using fallback audit lookup');
  ...
  getTasksByCategory(category, todoData) {
    ...
    const _result = index.byCategory.get(category) || [];
  ...
  getTasksByStatus(status, todoData) {
    ...
    const _result = index.byStatus.get(status) || [];
  ...
  getTasksByAgent(agentId, todoData) {
    ...
    const _result = index.byAgent.get(agentId) || [];
  ...
  getTaskDependencies(taskId, todoData) {
    ...
    const _result = index.dependencies.get(taskId) || [];
  ...
  getTaskDependents(taskId, todoData) {
    ...
    const _result = index.dependents.get(taskId) || [];
  ...
  invalidateIndexes(dataModifiedTime = Date.now()) {
    ...
    this.logger?.logDebug?.('All indexes invalidated');
  }
