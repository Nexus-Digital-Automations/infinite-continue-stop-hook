...
class TaskIndexing {
  ...
  getResearchTasksOptimized(parentTaskId, todoData) {
    ...
    if (parentTaskId) {
      const parentTask = index.byId.get(parentTaskId);
      _result = (parentTask?.subtasks || []).filter(subtask => subtask.type === 'research');
    } else {
      _result = Array.from(index.researchTasks.values());
    }
  ...
  getAuditTasksOptimized(parentTaskId, todoData) {
    ...
    if (parentTaskId) {
      const parentTask = index.byId.get(parentTaskId);
      _result = (parentTask?.subtasks || []).filter(subtask => subtask.type === 'audit');
    } else {
      _result = Array.from(index.auditTasks.values());
    }
  ...
  getTasksByAgent(agentId, todoData) {
    ...
    const result = index.byAgent.get(agentId) || [];
