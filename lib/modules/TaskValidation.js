...
class TaskValidation {
  ...
  validateCompletionData(completionData, task) {
    ...
    const safeFields = ['details', 'notes', 'duration', 'difficulty'];
    for (const field of safeFields) {
      if (completionData[field] !== undefined) {
        sanitized[field] = completionData[field];
      }
    }
  ...
  _validateRequiredFields(task, result) {
    ...
    if (this.options.requireCategory) {
  ...
  _validateFieldFormats(task, result) {
    ...
    // Arrays validation
    const arrayFields = ['dependencies', 'success_criteria', 'important_files', 'assigned_agents', 'subtasks'];
    for (const field of arrayFields) {
      if (task[field] && !Array.isArray(task[field])) {
        result.warnings.push(`Field '${field}' should be an array`);
        result.autoRepairs.push({
          field,
          action: 'convert_to_array',
          oldValue: task[field],
          newValue: Array.isArray(task[field]) ? task[field] : [task[field]],
        });
      }
    }

    // Date fields validation
    const dateFields = ['created_at', 'updated_at', 'completed_at'];
  ...
  _validateFieldConstraints(task, result) {
    ...
    // Category validation
    if (task.category && !this.options.validCategories.includes(task.category)) {
      result.isValid = false;
      result.errors.push(`Invalid category '${task.category}'. Valid categories: ${this.options.validCategories.join(', ')}`);
    }

    // Status validation
    if (task.status && !this.options.validStatuses.includes(task.status)) {
    ...
    // Priority validation
    if (task.priority && !this.options.validPriorities.includes(task.priority)) {
  ...
  _validateSuccessCriteria(task, result) {
    ...
    for (let i = 0; i < task.success_criteria.length; i++) {
      const criterion = task.success_criteria[i];
  ...
  _validateRootStructure(todoData, result) {
    const requiredArrays = ['tasks', 'completed_tasks', 'features', 'agents'];

    for (const field of requiredArrays) {
      if (!todoData[field]) {
        result.warnings.push(`Missing root field: ${field}`);
        result.suggestions.push(`Add ${field}: [] to root structure`);
      } else if (!Array.isArray(todoData[field])) {
        result.isValid = false;
        result.errors.push(`Root field '${field}' should be an array`);
      }
    }
  ...
  /**
   * Validate agents structure
   * @param {Array} agents - Agents array
   * @param {Object} result - Validation result
   * @private
   */
  _validateAgentsStructure(agents, result) {
    ...
    for (let i = 0; i < agents.length; i++) {
      ...
      if (!agent || typeof agent !== 'object') {
        result.warnings.push(`Agent ${i} is invalid`);
        continue;
      }

      if (!agent.agentId) {
        result.warnings.push(`Agent ${i} missing agentId`);
      }

      if (!agent.status) {
        result.warnings.push(`Agent ${i} missing status`);
      }
    }
  }
   ...
   * @private
   */
  _evaluateTestCriterion(criterion, context) {
   ...
   * @private
   */
  _evaluateLintCriterion(criterion, context) {
    // This is a placeholder - in a real implementation, this would
   ...
   * @private
   */
  _evaluateBuildCriterion(criterion, context) {
    // This is a placeholder - in a real implementation, this would
   ...
   * @private
   */
  _evaluateFileCriterion(criterion, context) {
   ...
   * @private
   */
  _evaluateCustomCriterion(criterion, task, context) {
   ...
   * @private
   */
  _validateScopeRestrictions(task, agentId, context) {
    const result = {
      valid: true,
      reason: null,
      restrictions: [],
