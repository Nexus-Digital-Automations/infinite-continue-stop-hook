...
const _logger = require('../appLogger');
...
class TaskStorage {
  ...
  readTodoSync() {
    ...
    if (!_fs.existsSync(this.todoPath)) {
    ...
    try {
      ...
      const data = _fs.readFileSync(validatedTodoPath, 'utf8');
  ...
  readTodoFast() {
    ...
    if (this._cache.enabled) {
      try {
        const stats = _fs.statSync(this.todoPath);
  ...
  async writeTodo(data, skipBackup = false) {
    ...
    try {
      ...
      if (!skipBackup && _fs.existsSync(this.todoPath)) {
        backupPath = `${this.todoPath}.backup.${Date.now()}`;
        _fs.copyFileSync(this.todoPath, backupPath);
      ...
      _fs.writeFileSync(tempPath, jsonData, 'utf8');
      _fs.renameSync(tempPath, this.todoPath);
      ...
      if (backupPath && _fs.existsSync(backupPath)) {
        try {
          _fs.copyFileSync(backupPath, this.todoPath);
  ...
  listBackups() {
    ...
    try {
      const files = _fs.readdirSync(todoDir);
  ...
  async restoreFromBackup(backupFile = null) {
    ...
    let _selectedBackup;
    if (backupFile) {
      _selectedBackup = backups.find(backup => backup.filename === backupFile);
      if (!_selectedBackup) {
        return {
          success: false,
      ...
      _selectedBackup = backups[0]; // Latest backup
    }
    ...
    try {
      ...
      if (_fs.existsSync(this.todoPath)) {
        ...
        _fs.copyFileSync(this.todoPath, preRestoreBackup);
        this.logger?.logInfo?.(`Created pre-restore backup: ${preRestoreBackup}`);
      }

      // Restore from backup
      _fs.copyFileSync(_selectedBackup.path, this.todoPath);
      ...
      this.logger?.logInfo?.(`Successfully restored from backup: ${_selectedBackup.filename}`);
      return {
        success: true,
        message: `Restored from backup: ${_selectedBackup.filename}`,
        backupUsed: _selectedBackup,
      };
    } catch (error) {
   ...
   * @returns {Promise<Object>} Archive operation result with batch statistics
   */
  async archiveCompletedTasks(tasks) {
    ...
    try {
      ...
      if (_fs.existsSync(this.donePath)) {
        try {
          const doneContent = _fs.readFileSync(this.donePath, 'utf8');
      ...
      _fs.writeFileSync(tempPath, JSON.stringify(doneData, null, 2), 'utf8');
      _fs.renameSync(tempPath, this.donePath);
  ...
  async archiveCompletedTask(task) {
    ...
    try {
      ...
      // Write DONE.json
      _fs.writeFileSync(this.donePath, JSON.stringify(doneData, null, 2), 'utf8');
  ...
  async readDone() {
    ...
    if (!_fs.existsSync(this.donePath)) {
      ...
      try {
        _fs.writeFileSync(this.donePath, JSON.stringify(defaultData, null, 2), 'utf8');
        return defaultData;
      } catch (error) {
    ...
    try {
      const data = _fs.readFileSync(this.donePath, 'utf8');
      const parsedData = JSON.parse(data);
  ...
  getFileStatus() {
    ...
    const status = {
      todoExists: _fs.existsSync(this.todoPath),
      doneExists: _fs.existsSync(this.donePath),
    ...
    if (status.doneExists) {
      const stats = _fs.statSync(this.donePath);
