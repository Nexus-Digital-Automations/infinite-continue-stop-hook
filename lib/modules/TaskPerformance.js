...
class TaskPerformance {
  ...
  constructor(options = {}) {
    ...
    if (this.enabled) {
      ...
      this.logger?.logInfo?.('TaskPerformance monitoring initialized');
    } else {
      this.logger?.logDebug?.('TaskPerformance monitoring disabled');
    }
  }
  ...
  recordQueryTime(operation, duration, metadata = {}) {
    ...
    if (duration > 100) {
      ...
      this.logger?.logWarning?.(`Slow operation detected: ${operation} took ${duration}ms`);
    }
    ...
    this.logger?.logDebug?.(`Recorded query time: ${operation} = ${duration}ms`);
  }
   ...
   * @param {Object} metadata - Additional metadata
   */
  recordCacheHit(hit, operation = 'unknown', metadata = {}) {
    if (!this.enabled) {return;}
    ...
    this.logger?.logDebug?.(`Cache ${hit ? 'hit' : 'miss'} for operation: ${operation}`);
  ...
  recordBatchOperation(operation, itemCount, duration, metadata = {}) {
    ...
    this.logger?.logInfo?.(`Batch operation: ${operation} processed ${itemCount} items in ${duration}ms (${record.throughput.toFixed(2)} items/sec)`);
  }
  ...
  recordMemoryUsage(additionalMetrics = {}) {
    ...
    this.logger?.logDebug?.(`Memory usage: ${(heapUsedMB).toFixed(2)}MB heap used`);
  }
  ...
  getPerformanceReport() {
    ...
    this.logger?.logInfo?.('Generated performance report');
    return report;
  }
  ...
  clearMetrics() {
    ...
    this.logger?.logInfo?.('Performance metrics cleared');
  }
  ...
  enable() {
    ...
    this.logger?.logInfo?.('Performance monitoring enabled');
  }
  ...
  disable() {
    this.enabled = false;
    this.logger?.logInfo?.('Performance monitoring disabled');
  }
  ...
  /**
   * Analyze batch operation metrics
  ...
  _analyzeBatchMetrics() {
    ...
    const totalDuration = this._metrics.batchOperations.reduce((sum, b) => sum + b.duration, 0);
  ...
  _addPerformanceWarning(category, type, metadata = {}) {
    ...
    if (!isDuplicate) {
      ...
      this.logger?.logWarning?.(`Performance warning: ${category}:${type}`, metadata);
    }
  }
}
