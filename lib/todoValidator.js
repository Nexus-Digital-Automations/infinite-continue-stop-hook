...
class TodoValidator {
     ...
     */
    _validateSingleTask(task, index, allTasks) {
        ...
        this.requiredFields.task.forEach(field => {
            if (!(field in task)) {
                ...
                this.fixes.push({
                    ...
                    message: `Added missing field '${field}' to task ${task.id || index}`,
                    field,
                    value: defaultValue,
                    automated: true
                });
        ...
        if (!task.id || typeof task.id !== 'string') {
            task.id = task.id ? String(task.id) : `task-${Date.now()}-${index}`;
            this.fixes.push({
                type: 'ID_CORRECTION',
    ...
    _validateReviewStrikes(data) {
        const reviewTasks = data.tasks.filter(t => t.is_review_task || t.mode === 'REVIEWER');
