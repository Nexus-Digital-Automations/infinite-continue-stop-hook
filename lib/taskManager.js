...
class TaskManager {
    constructor(todoPath, options = {}) {
        ...
        // Initialize distributed lock manager for multi-agent coordination
        ...
        this.options = {
            ...
            enableLinterReminders: options.enableLinterReminders !== false,
    ...
    async getTaskContinuationGuidance(agentId) {
        ...
        // Check for pending linter feedback first
        if (todoData.pending_linter_feedback) {
            const linterFeedback = todoData.pending_linter_feedback;
            const linterMessage = await this.generateLinterFeedbackMessage();
            
            return {
                action: 'linter_feedback_required',
                taskId: linterFeedback.taskId,
                taskTitle: linterFeedback.taskTitle,
                requiresLinterCheck: linterFeedback.requiresLinterCheck,
                message: linterMessage ? linterMessage.message : 'Please run linter checks on recently completed code.',
                clearShellCommand: `./scripts/taskmanager/taskmanager.sh linter-clear`,
                lintCommands: linterMessage ? linterMessage.directories.map(dir => `npm run lint ${dir}`) : ['npm run lint']
            };
        }
        ...
        if (currentTask) {
            ...
            const needsLinterCheck = this.shouldCheckLinter(currentTask);
            const guidance = {
                action: 'continue_task',
                task: currentTask,
                instructions: this.generateTaskInstructions(currentTask),
                requiresLinterCheck: currentTask.status === 'in_progress' && needsLinterCheck,
                ...
                completionWithLinterCommand: needsLinterCheck ? `node -e "const TaskManager = require('./lib/taskManager'); const tm = new TaskManager('./TODO.json'); tm.updateTaskStatus('${currentTask.id}', 'completed', {runLinterFeedback: true}).then(() => console.log('Task completed with linter feedback'));"` : null,
                note: needsLinterCheck ? 'Use completionWithLinterCommand to run linter feedback when marking task as completed' : null
            };
            ...
            if (nextTask) {
                const needsLinterCheck = this.shouldCheckLinter(nextTask);
                const guidance = {
                    ...
                    completionWithLinterCommand: needsLinterCheck ? `node -e "const TaskManager = require('./lib/taskManager'); const tm = new TaskManager('./TODO.json'); tm.updateTaskStatus('${nextTask.id}', 'completed', {runLinterFeedback: true}).then(() => console.log('Task completed with linter feedback'));"` : null,
                    note: needsLinterCheck ? 'Use completionWithLinterCommand to run linter feedback when marking task as completed' : null
                };
    ...
    /**
     * Check if linter should be run for this task
     * @param {Object} task - Task object
     * @returns {boolean} Whether to run linter
     */
    shouldCheckLinter(task) {
        return task.mode === 'DEVELOPMENT' || task.mode === 'TESTING' || task.mode === 'REFACTORING';
    }

    // Removed slow runLinterFeedback method - using fast shell scripts instead
    ...
    async updateTaskStatus(taskId, status, _options = {}) {
        ...
        if (task) {
            ...
            if (status === 'completed' && oldStatus !== 'completed') {
                // Set linter feedback requirement - but with clear stopping instructions
                if (this.shouldCheckLinter(task)) {
                    todoData.pending_linter_feedback = {
                        taskId: task.id,
                        taskTitle: task.title,
                        timestamp: new Date().toISOString(),
                        requiresLinterCheck: true
                    };
                }
                
                // Track directories for linter reminders
                if (this.options.enableLinterReminders) {
                    this.trackTaskDirectoriesForLinting(task);
                }
                
                // Don't archive yet - wait for linter feedback to be cleared
            } else if (status === 'completed' && this.options.enableArchiving && !todoData.pending_linter_feedback) {
                // Archive completed task if no linter feedback is pending
    ...
    /**
     * Track directories that need linting after task completion
     */
    trackTaskDirectoriesForLinting(task) {
        if (!task) return;

        // Get directories from important_files if available
        const directories = new Set();
        
        if (task.important_files && Array.isArray(task.important_files)) {
            task.important_files.forEach(filePath => {
                if (filePath) {
                    const path = require('path');
        ...
        // Store linter reminder data in task for retrieval by stop hook
        task.linterReminder = {
            taskId: task.id,
            taskTitle: task.title,
            directories: Array.from(directories),
            timestamp: new Date().toISOString(),
            needsLinting: directories.size > 0
        };
    }
...
After addressing any linting issues, you can continue with your next task. Clean code is maintainable code!`;
    ...
    /**
     * Clear pending linter feedback to allow next task assignment
     * This should be called after linter issues are addressed
     */
    async clearLinterFeedback() {
        const todoData = await this.readTodo();
        
        if (!todoData.pending_linter_feedback) {
            return { success: false, message: 'No pending linter feedback found' };
        }
        
        const clearedFeedback = todoData.pending_linter_feedback;
        delete todoData.pending_linter_feedback;
        
        // If the task was marked for archiving, archive it now
        if (this.options.enableArchiving) {
            const completedTask = todoData.tasks.find(t => t.id === clearedFeedback.taskId && t.status === 'completed');
            if (completedTask) {
                await this.archiveCompletedTask(completedTask);
                todoData.tasks = todoData.tasks.filter(t => t.id !== clearedFeedback.taskId);
            }
        }
        ...
        return {
            success: true,
            message: 'Linter feedback cleared - task progression will resume',
            clearedTask: clearedFeedback.taskTitle,
            clearedAt: new Date().toISOString()
        };
    }

    /**
     * Check if there is pending linter feedback
     */
    async hasPendingLinterFeedback() {
        const todoData = await this.readTodo();
        return !!todoData.pending_linter_feedback;
    }

    /**
     * Get pending linter feedback details
     */
    async getPendingLinterFeedback() {
        const todoData = await this.readTodo();
        return todoData.pending_linter_feedback || null;
    }
}
