...
After addressing any linting issues, you can continue with your next task. Clean code is maintainable code!`;
    ...
    generateStuckTaskInstructions(task, repetitionInfo) {
        let instructions = `**🚨 STUCK TASK INTERVENTION DETECTED 🚨**\n\n`;
        instructions += `**Task: ${task.title}**\n`;
        instructions += `**Repetition Count:** ${repetitionInfo.count} times in the last hour\n`;
        instructions += `**Status:** ${task.status}\n\n`;
        
        instructions += `## 🔄 TASK REPETITION DETECTED\n\n`;
        instructions += `This task has been accessed ${repetitionInfo.count} times recently without completion. `;
        instructions += `This suggests you may be stuck or encountering difficulties.\n\n`;
        
        instructions += `## 📋 COMPLETION CHECKLIST\n\n`;
        instructions += `**Before working on this task further, please:**\n\n`;
        instructions += `1. **Review what you've already tried** - Check if you're repeating the same approach\n`;
        instructions += `2. **Identify specific blockers** - What exactly is preventing completion?\n`;
        instructions += `3. **Break down the work** - Can this be split into smaller, manageable pieces?\n`;
        instructions += `4. **Check dependencies** - Are there prerequisite tasks that need to be completed first?\n\n`;
        
        instructions += `## 🎯 COMPLETION STRATEGIES\n\n`;
        instructions += `**Choose ONE approach:**\n\n`;
        
        instructions += `### Option A: Complete the Task\n`;
        instructions += `If you can finish this task now:\n`;
        instructions += `- Focus on the specific remaining work\n`;
        instructions += `- **IMMEDIATELY** mark as completed when done using:\n`;
        ...
        instructions += `### Option B: Break Down the Task\n`;
        instructions += `If the task is too large or complex:\n`;
        instructions += `- Create smaller, specific subtasks\n`;
        instructions += `- Mark this task as completed\n`;
        instructions += `- Use the TaskManager API to create new focused tasks\n\n`;
        
        instructions += `### Option C: Escalate or Skip\n`;
        instructions += `If you're blocked by external factors:\n`;
        instructions += `- Document the specific blocker\n`;
        instructions += `- Mark task as 'blocked' if that status is available\n`;
        instructions += `- Move to the next available task\n\n`;
        
        instructions += `## ⚠️ CRITICAL REQUIREMENT\n\n`;
        instructions += `**YOU MUST** take definitive action on this task. `;
        instructions += `Continuing to access it without completion will trigger additional interventions.\n\n`;
        ...
        instructions += `## 🔧 TASK COMPLETION COMMAND\n\n`;
        instructions += `When you finish this task, run:\n`;
        instructions += `\`\`\`bash\n`;
        instructions += `node -e "const TaskManager = require('./lib/taskManager'); const tm = new TaskManager('./TODO.json'); tm.updateTaskStatus('${task.id}', 'completed').then(() => console.log('Task marked as completed'));"\n`;
        instructions += `\`\`\`\n\n`;
        
        instructions += `**Remember:** The goal is progress, not perfection. Take action to move forward.`;
    ...
    async _generateCompletionInstructions(task) {
        ...
        if (instructions.lintingCommands.length === 0) {
            ...
            instructions.summary.push('Run general project linting');
        }
    ...
    _generateLintCommands(extensions, files) {
        ...
        if (extensions.has('js') || extensions.has('ts') || extensions.has('jsx') || extensions.has('tsx')) {
            ...
            if (jsFiles.length > 0 && jsFiles.length <= 5) {
                commands.push({
                    command: `npx eslint ${jsFiles.join(' ')}`,
        ...
        if (extensions.has('py')) {
            commands.push({
                ...
                description: 'Run flake8 linting on Python files',
                scope: 'python',
