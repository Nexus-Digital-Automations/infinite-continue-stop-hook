    ...
    async updateTaskStatus(taskId, status, _options = {}) {
        ...
        if (task) {
            ...
            if (status === 'completed' && oldStatus !== 'completed') {
                // 🚨 CRITICAL: ENFORCE PERFECTION STANDARDS BEFORE COMPLETION
                // Prevent premature task completion by validating ALL quality gates
                
                // Allow bypass for system-level fixes (like fixing the validation system itself)
                if (!_options.bypassValidation) {
                    const validationResult = await this._validateTaskCompletion(task, _options);
                    
                    if (!validationResult.canComplete) {
                    // Task completion BLOCKED due to quality issues
                    console.error(`Task completion blocked: ${validationResult.blockingReason}`);
                    console.error(`Issues detected: ${validationResult.issues.join(', ')}`);
                    console.error(`Resolution required: ${validationResult.resolutionSteps.join(' → ')}`);
                    
                    // Create task to address validation requirements
                    const fixTaskId = await this._createBlockingIssueTask(task, validationResult);
                    console.error(`Created validation task: ${fixTaskId}`);
                    
                    // Revert status back to in_progress and add validation failure details
                    task.status = 'in_progress';
                    task.completion_blocked = {
                        timestamp: new Date().toISOString(),
                        reason: validationResult.blockingReason,
                        issues: validationResult.issues,
                        resolution_task_id: fixTaskId,
                        attempts: (task.completion_blocked?.attempts || 0) + 1
                    };
                    
                    await this.writeTodo(todoData);
                    
                    // Throw error to prevent completion
                    throw new Error(`Task completion blocked: ${validationResult.blockingReason}. Issues: ${validationResult.issues.join(', ')}. Resolution task created: ${fixTaskId}`);
                    }
                    
                    // 🎉 All quality gates passed - allow completion
                    console.log(`✅ Task completion validated: All quality gates passed for ${task.title}`);
                    
                    // Generate completion instructions with linting recommendations
                    const completionInstructions = await this._generateCompletionInstructions(task);
                    
                    // Add completion instructions to task for reference
                    task.completion_instructions = completionInstructions;
                    task.completed_at = new Date().toISOString();
                    task.validation_passed = {
                        timestamp: new Date().toISOString(),
                        checks_performed: validationResult.checksPerformed,
                        quality_score: validationResult.qualityScore
                    };
                    
                    // Clear any previous completion blocks
                    delete task.completion_blocked;
                } else {
                    // Bypass validation - for system-level fixes
                    console.log(`⚠️ Task completion validation BYPASSED for system fix: ${task.title}`);
                    
                    // Generate basic completion instructions
                    const completionInstructions = await this._generateCompletionInstructions(task);
                    
                    // Add completion instructions to task for reference  
                    task.completion_instructions = completionInstructions;
                    task.completed_at = new Date().toISOString();
                    task.validation_bypassed = {
                        timestamp: new Date().toISOString(),
                        reason: 'System-level fix - validation bypassed',
                        agent: _options.agentId || 'system'
                    };
...
After addressing any linting issues, you can continue with your next task. Clean code is maintainable code!`;
    ...
    async _generateCompletionInstructions(task) {
        ...
        if (instructions.lintingCommands.length === 0) {
            // Add both Python and TypeScript linting commands as fallback
    ...
    generateLinterFeedback(completionFeedback) {
        ...
        let feedback = `🚨🚨🚨 TASK COMPLETED - IMMEDIATE LINTER CHECKS MANDATORY 🚨🚨🚨\n\n`;
        feedback += `**STOP EVERYTHING YOU ARE DOING RIGHT NOW!**\n\n`;
        feedback += `Task "${taskTitle}" has been marked as completed.\n\n`;
        feedback += `**YOU MUST IMMEDIATELY RUN LINTER CHECKS ON ALL FILES AND FOLDERS YOU WORKED ON/EDITED!**\n`;
        feedback += `**THIS IS NOT OPTIONAL - THIS IS MANDATORY!**\n`;
        feedback += `**DO NOT CONTINUE WITH ANY OTHER WORK UNTIL LINTING IS COMPLETE!**\n\n`;
        
        feedback += `**MANDATORY LINTING PROTOCOL:**\n\n`;
        ...
        if (hasPythonContext) {
            feedback += `**🐍 PYTHON PROJECT DETECTED - EXECUTE THESE COMMANDS IMMEDIATELY! 🐍**\n`;
            feedback += `**RUN THESE NOW - NO DELAYS, NO EXCUSES!**\n`;
            feedback += `\`\`\`bash\n`;
            feedback += `ruff check . --fix                           # FIX ALL Python linting issues NOW!\n`;
            feedback += `black .                                     # FORMAT Python code NOW!\n`;
            feedback += `isort .                                     # SORT Python imports NOW!\n`;
            feedback += `mypy . --ignore-missing-imports             # TYPE CHECK Python files NOW!\n`;
            feedback += `pytest --tb=short                           # RUN Python tests NOW!\n`;
            if (pythonFiles.length > 0) {
                feedback += `# SPECIFIC FILES YOU MUST LINT IMMEDIATELY:\n`;
                pythonFiles.forEach(file => {
                    feedback += `ruff check ${file} --fix              # LINT ${file} NOW!\n`;
                    feedback += `black ${file}                        # FORMAT ${file} NOW!\n`;
                    feedback += `mypy ${file} --ignore-missing-imports # TYPE CHECK ${file} NOW!\n`;
                });
            }
            feedback += `\`\`\`\n\n`;
        }
        ...
        if (hasJsContext && projectType !== 'python') {
            feedback += `**⚡ TYPESCRIPT/JAVASCRIPT PROJECT DETECTED - EXECUTE IMMEDIATELY! ⚡**\n`;
            feedback += `**RUN THESE COMMANDS NOW - NO DELAYS, NO EXCUSES!**\n`;
            feedback += `\`\`\`bash\n`;
            feedback += `eslint . --max-warnings 0 --fix           # FIX ALL JS/TS linting issues NOW!\n`;
            feedback += `prettier --write .                         # FORMAT JS/TS code NOW!\n`;
            if (projectType === 'typescript') {
                feedback += `tsc --noEmit --strict                      # TYPE CHECK files NOW!\n`;
            }
            feedback += `npm test                                   # RUN JS/TS tests NOW!\n`;
            if (jsFiles.length > 0) {
                feedback += `# SPECIFIC FILES YOU MUST LINT IMMEDIATELY:\n`;
                jsFiles.forEach(file => {
                    feedback += `eslint ${file} --fix                 # LINT ${file} NOW!\n`;
                    feedback += `prettier --write ${file}             # FORMAT ${file} NOW!\n`;
                });
            }
            feedback += `\`\`\`\n\n`;
        }
        
        // Intelligent fallback based on project type detection
        if (!hasPythonContext && !hasJsContext) {
            feedback += `**🔍 PROJECT TYPE DETECTION: ${projectType.toUpperCase()}**\n\n`;
            
            if (projectType === 'python') {
                feedback += `**🐍 PYTHON PROJECT DETECTED - EXECUTE PYTHON TOOLS IMMEDIATELY! 🐍**\n`;
                feedback += `**RUN THESE COMMANDS NOW - NO DELAYS, NO EXCUSES!**\n`;
                feedback += `\`\`\`bash\n`;
                feedback += `ruff check . --fix                       # FIX ALL Python linting issues NOW!\n`;
                feedback += `black .                                 # FORMAT Python code NOW!\n`;
                ...
                feedback += `**⚡ TYPESCRIPT PROJECT DETECTED - EXECUTE TYPESCRIPT TOOLS IMMEDIATELY! ⚡**\n`;
                feedback += `**RUN THESE COMMANDS NOW - NO DELAYS, NO EXCUSES!**\n`;
                feedback += `\`\`\`bash\n`;
                feedback += `eslint . --max-warnings 0 --fix         # FIX ALL TS linting issues NOW!\n`;
                feedback += `prettier --write .                       # FORMAT TypeScript code NOW!\n`;
                feedback += `tsc --noEmit --strict                    # TYPE CHECK files NOW!\n`;
                feedback += `npm test                                 # RUN TypeScript tests NOW!\n`;
                feedback += `\`\`\`\n\n`;
            } else if (projectType === 'javascript') {
                feedback += `**⚡ JAVASCRIPT PROJECT DETECTED - EXECUTE JAVASCRIPT TOOLS IMMEDIATELY! ⚡**\n`;
                feedback += `**RUN THESE COMMANDS NOW - NO DELAYS, NO EXCUSES!**\n`;
                feedback += `\`\`\`bash\n`;
                feedback += `eslint . --max-warnings 0 --fix         # FIX ALL JS linting issues NOW!\n`;
                feedback += `prettier --write .                       # FORMAT JavaScript code NOW!\n`;
                feedback += `npm test                                 # RUN JavaScript tests NOW!\n`;
                feedback += `\`\`\`\n\n`;
            } else if (projectType === 'mixed') {
                feedback += `**🔀 MIXED PROJECT DETECTED - EXECUTE APPROPRIATE TOOLS IMMEDIATELY! 🔀**\n`;
                feedback += `**RUN THESE COMMANDS NOW - NO DELAYS, NO EXCUSES!**\n`;
                feedback += `\`\`\`bash\n`;
                feedback += `# PYTHON TOOLS:\n`;
                feedback += `ruff check . --fix && black . && isort . # PYTHON LINTING NOW!\n`;
                feedback += `mypy . --ignore-missing-imports         # PYTHON TYPE CHECK NOW!\n`;
                feedback += `pytest --tb=short                       # PYTHON TESTS NOW!\n\n`;
                feedback += `# JAVASCRIPT/TYPESCRIPT TOOLS:\n`;
                feedback += `eslint . --fix && prettier --write .     # JS/TS LINTING NOW!\n`;
                feedback += `npm test                                 # JS/TS TESTS NOW!\n`;
                feedback += `\`\`\`\n\n`;
            } else {
                feedback += `**🔥 UNKNOWN PROJECT TYPE - RUN AVAILABLE TOOLS IMMEDIATELY! 🔥**\n`;
                feedback += `**EXECUTE THESE COMMANDS NOW - FIND AND FIX ALL ISSUES!**\n`;
                feedback += `\`\`\`bash\n`;
        ...
        feedback += `**🚨 ABSOLUTE NON-NEGOTIABLE REQUIREMENTS 🚨**\n`;
        feedback += `**THESE ARE MANDATORY - NO EXCEPTIONS!**\n`;
        feedback += `- ❌ **ZERO LINTING ERRORS** must remain - FIX ALL NOW!\n`;
        feedback += `- ❌ **ZERO FORMATTING ISSUES** must remain - FORMAT ALL NOW!\n`;
        feedback += `- ❌ **ZERO TYPE ERRORS** must remain - FIX ALL NOW!\n`;
        feedback += `- ✅ **ALL LINTING TOOLS** must pass completely - VERIFY NOW!\n`;
        feedback += `- ✅ **CREATE HIGH-PRIORITY TASKS** for any issues not immediately fixable!\n\n`;
        
        feedback += `**⚠️ IF YOU ENCOUNTER ANY LINTING ERRORS - IMMEDIATE ACTION REQUIRED:**\n`;
        feedback += `1. ❌ **DO NOT IGNORE THEM** - This is FORBIDDEN!\n`;
        feedback += `2. ❌ **DO NOT ADD TO IGNORE FILES** - This is FORBIDDEN!\n`;
        feedback += `3. ✅ **FIX THEM IMMEDIATELY** - This is MANDATORY!\n`;
        feedback += `4. ✅ **CREATE HIGH-PRIORITY TASK** if not immediately fixable!\n\n`;
        
        feedback += `**🔥 EXECUTE THE LINTING COMMANDS RIGHT NOW - DO NOT DELAY! 🔥**\n`;
        feedback += `**DO NOT CONTINUE WITH ANY OTHER WORK UNTIL ALL LINTING IS PERFECT!**\n`;
    ...
    _sortTasks(tasks) {
        return tasks.sort((a, b) => {
            // Primary sort: Category rank (lower rank = higher priority)
            ...
            // Secondary sort: Priority value (higher value = higher priority)
            ...
            // Tertiary sort: Creation time (newer first for same category and priority)
    ...
    /**
     * 🚨 CRITICAL: Validate task completion with comprehensive quality checks
     * This method enforces ZERO TOLERANCE for incomplete or low-quality work
     * @param {Object} task - Task object to validate
     * @param {Object} options - Validation options
     * @returns {Object} Validation result with canComplete flag and details
     */
    async _validateTaskCompletion(task, options = {}) {
        const checks = [];
        const issues = [];
        ...
        try {
            // 🚨 NEW: MANDATORY VALIDATION EVIDENCE CHECK
            if (!options.validationEvidence && !options.bypassValidation) {
                issues.push('Validation evidence required: command outputs, testing results, requirement verification, completion statement');
                resolutionSteps.push('Provide validation evidence with command outputs showing all checks pass');
                qualityScore -= 50;
                
                return {
                    canComplete: false,
                    blockingReason: 'Missing validation evidence - task completion requires proof of validation',
                    issues,
                    resolutionSteps,
                    qualityScore,
                    checksPerformed: ['validation_evidence'],
            ...
            if (options.validationEvidence) {
                ...
                if (!evidence.commands || !Array.isArray(evidence.commands) || evidence.commands.length === 0) {
                    issues.push('Validation evidence missing command outputs');
                    qualityScore -= 15;
                }
                if (!evidence.testing || typeof evidence.testing !== 'string' || evidence.testing.trim().length === 0) {
                    issues.push('Validation evidence missing testing results');
                    qualityScore -= 15;
                }
                if (!evidence.requirements || typeof evidence.requirements !== 'string' || evidence.requirements.trim().length === 0) {
                    issues.push('Validation evidence missing requirement verification');
                    qualityScore -= 15;
                }
                if (!evidence.statement || typeof evidence.statement !== 'string' || evidence.statement.trim().length === 0) {
                    issues.push('Validation evidence missing completion statement');
                    qualityScore -= 15;
                }
            }
            // 1. MANDATORY: Check TypeScript compilation for development tasks
            if (task.mode === 'DEVELOPMENT' || task.title.toLowerCase().includes('typescript')) {
                const tsCheck = await this._checkTypeScriptCompilation();
                checks.push('typescript_compilation');
                
                if (!tsCheck.success) {
                    issues.push(`TypeScript compilation errors: ${tsCheck.errorCount} errors found`);
                    resolutionSteps.push('Fix all TypeScript compilation errors');
                    qualityScore -= 30;
                }
            }

            // 2. MANDATORY: Check for linting errors
            const lintCheck = await this._checkLinting();
            checks.push('linting');
            
            if (!lintCheck.success) {
                issues.push(`Linting errors: ${lintCheck.errorCount} issues found`);
                resolutionSteps.push('Fix all linting errors using autofix and manual fixes');
                qualityScore -= 20;
            }

            // 3. MANDATORY: Check build success for development tasks
            if (task.mode === 'DEVELOPMENT') {
                const buildCheck = await this._checkBuildSuccess();
                checks.push('build_verification');
                
                if (!buildCheck.success) {
                    issues.push(`Build failed: ${buildCheck.error}`);
                    resolutionSteps.push('Ensure all builds complete successfully');
                    qualityScore -= 25;
                }
            }

            // 4. MANDATORY: Execute quality gates if defined
            if (task.success_criteria && task.success_criteria.length > 0) {
                const qualityGatesResult = await this.executeQualityGates(task.id);
                checks.push('quality_gates');
                
                if (!qualityGatesResult.success) {
                    const failedCriteria = qualityGatesResult.results?.filter(r => !r.passed) || [];
                    issues.push(`Quality gates failed: ${failedCriteria.length} criteria not met`);
                    resolutionSteps.push('Satisfy all success criteria defined for the task');
                    qualityScore -= 15;
                }
            }

            // 5. MANDATORY: Check for test failures for testing tasks
            if (task.mode === 'TESTING' || task.title.toLowerCase().includes('test')) {
                const testCheck = await this._checkTestSuccess();
                checks.push('test_verification');
                
                if (!testCheck.success) {
                    issues.push(`Tests failing: ${testCheck.failureCount} test failures`);
                    resolutionSteps.push('Fix all failing tests');
                    qualityScore -= 20;
                }
            }

            // 6. MANDATORY: Check for missing critical files
            if (task.important_files && task.important_files.length > 0) {
                const fileCheck = await this._checkImportantFiles(task.important_files);
                checks.push('file_verification');
                
                if (!fileCheck.success) {
                    issues.push(`Missing files: ${fileCheck.missingFiles.join(', ')}`);
                    resolutionSteps.push('Create or fix all important files listed in task');
                    qualityScore -= 10;
                }
            }

            // 7. ADVANCED: Check for security vulnerabilities
            const securityCheck = await this._checkSecurityIssues();
            checks.push('security_audit');
            
            if (!securityCheck.success && securityCheck.criticalIssues > 0) {
                issues.push(`Security vulnerabilities: ${securityCheck.criticalIssues} critical issues`);
                resolutionSteps.push('Fix all security vulnerabilities');
                qualityScore -= 15;
            }

            // 8. PERFECTION ENFORCEMENT: Zero tolerance check
            const canComplete = issues.length === 0 && qualityScore >= 95;
            
            return {
                canComplete,
                blockingReason: issues.length > 0 ? 
                    `${issues.length} quality issues prevent completion` : 
                    (qualityScore < 95 ? `Quality score ${qualityScore} below required 95%` : null),
                issues,
                resolutionSteps,
            ...
            return {
                canComplete: false,
                blockingReason: `Validation error: ${error.message}`,
                issues: [`Validation system error: ${error.message}`],
                resolutionSteps: ['Fix validation system error and retry'],
                checksPerformed: checks,
                qualityScore: 0,
                timestamp: new Date().toISOString()
            };
    ...
    /**
     * Check TypeScript compilation or Python type checking status
     */
    async _checkTypeScriptCompilation() {
        try {
            const { exec } = require('child_process');
            const commands = this._getValidationCommands();
            ...
            return new Promise((resolve) => {
                exec(typeCheckCommand, { timeout: 60000 }, (error, stdout, stderr) => {
                    if (error) {
                        ...
                        const projectType = this._detectProjectType();
                        const successMessage = projectType === 'python' ? 
                            'Python type checking successful' : 
            ...
            return { success: false, errorCount: 1, output: `Check failed: ${error.message}` };
        }
    }

    /**
     * Detect project type based on project files and structure
     */
    _detectProjectType() {
        const projectDir = _path.dirname(this.todoPath);
        ...
        const hasPythonIndicators = pythonFiles.some(file => 
            fs.existsSync(_path.join(projectDir, file))
        );
        
        // Check for Python source files in common locations
        const pythonSourceDirs = ['src', 'app', '.'];
        const hasPythonSource = pythonSourceDirs.some(dir => {
            const dirPath = _path.join(projectDir, dir);
        ...
        if (hasPythonIndicators || hasPythonSource) {
            return 'python';
        }
        
        // Check for Node.js/TypeScript project indicators
        const nodeFiles = ['package.json', 'tsconfig.json', 'yarn.lock', 'pnpm-lock.yaml'];
        const hasNodeIndicators = nodeFiles.some(file => 
            fs.existsSync(_path.join(projectDir, file))
        );
        ...
        // Default to nodejs for backwards compatibility
        return 'nodejs';
    }

    /**
     * Get validation commands based on project type
     */
    _getValidationCommands(projectType = null) {
        const type = projectType || this._detectProjectType();
        
        if (type === 'python') {
            return {
                lint: 'ruff check .',
                typeCheck: 'mypy . --strict',
                build: 'python -m py_compile $(find . -name "*.py" | head -10)', // Compile check sample files
                test: 'pytest'
            };
        } else {
            // Default to Node.js/TypeScript commands
            return {
                lint: 'npm run lint',
    ...
    /**
     * Get validation success criteria based on project type
     */
    _getValidationSuccessCriteria(projectType = null) {
        const commands = this._getValidationCommands(projectType);
        return [
            commands.lint,
            commands.typeCheck,
            commands.build,
    ...
    async _checkLinting() {
        try {
            ...
            return new Promise((resolve) => {
                exec(lintCommand, { timeout: 60000 }, (error, stdout, stderr) => {
                    if (error) {
                        const output = stdout + stderr;
                        const errorLines = output.split('\n').filter(line => 
                            line.includes('error') || line.includes('✖') || line.includes('E')
                        );
                        resolve({
                            success: false,
                            errorCount: errorLines.length,
    ...
    async _checkBuildSuccess() {
        try {
            ...
            return new Promise((resolve) => {
                exec(buildCommand, { timeout: 300000 }, (error, stdout, stderr) => {
                    if (error) {
                        resolve({
                            success: false,
                            error: error.message,
                        ...
                        const projectType = this._detectProjectType();
                        const successMessage = projectType === 'python' ? 
                            'Python compilation check successful' : 
                            'Build successful';
    ...
    async _checkTestSuccess() {
        try {
            ...
            return new Promise((resolve) => {
                exec(testCommand, { timeout: 300000 }, (error, stdout, stderr) => {
                    if (error) {
                        const output = stdout + stderr;
                        let failureCount = 1;
                        
                        // Parse failures based on project type
                        const projectType = this._detectProjectType();
                        if (projectType === 'python') {
                            // Parse pytest failures
                            const failureMatch = output.match(/(\d+) failed/);
                            failureCount = failureMatch ? parseInt(failureMatch[1]) : 1;
                        } else {
                            // Parse npm test failures
            ...
            return { success: false, failureCount: 1, output: `Check failed: ${error.message}` };
        }
    }
     ...
     */
    async _createBlockingIssueTask(originalTask, validationResult) {
        const taskId = `task_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
        
        const blockingTask = {
            id: taskId,
            title: `Resolve validation requirements: ${originalTask.title}`,
...
**Resolution Steps:**
${validationResult.resolutionSteps.map((step, i) => `${i + 1}. ${step}`).join('\n')}
...
This task blocks completion of "${originalTask.title}" until validation requirements are met.`,
            mode: 'DEVELOPMENT',
            priority: 'critical',
            status: 'pending',
            dependencies: [],
            important_files: originalTask.important_files || [],
            success_criteria: this._getValidationSuccessCriteria(),
            estimate: '2-4 hours',
            requires_research: false,
            subtasks: [],
            created_at: new Date().toISOString(),
            blocking_for: originalTask.id,
            blocking_reason: validationResult.blockingReason,
            auto_created: true,
            urgent: true
        };
        ...
        // Insert at the very beginning of the tasks array (highest priority)
        todoData.tasks.unshift(blockingTask);
     ...
     */
    async distributeTasksToAgents(tasks, agents) {
        const assignments = [];
        ...
        for (let i = 0; i < tasks.length && i < agents.length; i++) {
            ...
            await this.assignTaskToAgent(task.id, agent.agentId, 'primary');
            
            // Update task status to in_progress for concurrent processing
            await this.updateTaskStatusConcurrent(task.id, 'in_progress');
     ...
     */
    async updateTaskStatusConcurrent(taskId, status, notes = '') {
        // Bypass the single in_progress validation for concurrent processing
        const todoData = await this.readTodoFast();
        const task = todoData.tasks.find(t => t.id === taskId);
        ...
        // Update task status without single-task restriction
        task.status = status;
        ...
        // Use standard write method - validation now supports concurrent tasks
        await this.writeTodo(todoData);
        return true;
    }
    ...
    async getNextPendingTask() {
        ...
        // Find first pending task (highest priority first due to auto-sorting)
        const nextTask = todoData.tasks.find(task => 
            task && task.status === 'pending'
        );
        
        if (nextTask) {
            // Claim the task immediately to prevent conflicts
            nextTask.status = 'in_progress';
            nextTask.started_at = new Date().toISOString();
            nextTask.claimed_by = `agent_${Date.now()}`; // Simple claim identifier
