...
class TaskManager {
  ...
  async writeTodo(data) {
    try {
      ...
      if (!writeResult.success) {
        ...
        if (fs.existsSync(backupPath)) {
          try {
            ...
            console.log('🔄 Restored TODO.json from backup after write failure');
          } catch (restoreError) {
      ...
      try {
        ...
        if (writtenContent.trim().startsWith('"') && writtenContent.trim().endsWith('"')) {
          console.error('🚨 CRITICAL: TODO.json was corrupted with double-encoding after write!');

          // Attempt to fix immediately
          const fixResult = await this.autoFixer.fixJsonFile(this.todoPath);
          if (fixResult.fixed) {
            console.log('✅ Successfully fixed TODO.json corruption immediately after detection');
          } else {
        ...
        console.error('🚨 CRITICAL: TODO.json verification failed after write!', verificationError.message);

        // Attempt to restore from backup
        if (fs.existsSync(backupPath)) {
          try {
            fs.copyFileSync(backupPath, this.todoPath);
            console.log('🔄 Restored TODO.json from backup after verification failure');
          } catch (restoreError) {
            console.error(`Failed to restore backup: ${restoreError.message}`);
          }
        }
        throw new Error(`TODO.json verification failed: ${verificationError.message}`);
      }
  ...
  async createFeature(featureData, userAuthorized = false) {
    ...
    if (!userAuthorized) {
      throw new Error('Feature creation requires explicit user authorization. Features cannot be added without user approval.');
    }
  ...
  async approveFeature(featureId, userId = 'user') {
    const todoData = await this.readTodoFast();
    if (!todoData.features) {return false;}
    ...
    if (feature.status !== 'suggested') {
      throw new Error(`Feature ${featureId} is not in suggested status (current: ${feature.status})`);
    }
  ...
  async rejectFeature(featureId, userId = 'user', reason = '') {
    ...
    console.log(`Feature rejected: ${feature.title} by ${userId}${reason ? ` - Reason: ${reason}` : ''}`);
  ...
  async getFeature(featureId) {
    const todoData = await this.readTodoFast();
    if (!todoData.features) {return null;}
  ...
  async updateFeature(featureId, updates) {
    ...
    if (updates.subtasks || todoData.tasks) {
      feature.metadata.completion_percentage = this._calculateFeatureCompletion(feature, todoData.tasks);
    }
  ...
  _calculateFeatureCompletion(feature, tasks) {
    if (!feature.subtasks || feature.subtasks.length === 0) {return 0;}
    ...
    const completedCount = subtaskStatuses.filter(status => status === 'completed').length;
  ...
  async createTask(taskData) {
    // Validate required task_type parameter
    if (!taskData.task_type) {
      throw new Error('task_type parameter is required. Must be one of: error, feature, subtask, test');
    }

    // Validate task_type value
    const validTaskTypes = ['error', 'feature', 'subtask', 'test'];
    if (!validTaskTypes.includes(taskData.task_type)) {
      throw new Error(`Invalid task_type '${taskData.task_type}'. Must be one of: ${validTaskTypes.join(', ')}`);
    }
    ...
    // Use provided task_type to assign appropriate ID prefix
    const taskTypePrefix = taskData.task_type;
    ...
    // Map task_type to category for priority ordering
    ...
    const newTask = {
      ...
      mode: taskData.mode,
      priority: priority,
      category: taskData.category || taskTypeToCategory[taskData.task_type] || 'feature',
    ...
    if (phaseInfo && !taskData.skip_phase_insertion) {
      try {
        const phaseInsertionResult = await this.performIntelligentPhaseInsertion(newTask, true);
        if (phaseInsertionResult.success && phaseInsertionResult.renumbering.length > 0) {
          console.log(`TaskManager: Intelligent phase insertion completed - renumbered ${phaseInsertionResult.renumbering.length} tasks`);
        }
      } catch (error) {
        console.warn('TaskManager: Phase insertion failed, continuing with normal creation:', error.message);
  ...
  /**
     * Inserts task into correct section based on three-section structure:
     * SECTION 1: ERROR TASKS (linter-error, build-error, start-error, test-error, test-linter-error)
     * SECTION 2: FEATURE TASKS (all development features and subtasks)
     * SECTION 3: REVIEW TASKS (test, validation, review categories)
     * @private
     * @param {Object} todoData - TODO data object
     * @param {Object} task - Task to insert
     */
  _insertTaskInCorrectSection(todoData, task) {
    const errorCategories = ['linter-error', 'build-error', 'start-error', 'error'];
    const reviewCategories = ['missing-test', 'test-setup', 'test-refactor', 'test-performance', 'test-feature', 'test-failure', 'test-linter-failure', 'test-coverage', 'validation', 'review', 'testing'];
    ...
    // SECTION 3: REVIEW TASKS - Insert at end (lowest priority)
    if (reviewCategories.includes(task.category) ||
    ...
    // SECTION 2: FEATURE TASKS - Insert between error and review sections
    // Find the insertion point (after all error tasks, before review tasks)
    let insertIndex = todoData.tasks.length;

    // Find first review task to insert before it
    for (let i = 0; i < todoData.tasks.length; i++) {
      const currentTask = todoData.tasks[i];
      if (reviewCategories.includes(currentTask.category) ||
  ...
  async getExecutableTasks() {
    ...
    return executable.sort((a, b) => {
      ...
      if (a.parent_feature && b.parent_feature) {
        const aFeature = todoData.features?.find(f => f.id === a.parent_feature);
        const bFeature = todoData.features?.find(f => f.id === b.parent_feature);

        if (aFeature && bFeature && this._isFeatureEarlier(aFeature, bFeature)) {
          return -1;
        } else if (aFeature && bFeature && this._isFeatureEarlier(bFeature, aFeature)) {
          return 1;
        }
      ...
      if (a.parent_feature && !b.parent_feature) {return -1;}
      if (!a.parent_feature && b.parent_feature) {return 1;}
...
After addressing any linting issues, you can continue with your next task. Clean code is maintainable code!`;
  ...
  _validateTaskClaiming(taskId, todoData, options = {}) {
    ...
    if (!options.allowOutOfOrder) {
      ...
      const pendingErrorTasks = todoData.tasks.filter(t =>
        t.status === 'pending' &&
                !t.assigned_agent &&
                t.category === 'error',
      );
      ...
      if (task.parent_feature) {
        const feature = todoData.features?.find(f => f.id === task.parent_feature);
        if (feature) {
          ...
          if (incompleteEarlierFeatures.length > 0) {
            ...
            const earlierTasks = todoData.tasks.filter(t =>
          ...
          const earlierIncompleteTasks = featureTasks.slice(0, taskIndex)
        ...
        const earlierPendingTasks = todoData.tasks.slice(0, taskIndex)
          .filter(t => t.status === 'pending' && !t.assigned_agent && !t.parent_feature);
  ...
  _isFeatureEarlier(featureA, featureB) {
    ...
    const getFeatureNumber = (feature) => {
      const match = feature.title.match(/(?:Feature|Phase)\s+(\d+)(?:\.(\d+))?/i);
      if (match) {
  ...
  _compareTaskOrder(taskA, taskB) {
    ...
    const getSubtaskNumber = (task) => {
      const match = task.title.match(/(?:Subtask|Step)\s+(\d+)(?:\.(\d+))?/i);
      if (match) {
        const major = parseInt(match[1], 10);
        const minor = match[2] ? parseInt(match[2], 10) : 0;
  ...
  async insertFeature(featureData, insertAtNumber, replace = false) {
    ...
    if (!replace) {
      ...
      todoData.tasks.forEach(task => {
        const match = task.title.match(/(?:Feature|Phase)\s+(\d+)(?:\.(\d+))?/i);
        if (match) {
          const featureNum = parseInt(match[1], 10);
          if (featureNum >= insertAtNumber) {
  ...
  async insertSubtask(taskData, featureId, insertAtNumber, replace = false) {
    ...
    const featureTasks = todoData.tasks.filter(t => t.parent_feature === featureId);
  ...
  _sortTasks(tasks) {
    ...
    return tasks.sort((a, b) => {
      // PRIMARY SORT: Phase ordering (SUPREME PRIORITY - phases trump all else)
      const phaseA = this._extractPhase(a.title);
      const phaseB = this._extractPhase(b.title);

      // If both tasks have phases, sort by phase first (THIS OVERRIDES EVERYTHING)
      if (phaseA && phaseB) {
        const phaseComparison = this._comparePhases(phaseA, phaseB);
        if (phaseComparison !== 0) {
          return phaseComparison;
        }
        // If phases are identical, continue to secondary sorting within same phase
      }

      // If only one has a phase, phase tasks always come first
      if (phaseA && !phaseB) {return -1;}
      if (!phaseA && phaseB) {return 1;}

      // SECONDARY SORT: Dependency relationships (within same phase or non-phase tasks)
      ...
      // TERTIARY SORT: Category rank (lower rank = higher priority)
      ...
      // QUATERNARY SORT: Priority value (higher value = higher priority)
      ...
      // QUINARY SORT: Creation time (newer first for same category and priority)
  ...
  _checkPhaseInsertion(newPhase, existingTasks) {
    if (!newPhase) {return { hasConflicts: false, renumberingNeeded: [] };}
    ...
    existingTasks.forEach(task => {
      ...
      if (existingPhase.major === newPhase.major &&
      ...
      if (shouldRenumber) {
        const newPhaseNumber = this._calculateNewPhaseNumber(existingPhase, newPhase);
  ...
  async performIntelligentPhaseInsertion(newTaskData, autoRenumber = true) {
    ...
    if (!newPhase) {
      ...
      return { success: true, renumbering: [], message: 'No phase detected, normal task creation' };
    }
    ...
    if (!insertionAnalysis.needsRenumbering) {
      ...
      return { success: true, renumbering: [], message: 'No phase conflicts detected' };
    }
    ...
    try {
      ...
      for (const renumberInfo of insertionAnalysis.renumberingNeeded) {
        const task = currentData.tasks.find(t => t.id === renumberInfo.taskId);
        if (!task) {continue;}
  ...
  async revertStaleInProgressTasks() {
    ...
    const staleThreshold = new Date(now - this.options.staleTaskTimeout * 60 * 1000);
    const warningThreshold = new Date(now - this.options.staleTaskTimeout * 60 * 1000 * this.options.staleTaskWarningThreshold);

    logger.debug(`[STALE_CHECK] Checking tasks with timeout: ${this.options.staleTaskTimeout}min, warning at: ${Math.round(this.options.staleTaskTimeout * this.options.staleTaskWarningThreshold)}min`, {
      totalInProgress: todoData.tasks.filter(t => t.status === 'in_progress').length,
    ...
    for (const task of todoData.tasks) {
      if (task.status === 'in_progress' && task.started_at) {
        ...
        if (startedTime <= staleThreshold) {
          ...
          logger.warn(`[STALE_REVERT] Task ${task.id} (${task.title}) reverted after ${taskDurationMinutes} minutes`, {
            taskId: task.id,
            category: task.category,
        ...
        } else if (this.options.enableStaleTaskWarnings && startedTime <= warningThreshold) {
          ...
          warningTasks.push({
            ...
            assignedAgent: task.assigned_agent,
            durationMinutes: taskDurationMinutes,
            minutesUntilStale: this.options.staleTaskTimeout - taskDurationMinutes,
          });

          logger.warn(`[STALE_WARNING] Task ${task.id} (${task.title}) has been in progress for ${taskDurationMinutes}min, will be auto-reverted in ${this.options.staleTaskTimeout - taskDurationMinutes}min`, {
            taskId: task.id,
            category: task.category,
            assignedAgent: task.assigned_agent,
            durationMinutes: taskDurationMinutes,
            warningThresholdMin: Math.round(this.options.staleTaskTimeout * this.options.staleTaskWarningThreshold),
          });
    ...
    if (revertedTasks.length > 0) {
      ...
      logger.info(`[STALE_SUMMARY] Reverted ${revertedTasks.length} stale tasks`, {
    ...
    if (warningTasks.length > 0) {
      logger.info(`[STALE_WARNINGS] Issued warnings for ${warningTasks.length} tasks approaching stale threshold`, {
  ...
  _recordStaleTaskEvent(staleEvent) {
    ...
    const totalDuration = this._staleTaskHistory.reduce((sum, event) => sum + event.durationMinutes, 0);
    this._staleTaskStats.avgStaleTime = totalDuration / this._staleTaskHistory.length;

    // Track frequently stale agents
    if (staleEvent.assignedAgent) {
      const agentCount = this._staleTaskStats.frequentlyStaleAgents.get(staleEvent.assignedAgent) || 0;
      this._staleTaskStats.frequentlyStaleAgents.set(staleEvent.assignedAgent, agentCount + 1);
    }

    // Track frequently stale task categories
    if (staleEvent.category) {
      const categoryCount = this._staleTaskStats.frequentlyStaleTaskTypes.get(staleEvent.category) || 0;
      this._staleTaskStats.frequentlyStaleTaskTypes.set(staleEvent.category, categoryCount + 1);
    }

    logger.debug(`[STALE_STATS] Recorded stale event for task ${staleEvent.taskId}`, {
      totalStaleEvents: this._staleTaskStats.totalStaleEvents,
      avgStaleTimeMin: Math.round(this._staleTaskStats.avgStaleTime),
  ...
  _checkStaleTaskPatterns(staleEvent) {
    ...
    if (staleEvent.assignedAgent) {
      const agentStaleCount = this._staleTaskStats.frequentlyStaleAgents.get(staleEvent.assignedAgent);
      if (agentStaleCount >= 3) {
        logger.warn(`[PATTERN_ALERT] Agent ${staleEvent.assignedAgent} has had ${agentStaleCount} stale tasks - may need investigation`, {
    ...
    if (staleEvent.category) {
      const categoryStaleCount = this._staleTaskStats.frequentlyStaleTaskTypes.get(staleEvent.category);
      if (categoryStaleCount >= 5) {
        logger.warn(`[PATTERN_ALERT] Task category "${staleEvent.category}" has had ${categoryStaleCount} stale tasks - may indicate systematic issues`, {
          category: staleEvent.category,
    ...
    if (recentStaleCount >= 3) {
      logger.warn(`[PATTERN_ALERT] High stale task rate: ${recentStaleCount} stale tasks in the last hour`, {
        recentStaleCount,
        totalStaleEvents: this._staleTaskStats.totalStaleEvents,
        avgStaleTimeMin: Math.round(this._staleTaskStats.avgStaleTime),
      });
  ...
  _getLogger(component = 'TaskManager') {
    return {
      debug: (message, context = {}) => {
        if (this.options.logLevel === 'debug' || process.env.NODE_ENV === 'development') {
          console.log(`[DEBUG] [${component}] ${message}`, context);
        }
      },
      info: (message, context = {}) => {
        console.log(`[INFO] [${component}] ${message}`, context);
      },
      warn: (message, context = {}) => {
        console.warn(`[WARN] [${component}] ${message}`, context);
      },
      error: (message, context = {}) => {
        console.error(`[ERROR] [${component}] ${message}`, context);
      },
    };
  }
  ...
  getStaleTaskStatistics() {
    ...
    const stats = {
      ...
      statistics: {
        ...
        frequentlyStaleAgents: Object.fromEntries(this._staleTaskStats.frequentlyStaleAgents),
        frequentlyStaleTaskTypes: Object.fromEntries(this._staleTaskStats.frequentlyStaleTaskTypes),
      },
  ...
  _getMostProblematicCategory() {
    ...
    for (const [category, count] of this._staleTaskStats.frequentlyStaleTaskTypes) {
      if (count > maxCount) {
        maxCount = count;
    ...
    return maxCategory ? { category: maxCategory, staleTaskCount: maxCount } : null;
  }
  ...
  _shouldAutoCreateResearchTask(taskData) {
    ...
    const simpleCategories = ['chore', 'documentation', 'test-failure', 'linter-error', 'build-error'];
  ...
  _determineTaskTypeFromContent(taskData) {
    ...
    const isError = errorPatterns.some(pattern => pattern.test(allText)) ||
                       ['linter-error', 'build-error', 'start-error', 'error', 'bug'].includes(category);

    if (isError) {
      // Check if it's actually a test-related error (should be test_ not error_)
      const testRelated = /test.*error|test.*fail|coverage|spec|jest|mocha|cypress/.test(allText);
      if (testRelated && !/(build.*fail|compilation|cannot.*build|start.*fail)/.test(allText)) {
    ...
    const isTest = testPatterns.some(pattern => pattern.test(allText)) ||
                      category.startsWith('test-') ||
                      ['missing-test', 'test-setup', 'test-refactor', 'testing'].includes(category);
    ...
    if (taskData.feature_id || taskData.implementing_feature || taskData.parent_feature_id) {
      return 'subtask';
    }
...
**Research Priority:** This research task must be completed before implementation begins`;
  ...
  _validateTodoDataStructure(data) {
    ...
    if (typeof data === 'string') {
      throw new Error('TODO.json data cannot be a string - this causes corruption');
    }
    ...
    for (const field of requiredFields) {
      if (!(field in data)) {
        console.warn(`Missing required field in TODO.json: ${field}`);
        // Auto-fix missing fields
        if (field === 'project') {data.project = 'unknown-project';}
        if (field === 'tasks') {data.tasks = [];}
        if (field === 'current_mode') {data.current_mode = 'DEVELOPMENT';}
      }
    }

    // Validate tasks array
    if (!Array.isArray(data.tasks)) {
      console.warn('TODO.json tasks is not an array, converting');
      data.tasks = [];
    }
    ...
    if (data.features && !Array.isArray(data.features)) {
      console.warn('TODO.json features is not an array, converting');
      data.features = [];
    }

    // Validate agents object if present
    if (data.agents && typeof data.agents !== 'object') {
      console.warn('TODO.json agents is not an object, converting');
      data.agents = {};
    }

    console.log('✅ TODO.json data structure validation passed');
  }
}
