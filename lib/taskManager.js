...
class TaskManager {
    ...
    async getTaskContinuationGuidance(agentId) {
        ...
        if (currentTask) {
            ...
            const guidance = {
                ...
                nextTaskCommand: `node -e "const TaskManager = require('./lib/taskManager'); const tm = new TaskManager('./TODO.json'); tm.getNextTask('${agentId}', '${currentTask.id}').then(task => console.log(task ? 'Next task: ' + task.title : 'No more tasks'));"`
            };
    ...
    async updateTaskStatus(taskId, status) {
        ...
        if (task) {
            ...
            // If task is now completed, track directories for linter reminders
            if (status === 'completed' && oldStatus !== 'completed' && this.options.enableLinterReminders) {
