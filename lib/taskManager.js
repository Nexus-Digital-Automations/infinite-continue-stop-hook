...
class TaskManager {
    ...
    _insertTaskInCorrectSection(todoData, task) {
        ...
        const reviewCategories = ['missing-test', 'test-setup', 'test-refactor', 'test-performance', 'test-feature', 'test-error', 'test-linter-error', 'validation', 'review', 'testing'];
    ...
    async getExecutableTasks() {
        ...
        // Sort tasks with ERROR CATEGORIES having absolute priority
        return executable.sort((a, b) => {
            // Define error categories in priority order
            const errorCategories = [
                'linter-error', 'build-error', 'start-error', 'error', 
                'test-linter-error', 'test-error'
            ];
            
            const aIsError = errorCategories.includes(a.category);
            const bIsError = errorCategories.includes(b.category);
            
            // Error tasks always come first
            if (aIsError && !bIsError) return -1;
            if (!aIsError && bIsError) return 1;
            
            // If both are error tasks, sort by error category priority
            if (aIsError && bIsError) {
                const aErrorIndex = errorCategories.indexOf(a.category);
                const bErrorIndex = errorCategories.indexOf(b.category);
                if (aErrorIndex !== bErrorIndex) return aErrorIndex - bErrorIndex;
            }
...
After addressing any linting issues, you can continue with your next task. Clean code is maintainable code!`;
    ...
    _validateTaskClaiming(taskId, todoData, options = {}) {
        ...
        if (!options.allowOutOfOrder) {
            ...
            if (task.category === 'error' || task.category === 'build-error' || 
                task.category === 'linter-error' || task.category === 'start-error' ||
                task.category === 'test-error' || task.category === 'test-linter-error') {
                return { valid: true };
            }
            ...
            const pendingErrorTasks = todoData.tasks.filter(t => 
                t.status === 'pending' && 
                !t.assigned_agent &&
                (t.category === 'error' || t.category === 'build-error' || 
                 t.category === 'linter-error' || t.category === 'start-error' ||
                 t.category === 'test-error' || t.category === 'test-linter-error')
            );
            
            if (pendingErrorTasks.length > 0 && task.category !== 'error' && 
                task.category !== 'build-error' && task.category !== 'linter-error' && 
                task.category !== 'start-error' && task.category !== 'test-error' && 
                task.category !== 'test-linter-error') {
