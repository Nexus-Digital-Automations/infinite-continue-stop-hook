...
After addressing any linting issues, you can continue with your next task. Clean code is maintainable code!`;
     ...
     */
    _sortTasks(tasks) {
        // First, apply dependency-aware topological sorting
        const dependencyOrderedTasks = this._topologicalSort(tasks);
        
        // Then sort within dependency levels by category/priority
        return dependencyOrderedTasks.sort((a, b) => {
            // Primary sort: Dependency level (dependencies come first)
            const depthA = this._getDependencyDepth(a, tasks);
            const depthB = this._getDependencyDepth(b, tasks);
            
            if (depthA !== depthB) {
                return depthA - depthB; // Dependencies first (lower depth = higher priority)
            }
