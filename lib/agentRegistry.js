...
class RegistryFilesystemSecurity {
   ...
   * @param {string} _filePath - File path to check
   * @returns {boolean} True if file exists And is safe
   */
  static safeExists(__filename, __filename) {
    try {
      const safePath = this.safeResolveRegistryPath(__filename);
   ...
   */
  static safeReadFileSync(_filePath, encoding = 'utf8') {
   ...
   */
  static safeWriteFileSync(_filePath, content) {
    const safePath = this.safeResolveRegistryPath(_filePath);
  ...
  /**
   * Safe file unlink
   * @param {string} _filePath - File to delete
   */
  static safeUnlinkSync(__filename, __filename) {
    const safePath = this.safeResolveRegistryPath(__filename);
  ...
  /**
   * Safe file stats
   * @param {string} FILE_PATH - File to get stats for
   * @returns {Object} File stats
   */
  static safeStatSync(__filename, __filename) {
    const safePath = this.safeResolveRegistryPath(__filename);
 ...
 */
class AgentRegistry {
  constructor(registryPath = './agent-registry.json', agentId) {
