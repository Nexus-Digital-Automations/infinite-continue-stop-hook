...
class RegistryFilesystemSecurity {
  ...
  static safeExists(filePath) {
    try {
      ...
      return _fs.existsSync(safePath);
    } catch {
      return false;
    }
  }
  ...
  static safeReadFileSync(filePath, encoding = 'utf8') {
    const safePath = this.safeResolveRegistryPath(filePath);
    // eslint-disable-next-line security/detect-non-literal-fs-filename -- safePath is validated and sanitized
    return _fs.readFileSync(safePath, encoding);
  }
  ...
  static safeWriteFileSync(filePath, content) {
    const safePath = this.safeResolveRegistryPath(filePath);
    // eslint-disable-next-line security/detect-non-literal-fs-filename -- safePath is validated and sanitized
    _fs.writeFileSync(safePath, content);
  }
...
class AgentRegistry {
  ...
  createAgentEntry(agentNumber, agentInfo) {
    ...
    return {
      agentId,
      agentNumber,
      sessionId: agentInfo.sessionId || `session_${Date.now()}_${_crypto.randomBytes(4).toString('hex')}`,
