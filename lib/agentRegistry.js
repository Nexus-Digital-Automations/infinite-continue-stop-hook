...
class AgentRegistry {
  ...
  /**
     ...
     */
  updateAgentActivity(_AGENT_ID) {
    return this.withFileLock(() => {
      const registry = this.readRegistry();

      if (registry.agents[_AGENT_ID]) {
        registry.agents[_AGENT_ID].lastActivity = Date.now();
        registry.agents[_AGENT_ID].totalRequests = (registry.agents[_AGENT_ID].totalRequests || 0) + 1;
        this.writeRegistry(registry);
        return true;
      }
  ...
  getAgent(_AGENT_ID) {
    const registry = this.readRegistry();
    return registry.agents[_AGENT_ID] || null;
  }
