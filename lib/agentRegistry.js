...
class RegistryFilesystemSecurity {
   ...
   */
  static safeExists(_filePath) {
    try {
      const safePath = this.safeResolveRegistryPath(_filePath);

      // eslint-disable-next-line security/detect-non-literal-fs-filename -- Path validated by safeResolveRegistryPath utility
   ...
   */
  static safeUnlinkSync(_filePath) {
    const safePath = this.safeResolveRegistryPath(_filePath);
 ...
 */
class AgentRegistry {
  constructor(registryPath = './agent-registry.json', _agentId) {
  ...
  /**
     ...
     */
  updateAgentActivity(AGENT_ID) {
    return this.withFileLock(() => {
      const registry = this.readRegistry();

      if (registry.agents[AGENT_ID]) {
        registry.agents[AGENT_ID].lastActivity = Date.now();
        registry.agents[AGENT_ID].totalRequests = (registry.agents[AGENT_ID].totalRequests || 0) + 1;
        this.writeRegistry(registry);
        return true;
      }
  ...
  getAgent(AGENT_ID) {
    const registry = this.readRegistry();
    return registry.agents[AGENT_ID] || null;
  }
  ...
  createAgentEntry(agentNumber, agentInfo) {
    const AGENT_ID = `agent_${agentNumber}`;
  ...
  cleanupInactiveAgents(registry) {
    ...
    const inactiveAgents = Object.entries(registry.agents).filter(([_agentId, agent]) =>
      (now - agent.lastActivity) >= this.inactivityTimeout,
    );

    if (inactiveAgents.length > 0) {
      // Mark agents as inactive but keep the slots for reuse
      inactiveAgents.forEach(([_agentId, agent]) => {
  ...
  readRegistry() {
    try {
      ...
      throw new Error(`Failed to read agent registry`);
    }
  }
  ...
  writeRegistry(registry) {
    try {
      ...
      throw new Error(`Failed to write agent registry`);
    }
  }
