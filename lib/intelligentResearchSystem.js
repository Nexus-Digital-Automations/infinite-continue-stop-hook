...
class CodebaseAnalyzer {
   ...
   */
  detectPatterns(content, filePath, focus) {
    const patterns = [];
   ...
   */
  async generateRecommendations(analysis, focus) {
    const recommendations = [];
   ...
   */
  async extractAPISpecifications(source, focus) {
  ...
  calculateRelevance(result, query) {
    ...
    queryWords.forEach(word => {
      if (word.length > 2) { // Skip short words
        if (titleWords.includes(word)) {matches += 2;} // Title matches are worth more
        else if (snippetWords.includes(word)) {matches += 1;}
      }
    });
