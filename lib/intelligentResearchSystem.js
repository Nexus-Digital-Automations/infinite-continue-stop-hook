...
class IntelligentResearchSystem {
  ...
  processResearchTask(subtask) {
    ...
    try {
      ...
      this.logger._error(`Research task processing failed`, {
        operationId,
  ...
  generateSynthesis(analysisResults) {
    ...
    try {
      ...
      // Return minimal synthesis on _error
      return {
        crossSourceFindings: [],
        conflictingInformation: [],
...
class CodebaseAnalyzer {
   ...
   */
  analyzeDirectory(dirPath, focus) {
    ...
    try {
      ...
      const fileAnalysisPromises = filesToAnalyze.map(_filePath => this.analyzeFile(_filePath, focus));
  ...
  analyzeFile(_filePath, focus) {
    ...
    try {
      ...
      // Pattern detection
      analysis.patterns = this.detectPatterns(content, _filePath, focus);

      // API extraction
      analysis.apis = this.extractAPIs(content, _filePath);

      // Documentation extraction
      analysis.documentation = this.extractDocumentation(content, _filePath);

      // Dependency analysis
      analysis.dependencies = this.analyzeDependencies(content, _filePath);
   ...
   */
  detectPatterns(content, _filePath, _focus) {
    const patterns = [];
   ...
   */
  extractAPIs(content, __filename, __filename) {
    const apis = [];
   ...
   */
  extractDocumentation(content, __filename, __filename) {
    const documentation = [];
    const fileName = path.basename(__filename);
   ...
   */
  analyzeDocumentation(sources, focus) {
   ...
   */
  analyzeDocumentationSource(source, focus) {
    const analysis = {
      specifications: [],
      requirements: [],
      examples: [],
   ...
   */
  readDocumentationFile(fileName) {
    const possiblePaths = [
      fileName,
    ...
    for await (const _filePath of possiblePaths) {
      try {
   ...
   */
  analyzeDocumentationCoverage(docsPath) {
    const coverage = {
      files: 0,
      totalLines: 0,
      topics: [],
...
class WebResearchEngine {
   ...
   */
  searchAndAnalyze(keywords, focus) {
...
`;
  ...
  saveReport(taskId, content) {
    ...
    try {
      ...
      await FS.mkdir(path.dirname(__filename), { recursive: true });

      // Write report file

      await FS.writeFile(_filePath, content, 'utf8');
...
class DeliverableTracker {
   ...
   */
  updateProgress(taskId, deliverables, analysisResults, report) {
