...
const workerPromises = [];
  ...
  async createWorker(workerId) {
    try {
      ...
      loggers.app._error(`❌ Failed to create worker ${workerId}: ${_error.message}`);
      loggers.stopHook._error(`❌ Failed to create worker ${workerId}: ${_error.message}`);
    }
}
...
const assignments = [];
  ...
  assignTaskToWorker(worker, task) {
    ...
    taskId: task.id,
      ...
      loggers.app._error(`❌ Failed to assign task ${task.id} to worker ${worker.id}: ${_error.message}`);
      ...
      if (task.retryCount < task.maxRetries) {
        ...
        loggers.stopHook._error(`❌ Task ${task.id} exceeded maximum retries`);
      } else {
        loggers.app._error(`❌ Task ${task.id} exceeded maximum retries`);
