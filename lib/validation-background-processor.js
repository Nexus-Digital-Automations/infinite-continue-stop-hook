...
`;
    ...
    // eslint-disable-next-line security/detect-non-literal-fs-filename -- scriptPath validated by caller for worker script creation
  ...
  assignTaskToWorker(worker, task) {
    try {
      ...
      if (task.retryCount < task.maxRetries) {
        ...
        console.log(`🔄 Task ${task.id} returned to queue for retry (${task.retryCount}/${task.maxRetries})`);
      } else {
