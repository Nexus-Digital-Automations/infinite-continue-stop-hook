...
const _TaskManager = require('./taskManager');
const _AgentManager = require('./agentManager');
 ...
 */
class SystemHealthMonitor extends _EventEmitter {
  constructor(projectRoot = process.cwd()) {
     ...
     */
    this.logger = {
      // eslint-disable-next-line no-console -- SystemHealthMonitor requires console output for diagnostic logging
      info: (message, data) => console.log(`[SystemHealthMonitor] INFO: ${message}`, data ? JSON.stringify(data, null, 2) : ''),
      // eslint-disable-next-line no-console -- SystemHealthMonitor requires console output for warning alerts
      warn: (message, data) => console.warn(`[SystemHealthMonitor] WARN: ${message}`, data ? JSON.stringify(data, null, 2) : ''),
      // eslint-disable-next-line no-console -- SystemHealthMonitor requires console output for error reporting
      error: (message, data) => console.error(`[SystemHealthMonitor] ERROR: ${message}`, data ? JSON.stringify(data, null, 2) : ''),
      // eslint-disable-next-line no-console -- SystemHealthMonitor requires console output for debug information
      debug: (message, data) => console.log(`[SystemHealthMonitor] DEBUG: ${message}`, data ? JSON.stringify(data, null, 2) : ''),
    };
  ...
  _initializeSystemComponents() {
    ...
    try {
      ...
      this.agentManager = new _AgentManager();
      ...
      this.logger.error('Failed to initialize system components', {
        error: error.message,
        stack: error.stack,
        initDuration: performance.now() - initStart,
      });
    ...
    try {
      this.resourceBaseline = {
        memory: {
          ...
          usagePercent: ((_os.totalmem() - _os.freemem()) / _os.totalmem()) * 100,
        ...
        cpu: {
          ...
          model: _os.cpus()[0]?.model || 'Unknown',
          speed: _os.cpus()[0]?.speed || 0,
        ...
        network: {
          ...
          networkInterfaces: Object.keys(_os.networkInterfaces()),
        },
        timestamp: new Date().toISOString(),
        collectionDuration: performance.now() - baselineStart,
      };

      this.logger.info('System resource baseline collected', {
        memoryUsagePercent: this.resourceBaseline.memory.usagePercent,
      ...
      this.logger.error('Failed to collect resource baseline', {
        error: error.message,
        stack: error.stack,
        collectionDuration: performance.now() - baselineStart,
      });
  ...
  async _checkFileSystemHealth() {
    ...
    try {
      ...
      const fileCheckPromises = criticalFiles.map(async (file) => {
        const filePath = _path.join(this.projectRoot, file);
        ...
        try {
          ...
          await _fs.access(filePath, _fsSync.constants.R_OK);
      ...
      const dirCheckPromises = criticalDirectories.map(async (dir) => {
        const dirPath = _path.join(this.projectRoot, dir);

        try {

          // eslint-disable-next-line security/detect-non-literal-fs-filename -- System monitoring path validated through health check
          const stats = await _fs.stat(dirPath);
          return {
            dir,
            status: {
              exists: true,
      ...
      this.logger.info('File system health check completed', {
        healthScore: fileHealthResults.statistics.healthScore,
        successfulChecks,
        totalChecks,
      ...
      this.logger.error('File system health check failed', {
        error: error.message,
        stack: error.stack,
  ...
  async _checkTaskManagerHealth() {
    ...
    try {
      ...
      this.logger.info('TaskManager health check completed', {
        healthScore: taskManagerResults.statistics.healthScore,
      ...
      this.logger.error('TaskManager health check failed', {
        error: error.message,
        stack: error.stack,
  ...
  async _checkAgentSystemHealth() {
    ...
    try {
      ...
      this.logger.info('Agent system health check completed', {
        healthScore: agentResults.statistics.healthScore,
        activeAgents: activeAgents.length,
        totalAgents: totalAgents.length,
      ...
      this.logger.error('Agent system health check failed', {
        error: error.message,
        stack: error.stack,
  ...
  async _testAgentHeartbeatSystem() {
    ...
    try {
      ...
      this.logger.error('Agent heartbeat system test failed', {
        error: error.message,
        stack: error.stack,
        duration: performance.now() - heartbeatTestStart,
      });
  ...
  _checkSystemResourceHealth() {
    ...
    try {
      ...
      this.logger.info('System resource health check completed', {
        healthScore: resourceResults.statistics.healthScore,
      ...
      this.logger.error('System resource health check failed', {
        error: error.message,
        stack: error.stack,
  ...
  async _checkPerformanceMetrics() {
    ...
    try {
      ...
      this.logger.info('Performance metrics check completed', {
        performanceScore: performanceResults.statistics.performanceScore,
      ...
      this.logger.error('Performance metrics check failed', {
        error: error.message,
        stack: error.stack,
  ...
  async _checkDataIntegrityHealth() {
    ...
    try {
      ...
      this.logger.info('Data integrity check completed', {
        integrityScore: integrityResults.statistics.integrityScore,
  ...
  async _validateJsonFileIntegrity(integrityResults) {
    ...
    for (const fileName of jsonFiles) {
      const filePath = _path.join(this.projectRoot, fileName);

      try {


        // eslint-disable-next-line no-await-in-loop -- Sequential file validation required, filePath from trusted source
        // eslint-disable-next-line security/detect-non-literal-fs-filename -- System monitoring path validated through health check
        ...
        integrityResults.fileIntegrity[fileName] = {
          ...
          // eslint-disable-next-line security/detect-non-literal-fs-filename -- System monitoring path validated through health check
          exists: _fsSync.existsSync(filePath),
          validJson: false,
          error: error.message,
        };
  ...
  async _validateBackupFiles(integrityResults) {
    try {
      ...
      // eslint-disable-next-line security/detect-non-literal-fs-filename -- System monitoring path validated through health check
      const backupFiles = await _fs.readdir(this.projectRoot)
      ...
      for (const backupFile of backupFiles) {
        if (backupFile.includes('TODO.json') || backupFile.includes('.json')) {
          try {
            const backupPath = _path.join(this.projectRoot, backupFile);


            // eslint-disable-next-line no-await-in-loop -- Sequential backup validation required, backupPath from trusted source
            // eslint-disable-next-line security/detect-non-literal-fs-filename -- System monitoring path validated through health check
            const backupContent = await _fs.readFile(backupPath, 'utf8');
  ...
  _generateHealthReport(aggregatedResults, overallHealthScore, totalDuration) {
    const report = {
      ...
      healthTrends: {
        ...
        baselineComparison: this.resourceBaseline ? {
          memoryTrend: 'stable', // Would be calculated from historical data
  ...
  async saveHealthReport(healthReport, filename = null) {
    try {
      ...
      const reportPath = _path.join(reportsDir, reportFilename);


      // eslint-disable-next-line security/detect-non-literal-fs-filename -- System monitoring path validated through health check
      await _fs.writeFile(reportPath, JSON.stringify(healthReport, null, 2), 'utf8');
      ...
      this.logger.error('Failed to save health report', {
        error: error.message,
        stack: error.stack,
        timestamp: new Date().toISOString(),
      });
  ...
  startContinuousMonitoring(options = {}) {
    ...
    this.performComprehensiveHealthCheck()
      .then(initialReport => {
        ...
        this.emit('monitoringStarted', {
          initialHealthScore: initialReport.summary.overallHealthScore,
          config,
        });
      })
      .catch(error => {
        this.logger.error('Initial health check failed during monitoring startup', {
          error: error.message,
          timestamp: new Date().toISOString(),
        });
      });
        ...
        if (monitoringState.lastHealthScore !== null) {
          ...
          if (Math.abs(scoreDelta) > 20) {
            this.emit('healthChange', {
              previousScore: monitoringState.lastHealthScore,
              currentScore,
              scoreDelta,
              report: healthReport,
            });
    ...
    return {
      stop: () => {
        ...
        this.logger.info('Continuous health monitoring stopped', {
          duration: Date.now() - monitoringState.startTime,
  ...
  async shutdown() {
    try {
      this.logger.info('SystemHealthMonitor shutting down', {
        timestamp: new Date().toISOString(),
      });
      ...
      this.logger.info('SystemHealthMonitor shutdown completed', {
        timestamp: new Date().toISOString(),
      });

    } catch (error) {
      this.logger.error('Error during SystemHealthMonitor shutdown', {
        error: error.message,
        stack: error.stack,
        timestamp: new Date().toISOString(),
      });
