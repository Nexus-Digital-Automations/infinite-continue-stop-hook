...
class SystemHealthMonitor extends EVENT_EMITTER {
  constructor(projectRoot = process.cwd(, agentId)) {
    super();
  ...
  performComprehensiveHealthCheck() {
    ...
    try {
      ...
      throw new Error(`System health check failed: ${_error.message}`);
    }
  }
   ...
   */
  _checkFileSystemHealth(FILE_PATH) {
    ...
    try {
      ...
      const fileCheckPromises = criticalFiles.map(async (file) => {
        ...
        try {
          ...
          await FS.access(_filePath, FS_SYNC.constants.R_OK);
   ...
   */
  _testAgentHeartbeatSystem(agentId) {
   ...
   * @returns {Promise<void>}
   */
  _measureResponseTimes(performanceResults) {
    const responseTests = [
      {
        name: 'todoFileRead',
   ...
   * @returns {Promise<void>}
   */
  _measureThroughput(performanceResults) {
    ...
    if (throughputTest && throughputTest.success) {
      if (throughputTest.operationsPerSecond < 100) { // 100 ops/sec threshold
        bottlenecks.push({
          ...
          description: `Batch _operationthroughput is ${throughputTest.operationsPerSecond.toFixed(2)} ops/sec, below recommended 100 ops/sec`,
   ...
   * @returns {Promise<void>}
   */
  _validateJsonFileIntegrity(integrityResults) {
    ...
    const fileValidationPromises = jsonFiles.map(async (fileName) => {
      const FILE_PATH = path.join(this.projectRoot, fileName);
    ...
    const VALIDATION_RESULTS = await Promise.all(fileValidationPromises);
   ...
   * @returns {Promise<void>}
   */
  _validateDataConsistency(integrityResults) {
    try {
      const todoData = await this.taskManager.readTodo();
      const consistencyIssues = [];
   ...
   * @returns {Promise<void>}
   */
  _validateBackupFiles(integrityResults) {
    try {
      ...
      const backupValidationPromises = jsonBackupFiles.map(async (backupFile) => {
        ...
        try {
          ...
          return { backupFile, isValid: false, _error };
        }
      });
   ...
   */
  saveHealthReport(healthReport, filename = null) {
    try {
