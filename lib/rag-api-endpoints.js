...
class RAGAPIEndpoints {
  ...
  async searchLessons(req, res) {
    try {
      ...
      const _result = await this.ragDB.searchLessons(searchQuery, parseInt(limit), parseFloat(threshold));
      res.status(200).json(result);
  ...
  async searchErrors(req, res) {
    try {
      ...
      const _result = await this.ragDB.searchErrors(searchQuery, parseInt(limit), parseFloat(threshold));
      res.status(200).json(result);
  ...
  async getStats(req, res) {
    try {
      ...
      const _result = await this.ragDB.getStats();
      res.status(200).json(result);
  ...
  async autoStoreLesson(taskData, completionData) {
    try {
      ...
      if (lessonData) {
        const _result = await this.ragDB.storeLesson(lessonData);
        console.log('[RAG-API] Auto-stored lesson:', result.message);
        return result;
      }

      return { success: true, message: 'No lesson extracted from task' };
    } catch {
      console.error('[RAG-API] Failed to auto-store lesson:', error);
  ...
  async autoStoreError(taskData, errorData) {
    try {
      ...
      const errorRecord = this.extractErrorFromTask(taskData, errorData);
        ...
        const _result = await this.ragDB.storeError(errorRecord);
        console.log('[RAG-API] Auto-stored error:', result.message);
        return result;
      }
      ...
      console.error('[RAG-API] Failed to auto-store error:', error);
      return { success: false, error: error.message };
    }
  }
  ...
  async getPreTaskLessons(taskData) {
    try {
      ...
      console.error('[RAG-API] Failed to get pre-task lessons:', error);
      return { success: false, error: error.message };
    }
  }
