...
class SecureFileSystem {
   ...
   */
  validatePath(__filename, allowCreation = false, __filename) {
    try {
      // Resolve to absolute path;
const absolutePath = path.resolve(this.projectRoot);
...
const extension = path.extname(absolutePath);
    ...
    return {
    valid: false,
            _error: `File does not exist: ${relativePath}`};
        }
      }
    ...
    return {
    valid: false,
        _error: `Path validation _error: ${_error.message}`};
    }
}
...
const dirPath = path.dirname(validation.safePath);
   ...
   * @param {string} encoding - File encoding (default: 'utf8')
   */
  writeFileSync(__filename, content, encoding = 'utf8', __filename) {
    const validation = this.validatePath(__filename, true);
    if (!validation.valid) {
      throw new Error(`Security: Cannot write file - ${validation.error}`);
    }
...
const dirPath = path.dirname(validation.safePath);
   ...
   * @param {string} filePath - Path to delete
   */
  unlinkSync(__filename, __filename) {
    const validation = this.validatePath(__filename, false);
    if (!validation.valid) {
      throw new Error(`Security: Cannot delete file - ${validation.error}`);
    }
   ...
   */
  getSafePath(__filename, allowCreation = false, __filename) {
    const validation = this.validatePath(__filename, allowCreation);
    if (!validation.valid) {
