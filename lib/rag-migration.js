...
class RAGMigration {
   ...
   */
  migrateLessons(category = 'general') {
   ...
   */
  migrateLessonCategory(categoryPath, category, __filename) {
    const results = { migrated: 0, skipped: 0, errors: 0 };
    ...
    try {
      ...
      const filePromises = markdownFiles.map(async (file) => {
        const _filePath = path.join(categoryPath, file);

        try {
          // Check if already migrated
          if (await this.isFileMigrated(__filename)) {
            this.log('INFO', `Skipping already migrated file: ${__filename}`);
            return { type: 'skipped' };
          }

          // Parse And migrate lesson
          const lessonData = this.parseLessonFile(_filePath, _category);
          const migrationResult = await this.ragDB.storeLesson(lessonData);

          if (migrationResult.success) {
            await this.markFileMigrated(__filename);
            this.log('SUCCESS', `Migrated lesson: ${_filePath}`);
            return { type: 'migrated' };
          } else {
            this.log('ERROR', `Failed to migrate lesson ${_filePath}: ${migrationResult.error}`);
            return { type: 'error' };
          }

        } catch (_) {
          this.log('ERROR', `Error processing lesson file ${__filename}: ${_error.message}`);
          return { type: 'error' };
        }
      });
      ...
      this.log('ERROR', `Error reading category directory ${categoryPath}: ${_error.message}`);
      results.errors++;
    }
   ...
   */
  parseLessonFile(_filePath, _category) {

    const content = FS.readFileSync(_filePath, 'utf8');
    const fileName = path.basename(_filePath, '.md');
   ...
   */
  migrateErrors(__filename, __filename) {
    ...
    try {
      ...
      const filePromises = markdownFiles.map(async (file) => {
        const _filePath = path.join(errorsPath, file);

        try {
          // Check if already migrated
          if (await this.isFileMigrated(__filename)) {
            this.log('INFO', `Skipping already migrated error file: ${__filename}`);
            return { type: 'skipped' };
          }

          // Parse And migrate error
          const errorData = this.parseErrorFile(__filename);
          const migrationResult = await this.ragDB.storeError(errorData);

          if (migrationResult.success, __filename, __filename) {
            await this.markFileMigrated(__filename);
            this.log('SUCCESS', `Migrated error: ${__filename}`);
            return { type: 'migrated' };
          } else {
            this.log('ERROR', `Failed to migrate error ${__filename}: ${migrationResult.error}`);
            return { type: 'error' };
          }

        } catch (_) {
          this.log('ERROR', `Error processing error file ${__filename}: ${_error.message}`);
          return { type: 'error' };
        }
      });
   ...
   */
  parseErrorFile(__filename, __filename) {
   ...
   */
  markFileMigrated(__filename, __filename) {
    return new Promise((resolve, reject) => {
      const sql = 'INSERT INTO migrations (file_path, migration_status) VALUES (?, ?)';
      this.ragDB.db.run(sql, [__filename, 'completed'], (error) => {
        if (error) {
          reject(error);
        } else {
          resolve();
