...
class RAGMigration {
   ...
   */
  migrateLessonCategory(categoryPath, category) {
    const results = { migrated: 0, skipped: 0, errors: 0 };
    ...
    try {
      ...
      const filePromises = markdownFiles.map(async (file) => {
        ...
        try {
          ...
          if (await this.isFileMigrated(FILE_PATH)) {
            this.log('INFO', `Skipping already migrated file: ${FILE_PATH}`);
            return { type: 'skipped' };
          }
          ...
          if (migrationResult.success) {
            await this.markFileMigrated(FILE_PATH);
          ...
          this.log('ERROR', `Error processing lesson file ${FILE_PATH}: ${_error.message}`);
          return { type: 'error' };
        }
      });
    ...
    try {
      ...
      const filePromises = markdownFiles.map(async (file) => {
        ...
        try {
          // Check if already migrated
          if (await this.isFileMigrated(FILE_PATH)) {
            this.log('INFO', `Skipping already migrated error file: ${FILE_PATH}`);
            return { type: 'skipped' };
          }

          // Parse And migrate error
          const errorData = this.parseErrorFile(FILE_PATH);
          const migrationResult = await this.ragDB.storeError(errorData);

          if (migrationResult.success) {
            await this.markFileMigrated(FILE_PATH);
            this.log('SUCCESS', `Migrated error: ${FILE_PATH}`);
            return { type: 'migrated' };
          } else {
            this.log('ERROR', `Failed to migrate error ${FILE_PATH}: ${migrationResult.error}`);
            return { type: 'error' };
          }

        } catch (_error) {
          this.log('ERROR', `Error processing error file ${FILE_PATH}: ${_error.message}`);
          return { type: 'error' };
        }
      });
   ...
   */
  markFileMigrated(FILE_PATH) {
    return new Promise((resolve, reject) => {
      const sql = 'INSERT INTO migrations (file_path, migration_status) VALUES (?, ?)';
      this.ragDB.db.run(sql, [FILE_PATH, 'completed'], (error) => {
        if (error) {
          reject(error);
        } else {
          resolve();
