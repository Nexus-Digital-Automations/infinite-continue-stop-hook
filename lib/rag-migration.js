...
class RAGMigration {
    ...
    try {
      ...
      const filePromises = markdownFiles.map(async (file) => {
        ...
        try {
          ...
          if (await this.isFileMigrated(_filePath)) {
          ...
          if (migrationResult.success) {
            await this.markFileMigrated(_filePath);
    ...
    try {
      ...
      const filePromises = markdownFiles.map(async (file) => {
        ...
        try {
          ...
          const errorData = this.parseErrorFile(_filePath);
   ...
   */
  async isFileMigrated(_filePath) {
   ...
   */
  async markFileMigrated(_filePath) {
    return new Promise((resolve, reject) => {
