...
class CustomValidationRulesManager {
  ...
  async loadCustomRules() {
    try {
      ...
      try {
        ...
        loggers.stopHook.log('No custom validation rules configuration found. Using default rules only.');
  ...
  async executeRule(ruleId) {
    ...
    try {
      ...
      const duration = Date.now() - startTime;
      this._recordExecution(ruleId, { success: false, error: _.message }, duration);
    ...
    try {
      ...
      if (rule.allow_failure) {
        return {
          ...
          details: `Command failed but failure is allowed: ${_.message}`,
  ...
  async _executeConditionalRule(rule) {
    ...
    loggers.stopHook.log(`  ❓ Evaluating condition: ${JSON.stringify(condition)} = ${conditionMet}`);
    ...
    for (const subRule of rules) {
      ...
      const _RESULT = await this._executeSubRule(subRule);
      results.push(result);
  ...
  async _executeCompositeRule(rule) {
    ...
    const results = [];
    loggers.stopHook.log(`  📋 Executing composite rule with ${rules.length} sub-rules (${operator})`);
  ...
  async _evaluateCondition(condition) {
    try {
      ...
      loggers.stopHook.warn(`Condition evaluation failed: ${_.message}`);
