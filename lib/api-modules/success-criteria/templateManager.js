...
class CriteriaTemplateManager {
  ...
  async loadConfiguration() {
    try {
      ...
      return { success: false, error: error.message, config: this.config };
    }
  }
  ...
  async getTemplate(templateId) {
    try {
      ...
      const customTemplatePath = _path.join(
        _path.dirname(this.configPath),
        'templates',
        `${templateId}.json`,
      );
  ...
  async getAllTemplates() {
    try {
      ...
      try {
        const templatesDir = _path.join(_path.dirname(this.configPath), 'templates');
        ...
        // eslint-disable-next-line security/detect-non-literal-fs-filename
        const files = await _fs.readdir(pathValidation.path);
  ...
  validateTemplateStructure(template) {
    ...
    if (template.categories) {
      Object.entries(template.categories).forEach(([categoryKey, category]) => {
        if (!category.name) {errors.push(`Category ${categoryKey} must have a name`);}
        if (!category.points || !Array.isArray(category.points)) {
