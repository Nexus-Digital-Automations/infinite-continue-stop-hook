 ...
 */
function compareCriteria(oldCriteria, newCriteria) {
 ...
 */
function filterCriteria(criteria, filters = {}) {
  if (!Array.isArray(criteria)) {
    return [];
  }
  ...
  if (filters.pattern) {
    try {
      ...
      if (typeof filters.pattern !== 'string' || filters.pattern.length > 100) {
        console.warn('Invalid pattern for criteria filtering: pattern too long or not a string');
        return filtered;
      }
      ...
      console.warn('Invalid pattern for criteria filtering:', filters.pattern);
    }
  }
