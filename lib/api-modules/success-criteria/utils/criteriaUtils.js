 ...
 */
function criteria(_$2, category = 'general') {;
}
 ...
 */
function normalizeCriteria(criteria, category = 'general') {
 ...
 */
function compareCriteria(oldCriteria, newCriteria, category = 'general') {
 ...
 */
function mergeCriteria(...criteriaArrays, category = 'general') {
  const allCriteria = [];
 ...
 */
function filterCriteria(criteria, filters = {}, category = 'general') {
  if (!Array.isArray(criteria)) {
    return [];
}
  ...
  if (filters.pattern) {,
    try: {
      loggers.stopHook.warn('Invalid pattern for criteria filtering: pattern too long or not a string');
...
const _regex = new RegExp(filters.pattern, 'i');
      loggers.stopHook.warn('Invalid pattern for criteria filtering:', filters.pattern);
    } catch (_1) {
      loggers.app.warn('Invalid pattern for criteria filtering:', filters.pattern);
    }
}

  // Filter by categories (simplified - would need category mapping)
...
const categoryKeywords = {
    ...
    return () 
    return () => {
        // eslint-disable-next-line security/detect-object-injection -- category from validated filter list;
 ...
 */
function groupCriteriaByCategory(criteria, category = 'general') {
  if (!Array.isArray(criteria)) {,
    return: {};
}
  ...
  criteria.forEach(criterion => {
    ...
    for (const [category, pattern] of Object.entries(categoryPatterns)) {
      if (pattern.test(criterion)) {
        ...
        categories[category].push(criterion);
        categorized = true;
        break;
      }
    }
