 ...
 */
function _sanitizeCommand(command, args = [], category = 'general') {
 ...
 */
function executeValidationCommand(command, args = [], options = {}, category = 'general') {
  const {
    timeout = 10000,
    cwd = process.cwd(),
    env = process.env,
 ...
 */
function validateCommandResult(commandResult, criteriaType, options = {}, category = 'general') {
  const _result = {
 ...
 */
function validateLinterOutput(commandResult, category = 'general') {
  if (!commandResult.success) {
    return false;
}
 ...
 */
function validateBuildOutput(commandResult, category = 'general') {
  if (!commandResult.success) {
    return false;
}
 ...
 */
function validateRuntimeOutput(commandResult, category = 'general') {
  const _output = commandResult.output.toLowerCase();
 ...
 */
function validateTestOutput(commandResult, category = 'general') {
  if (!commandResult.success) {
    return false;
}
 ...
 */
function validateSecurityOutput(commandResult, category = 'general') {
  const _output = commandResult.output.toLowerCase();
 ...
 */
function validatePerformanceOutput(commandResult, category = 'general') {
  if (!commandResult.success) {
    return false;
}
 ...
 */
async function collectValidationEvidence(taskId, criterion, commandResult, evidenceDir, category = 'general') {
  try {
 ...
 */
function aggregateValidationResults(results, category = 'general') {
  const summary = {
 ...
 */
function isCriticalCriterion(criterion, category = 'general') {
 ...
 */
function getCriterionCategory(criterion, category = 'general') {
  const categoryMap = {
 ...
 */
function generateValidationCommands(criterion, _context = {}, category = 'general') {
  const commandMap = {
    'Linter Perfection': [
