...
class ReportGenerator {
  ...
  constructor(dependencies) {
    ...
    this.initializeDirectories().catch(err => {
      this.logger.warn('Could not initialize report directories', { error: err.message });
    });
  }
  ...
  async initializeDirectories() {
    try {
      ...
      await _fs.mkdir(evidenceValidation.path, { recursive: true });
      ...
      // eslint-disable-next-line security/detect-non-literal-fs-filename
      await _fs.mkdir(reportValidation.path, { recursive: true });

      const tasksDir = _path.join(this.reportPath, 'tasks');
      ...
      // eslint-disable-next-line security/detect-non-literal-fs-filename
      await _fs.mkdir(tasksValidation.path, { recursive: true });

      const dashboardDir = _path.join(this.reportPath, 'dashboard');
      ...
      // eslint-disable-next-line security/detect-non-literal-fs-filename
      await _fs.mkdir(dashboardValidation.path, { recursive: true });

      const exportsDir = _path.join(this.reportPath, 'exports');
      ...
      // eslint-disable-next-line security/detect-non-literal-fs-filename
      await _fs.mkdir(exportsValidation.path, { recursive: true });
    } catch (error) {
  ...
  async generateValidationReport(taskId, validationResults, options = {}) {
    try {
      ...
      // eslint-disable-next-line security/detect-non-literal-fs-filename
      await _fs.writeFile(pathValidation.path, JSON.stringify(reportMetadata, null, 2));
  ...
  async generateReportFormat(reportData, format, reportId) {
    ...
    switch (format) {
      case 'json':
        // eslint-disable-next-line security/detect-non-literal-fs-filename
        await _fs.writeFile(pathValidation.path, JSON.stringify(reportData, null, 2));
        break;

      case 'markdown': {
        const markdownContent = this.generateMarkdownReport(reportData);
        // eslint-disable-next-line security/detect-non-literal-fs-filename
        await _fs.writeFile(pathValidation.path, markdownContent);
        break;
      }
      ...
      case 'html': {
        ...
        // eslint-disable-next-line security/detect-non-literal-fs-filename
        await _fs.writeFile(pathValidation.path, htmlContent);
        break;
      }
...
<body>
  ...
  async scheduleEvidenceCollection(taskId, criteria) {
    try {
      ...
      // eslint-disable-next-line security/detect-non-literal-fs-filename
      await _fs.writeFile(pathValidation.path, JSON.stringify(collectionPlan, null, 2));
