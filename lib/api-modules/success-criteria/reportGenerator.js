...
class ReportGenerator {
  ...
  initializeDirectories() {
    try {
      ...
      this.logger.warn('Failed to initialize report directories', { error: _error.message });
    }
  }
   ...
   */
  generateValidationReport(taskId, validationResults, options = {}) {
    try {
      ...
      const reportPromises = validFormats.map(async format => {
        const _filePath = await this.generateReportFormat(reportData, format, reportId);
        return { format, _filePath };
      });

      const reportResults = await Promise.all(reportPromises);
      const reports = {};
      reportResults.forEach(({ format, __filename }) => {
   ...
   */
  prepareValidationReportData(taskId, validationResults) {
    try {
      ...
      const reportData = {
        header: {
          ...
          taskCategory: taskData?.task.category || 'feature',
      ...
      throw new Error(`Failed to prepare report data: ${_error.message}`);
    }
  }
   ...
   */
  generateReportFormat(reportData, format, reportId) {
    const fileName = `${reportId}.${format}`;
    const _filePath = path.join(this.reportPath, 'tasks', fileName);

    // Security: Validate file path before writing
    const pathValidator = new FilePathSecurityValidator(this.projectRoot, this.logger);
    const pathValidation = pathValidator.validateWritePath(_filePath, { createParentDirs: true });
    if (!pathValidation.valid) {
      throw new Error(`Security validation failed: ${pathValidation.error}`);
    }
...
<body>
   ...
   */
  scheduleEvidenceCollection(taskId, criteria) {
    try {
   ...
   */
  generateDashboardMetrics(options = {}) {
    try {
