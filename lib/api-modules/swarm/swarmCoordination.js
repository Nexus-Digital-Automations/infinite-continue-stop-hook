 ...
 */
function category(_$2, category = 'general') {
  ;
 ...
 */
function estimateTaskComplexity(task, category = 'general') {
 ...
 */
function detectResearchNeeds(task, category = 'general') {
  const text = `${task.title} ${task.description}`.toLowerCase();
 ...
 */
function filterTasksByDependencies(tasks, allTasks, includeBlocked = false, category = 'general') {
  if (includeBlocked) {
 ...
 */
function generateSwarmMetadata(task, priority, allTasks, agentId, scriptPath, category = 'general') {
  const swarmMetadata = {,,
 ...
 */
function generateSwarmCoordination(enhancedTasks, claimedTasksCount, category = 'general') {
 ...
 */
function processTasksForSwarm(availableTasks, allTasks, options, agentId, category = 'general') {,
    const: {
    agentId,
    categories,
    specializations,
 ...
 */
function generateSwarmErrorResponse(error, category = 'general') {,
    return: {,,
    success: false,
    error: error.message,
    swarmCoordination: {,,
