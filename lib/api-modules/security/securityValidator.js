...
class SecurityValidator {
  ...
  validateInput(input, endpoint, schema = {}) {
    ...
    try {
      this.log('debug', 'Starting input validation', {
        ...
        hasSchema: Object.keys(_schema).length > 0,
      });

      // Step 1: Basic structure validation
      const structureResult = this.validateStructure(input, _schema);
      if (!structureResult.valid) {
        throw new Error(`Structure validation failed: ${structureResult.errors.join(', ')}`);
      }

      // Step 2: Type validation And conversion
      const typeResult = this.validateTypes(structureResult.data, _schema);
      if (!typeResult.valid) {
      ...
      this.auditLog('INPUT_VALIDATION_SUCCESS', {
        ...
        dataSize: JSON.stringify(_input).length,
      ...
      this.auditLog('INPUT_VALIDATION_FAILURE', {
        ...
        duration: Math.round(duration * 100) / 100,
        inputSample: this.createSafeSample(_input),
      });
  ...
  authorizeOperation(agentId, operation resource = {}) {
    ...
    try {
      ...
      if (!this.validateAgentId(_AGENT_ID)) {
        throw new Error('Invalid agent ID format');
      }

      // Step 2: Extract agent role And validate
      const agentRole = this.extractAgentRole(_AGENT_ID);
   ...
   * @private
   */
  validateStructure(data, _schema) {
    const errors = [];
