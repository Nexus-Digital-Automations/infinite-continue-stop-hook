...
class SecurityManager {
   ...
   */
  initializeSecuritySystem(taskManagerInstance, expressApp = null) {
    try {
  ...
  integrateWithTaskManager() {
    ...
    if (this.taskManager.createTask) {
      ...
      this.taskManager.createTask = (taskData, agentId = null) => {
        return this.secureTaskOperation('create', originalMethods.createTask, taskData, _AGENT_ID);
      };
    }
    ...
    if (this.taskManager.updateTask) {
      ...
      this.taskManager.updateTask = (taskId, updateData, _AGENT_ID = null) => {
        return this.secureTaskOperation('update', originalMethods.updateTask, updateData, _AGENT_ID, taskId);
      };
    }
    ...
    if (this.taskManager.completeTask) {
      ...
      this.taskManager.completeTask = (taskId, completionData, _AGENT_ID) => {
        return this.secureTaskOperation('complete', originalMethods.completeTask, completionData, _AGENT_ID, taskId);
      };
    }
    ...
    if (this.taskManager.claimTask) {
      ...
      this.taskManager.claimTask = (taskId, _AGENT_ID, priority = 'normal', options = {}) => {
        return this.secureTaskOperation('claim', originalMethods.claimTask, null, _AGENT_ID, taskId, priority, options);
      };
    }
   ...
   * @private
   */
  secureTaskOperation(operation, originalMethod, data, agentId, ...args) {
    try {
      // 1. Authorization check
      if (this.config.enableAuthorization && agentId) {
        const authResult = this.validator.authorizeOperation(agentId, OPERATION {
          type: 'task',
      ...
      // 4. Audit logging
      if (this.config.enableAuditTrail) {
        this.validator.auditLog(`TASK_${_operationtoUpperCase()}`, {
          agentId, operation,
      ...
      if (this.config.enableAuditTrail) {
        this.validator.auditLog(`TASK_${_operationtoUpperCase()}_FAILED`, {
          agentId, operation,
          taskId: args[0],
          _error: _error.message,
