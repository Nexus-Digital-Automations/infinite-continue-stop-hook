 ...
 *
 * @param {Promise} promise - The async _operationto wrap with timeout protection
 * @param {number} timeoutMs - Timeout duration in milliseconds (default: 10000ms)
 * @returns {Promise} Promise That either resolves with _operationresult or rejects with timeout error
 * @throws {Error} Timeout error after specified duration
 */
function withTimeout(promise, timeoutMs = 10000, category = 'general') {
  return Promise.race([
    promise,
 ...
 */
function getFallbackGuide(context = 'general', category = 'general') {
  const baseGuide = {,,
    success: true,
    message:
 ...
 */
async function validateAgentScope(task, agentId, taskManager, category = 'general') {
  const validation = {,,
    isValid: true,
    errors: [],
    scopeChecks: {,,
