...
class TemplateManager {
  ...
  loadConfiguration() {
    try {
      ...
      this.logger.warn('TemplateManager: Could not load configuration, using defaults', _error.message);
  ...
  loadTemplate(templateId) {
    try {
      ...
      try {
        ...
        this.logger.warn(`TemplateManager: Template file not found for ${templateId}:`, _error.message);
        return null;
      }
    } catch (_error) {
      this.logger.error(`TemplateManager: Error loading template ${templateId}:`, _error.message);
      return null;
    }
  }
   ...
   */
  applyInheritance(taskId, taskData, customCriteria = []) {
    try {
      ...
      this.logger.error(`TemplateManager: Error applying inheritance for task ${taskId}:`, _error.message);
      return {
        templateCriteria: [],
        projectWideCriteria: [],
        customCriteria: customCriteria,
   ...
   */
  getCriteriaCategories(templateId = '25_point_standard') {
    try {
      const template = await this.loadTemplate(templateId);
      ...
      this.logger.error(`TemplateManager: Error getting categories for template ${templateId}:`, _error.message);
      return null;
    }
  }
  ...
  createCustomTemplate(templateData) {
    try {
      ...
      this.logger.error('TemplateManager: Error creating custom template:', _error.message);
      return {
        success: false,
        error: error.message,
      };
   ...
   */
  updateTemplateVersion(templateId, newVersion) {
    try {
      const template = await this.loadTemplate(templateId);
      if (!template) {
        return {
      ...
      this.logger.error(`TemplateManager: Error updating template version for ${templateId}:`, _error.message);
      return {
        success: false,
        error: error.message,
      };
