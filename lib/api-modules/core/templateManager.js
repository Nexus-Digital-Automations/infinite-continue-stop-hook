...
class TemplateManager {
  ...
  async loadConfiguration() {
    try {
      ...
      const configData = await _fs.readFile(pathValidation.path, 'utf8');
  ...
  async loadTemplate(templateId) {
    try {
      ...
      try {
        ...
        // eslint-disable-next-line security/detect-non-literal-fs-filename
        const templateData = await _fs.readFile(pathValidation.path, 'utf8');
  ...
  async createCustomTemplate(templateData) {
    try {
      ...
      // eslint-disable-next-line security/detect-non-literal-fs-filename
      await _fs.mkdir(dirValidation.path, { recursive: true });
      ...
      // eslint-disable-next-line security/detect-non-literal-fs-filename
      await _fs.writeFile(fileValidation.path, JSON.stringify(templateData, null, 2), 'utf8');
  ...
  async updateTemplateVersion(templateId, newVersion) {
    try {
      ...
      if (templateId !== '25_point_standard') {
        ...
        // eslint-disable-next-line security/detect-non-literal-fs-filename
        await _fs.writeFile(pathValidation.path, JSON.stringify(template, null, 2), 'utf8');
      }
