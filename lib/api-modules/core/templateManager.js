...
// Security helper for validating file paths
class SecurityHelper {
  static validateFilePath(filePath, baseDir = process.cwd()) {
    if (typeof filePath !== 'string' || !filePath.trim()) {
      throw new Error('Security: File path must be a non-empty string');
    }

    const resolvedPath = _path.resolve(filePath);
    const resolvedBase = _path.resolve(baseDir);

    // Prevent directory traversal
    if (!resolvedPath.startsWith(resolvedBase)) {
      throw new Error('Security: File path must be within project directory');
    }
...
class TemplateManager {
  ...
  async loadConfiguration() {
    try {
      const configPath = _path.join(this.projectRoot, 'development', 'essentials', 'success-criteria-config.json');
      // Security: Validate config path before reading
      const validatedConfigPath = SecurityHelper.validateFilePath(configPath, this.projectRoot);
      const configData = await _fs.readFile(validatedConfigPath, 'utf8');
  ...
  async loadTemplate(templateId) {
    try {
      ...
      const templatePath = _path.join(this.projectRoot, 'development', 'templates', `${templateId}.json`);

      try {
        // Security: Validate template path before reading
        const validatedTemplatePath = SecurityHelper.validateFilePath(templatePath, this.projectRoot);
        const templateData = await _fs.readFile(validatedTemplatePath, 'utf8');
  ...
  async createCustomTemplate(templateData) {
    try {
      ...
      const templatesDir = _path.join(this.projectRoot, 'development', 'templates');
      // Security: Validate templates directory path
      const validatedTemplatesDir = SecurityHelper.validateFilePath(templatesDir, this.projectRoot);
      await _fs.mkdir(validatedTemplatesDir, { recursive: true });

      // Save template to file
      const templatePath = _path.join(templatesDir, `${templateData.id}.json`);
      // Security: Validate template path before writing
      const validatedTemplatePath = SecurityHelper.validateFilePath(templatePath, this.projectRoot);
      await _fs.writeFile(validatedTemplatePath, JSON.stringify(templateData, null, 2), 'utf8');
  ...
  async updateTemplateVersion(templateId, newVersion) {
    try {
      ...
      if (templateId !== '25_point_standard') {
        ...
        // Security: Validate template path before writing
        const validatedTemplatePath = SecurityHelper.validateFilePath(templatePath, this.projectRoot);
        await _fs.writeFile(validatedTemplatePath, JSON.stringify(template, null, 2), 'utf8');
      }
