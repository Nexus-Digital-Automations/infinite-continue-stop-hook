...
class SUCCESS_CRITERIA_MANAGER {
   ...
   */
  addCriteria(taskId, criteria, options = {}) {
    let guide = null;
    try {
   ...
   */
  deleteCriterion(taskId, criterionText) {
    let guide = null;
    try {
      guide = await this.getGuideForError('success-criteria-operations');
    } catch (_error) {
   ...
   */
  _defaultTaskExistsValidator(taskId) {
    try {
        ...
        (async () => {
          ...
          const report = {
            ...
            taskInfo: {
              ...
              task.category: task.task.category || 'unknown',
   ...
   */
  getCategorizedCriteria(templateId = '25_point_standard') {
    let guide = null;
    try {
      guide = await this.getGuideForError('success-criteria-operations');
    } catch (_error) {
   ...
   */
  applyInheritanceRules(taskId, customCriteria = []) {
    let guide = null;
    try {
      guide = await this.getGuideForError('success-criteria-operations');
    } catch (_error) {
