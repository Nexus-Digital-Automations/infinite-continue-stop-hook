...
class SuccessCriteriaManager {
  /**
   * Initialize SuccessCriteriaManager with enhanced template system support
   ...
   * @returns {Promise<Object>} Response with updated criteria or error
   */
  async updateCriteria(taskId, criteria) {
        ...
        (async () => {
          ...
          Object.keys(this.defaultCriteriaTemplates).forEach(templateName => {
            templates[templateName] = {
              name: templateName,
              criteria: this.defaultCriteriaTemplates[templateName],
              count: this.defaultCriteriaTemplates[templateName].length,
    ...
    try {
      const result = await this.withTimeout(
        (async () => {
          ...
          if (!this.defaultCriteriaTemplates[templateName]) {
            return {
              success: false,
          ...
          return this.addCriteria(taskId, this.defaultCriteriaTemplates[templateName], {
            replace,
            template: templateName,
          });
        })(),
...
module.exports = SuccessCriteriaManager;
