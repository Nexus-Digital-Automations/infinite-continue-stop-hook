      ...
      const result = await this.withTimeout(
        (async () => {
          ...
          if (taskData.category && taskData.category !== 'error') {
            throw new Error(
              `createErrorTask can only be used for error task.category tasks. ` +
              `Received task.category: "${taskData.task.category}". ` +
              `Use createTask() for non-error categories.`,
            );
          }

          // Ensure task has error task.category for proper prioritization
          const errorTaskData = {
            ...taskData,
            task.category: 'error', // Force error task.category for absolute priority
