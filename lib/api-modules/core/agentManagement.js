...
class AgentManagement {
  ...
  getCurrentTask(_AGENT_ID) {
    ...
    try {
      const result = await this.withTimeout(
        (async () => {
          const currentTask = await this.taskManager.getCurrentTaskForAgent(_AGENT_ID);
          ...
          return {
            ...
            message: currentTask
              ? `Agent ${_AGENT_ID} currently assigned to task: ${currentTask.id}`
              : `Agent ${_AGENT_ID} has no current task assignment`,
          };
        })(),
      );
  ...
  getAgentStatus(_AGENT_ID) {
    ...
    try {
      const result = await this.withTimeout(
        (async () => {
          const agent = await this.agentManager.getAgent(_AGENT_ID);

          if (!agent) {
            throw new Error(`Agent ${_AGENT_ID} not found`);
          }

          // Get current tasks for this agent
          const currentTasks = await this.taskManager.getTasksForAgent(_AGENT_ID);
    ...
    try {
      const result = await this.withTimeout(
        (async () => {
          ...
          if (!agent) {
            throw new Error(
              `Agent ${_AGENT_ID} not found. Use 'init' to create a new agent or check available agents with 'list-agents'.`,
            );
          }
        ...
        (async () => {
          ...
          if (_AGENT_ID) {
            ...
            if (agent) {
              ...
              throw new Error(`Agent ${_AGENT_ID} not found. Use 'init' to create a new agent.`);
            }
          } else {
            // No agent ID provided - check for existing agents
            const activeAgents = await this.agentManager.listActiveAgents();
            if (activeAgents.length === 1) {
              targetAgentId = activeAgents[0]._AGENT_ID;
   ...
   */
  performReinitializeWithScenario(
    agentId,
    config,
    scenario,
    guide,
   ...
   */
  validateAgentScope(task, _AGENT_ID) {
    ...
    const agent = await this.agentManager.getAgent(_AGENT_ID);
    if (!agent) {
      throw new Error(`Agent ${_AGENT_ID} not found for scope validation`);
    }
