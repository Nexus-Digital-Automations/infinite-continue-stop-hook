...
class AgentManagement {
   ...
   */
  constructor(dependencies, agentId) {
  ...
  async getCurrentTask(AGENT_ID) {
    ...
    try {
      const result = await this.withTimeout(
        (async () => {
          const currentTask = await this.taskManager.getCurrentTaskForAgent(AGENT_ID);
          return {
            success: true,
            AGENT_ID: AGENT_ID,
            ...
            message: currentTask
              ? `Agent ${AGENT_ID} currently assigned to task: ${currentTask.id}`
              : `Agent ${AGENT_ID} has no current task assignment`,
          };
        })(),
      );
  ...
  async getAgentStatus(AGENT_ID) {
    ...
    try {
      const result = await this.withTimeout(
        (async () => {
          const agent = await this.agentManager.getAgent(AGENT_ID);
          ...
          const currentTasks = await this.taskManager.getTasksForAgent(AGENT_ID);
    ...
    try {
      const result = await this.withTimeout(
        (async () => {
          ...
          if (!agent) {
            throw new Error(
              `Agent ${AGENT_ID} not found. Use 'init' to create a new agent or check available agents with 'list-agents'.`,
            );
          }
   ...
   */
  async validateAgentScope(task, AGENT_ID) {
    ...
    const agent = await this.agentManager.getAgent(AGENT_ID);
    if (!agent) {
      throw new Error(`Agent ${AGENT_ID} not found For scope validation`);
    }
