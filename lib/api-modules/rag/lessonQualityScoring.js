...
class LessonQualityScoring {
    ...
    return new Promise((resolve, reject) => {
      ...
      `, [], async (error, lessons) => {
        ...
        const migrationPromises = lessons.map(async (lesson) => {
          try {
            // Calculate and persist quality score for existing lesson
            const initialScore = this.calculateInitialQualityScore(lesson);
            await this.createInitialQualityRecord(lesson.id, initialScore);
            loggers.app.info(`[QUALITY] Migrated quality score for lesson ${lesson.id}`);
          } catch (error) {
  ...
  insertUsageRecord(lessonId, usageData) {
    return new Promise((resolve, reject) => {
      ...
      this.ragDB.db.run(sql, [
        ...
        usageData.usageDuration], function (error) {
        if (error) {
          reject(error);
        } else {
          resolve(this.lastID);
  ...
  async recordUserFeedback(lessonId, feedbackData) {
    try {
      const {
        ...
        difficultyRating = null } = feedbackData;
  ...
  insertFeedbackRecord(lessonId, feedbackData) {
    return new Promise((resolve, reject) => {
      ...
      this.ragDB.db.run(sql, [
        ...
        feedbackData.difficultyRating], function (error) {
        if (error) {
          reject(error);
        } else {
          resolve(this.lastID);
  ...
  insertOutcomeRecord(lessonId, outcomeData) {
    return new Promise((resolve, reject) => {
      ...
      this.ragDB.db.run(sql, [
        ...
        outcomeData.measuredImpact], function (error) {
        if (error) {
          reject(error);
        } else {
          resolve(this.lastID);
  ...
  async updateQualityMetrics(lessonId) {
    try {
      ...
      throw new Error(`Failed to update quality metrics: ${_error.message}`);
    }
  }
  ...
  storeSuggestion(lessonId, suggestion) {
    return new Promise((resolve, reject) => {
      ...
      this.ragDB.db.run(sql, [
        ...
        suggestion.type,
        suggestion.text,
        suggestion.priority], function (error) {
        if (error) {
          reject(error);
        } else {
          resolve(this.lastID);
