...
class TaskCategories {
  ...
  getCategoryRank(categoryKey) {
    const _CATEGORY = this.categories[categoryKey];
    return _CATEGORY ? _CATEGORY.rank : 999; // Unknown categories go to bottom
  }
  ...
  isBlocking(categoryKey) {
    const _CATEGORY = this.categories[categoryKey];
    return _CATEGORY ? _CATEGORY.blocking : false;
  }
  ...
  isUrgent(categoryKey) {
    const _CATEGORY = this.categories[categoryKey];
    return _CATEGORY ? _CATEGORY.urgent : false;
  }
  ...
  formatCategory(categoryKey) {
    ...
    return `${_CATEGORY.icon} ${_CATEGORY.name}`;
  }
