...
class ErrorRecovery {
    constructor(options = {}) {
        ...
        this.allowTestFiles = options.allowTestFiles || false; // Allow test TODO files
    }
    ...
    async createBackup(filePath) {
        try {
            // CRITICAL SAFETY CHECK: Only backup TODO.json files (or test files in test mode)
            const isValidTodoFile = filePath.includes('TODO.json') || 
                                  (this.allowTestFiles && filePath.includes('todo.json'));
            
            if (filePath.includes('node_modules') || !isValidTodoFile) {
                return {
                    success: false,
    ...
    async atomicWrite(filePath, content, createBackup = true) {
        ...
        if (filePath.includes('node_modules') || filePath.includes('/usr/') || filePath.includes('/bin/') || 
            filePath.includes('/lib/') || filePath.includes('/system/') || !isValidTodoFile) {
            return {
                success: false,
        ...
        try {
            ...
            return {
                ...
                size: backupResult ? backupResult.size : null,
                timestamp: backupResult ? backupResult.timestamp : null,
                checksum: backupResult ? backupResult.checksum : null
            };
