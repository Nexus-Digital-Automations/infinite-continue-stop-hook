...
class ProductionLoggerFactory {
  ...
  async initialize() {
    try {
      ...
      initLogger.error({
        ...
        stack: _.stack }, 'Failed to initialize production logging');
      throw error;
    }
  }
  ...
  logWithTracking(_level, logger, _data, _message, _module) {
    try {
      ...
      initLogger.error({
        logging_error: loggingError._message,
  ...
  timeOperation(logger, operationName, _module) {
    ...
    return {
        ...
        this.performanceLog(logger, timingData, `Operation ${operationName} completed in ${_duration.toFixed(2)}ms`);
  ...
  performHealthCheck() {
    try {
      ...
      initLogger.error({
        health_check_error: error._message,
        health_check_failed: true,
        error_stack: _.stack }, 'Health monitoring check failed');
    }
  }
