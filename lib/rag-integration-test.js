  ...
  async runAllTests() {
    ...
    for (const { name, test } of tests) {
      try {
        ...
        const result = await test(); // eslint-disable-line no-await-in-loop -- Sequential test execution required for proper isolation and error reporting
...
`;
  ...
  async testCrossProjectKnowledge() {
    ...
    const lessonPromises = projects.map(async (project) => {
      const lesson = {
      ...
      return await this.ragDB.storeLesson(lesson);
    });
  ...
  test.runAllTests()
    .then(summary => {
      if (!summary.integrationReady) {
        throw new Error('RAG integration tests failed - system not ready');
      }
