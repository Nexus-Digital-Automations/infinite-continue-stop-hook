...
class EmbeddingGenerator {
  constructor(config = {}) {
    ...
    this.logger = new _Logger('EmbeddingGenerator');
    this.models = new Map();
    this.cache = new Map();
   ...
   * @private
   */
  async _loadModel(modelName, task) {
  ...
  async generateEmbeddings(content, options = {}) {
    ...
    try {
      ...
      for (let i = 0; i < contents.length; i += this.config.batchSize) {
        ...
        const batchResults = await this._processBatch(batch, primaryModel, options);
   ...
   * @private
   */
  async _processBatch(batch, model, options) {
    const results = [];
    ...
    for (const content of batch) {
      try {
        ...
        results.push(new Array(this.config.embeddingDimension).fill(0));
      }
    }
   ...
   * @private
   */
  async _generateSingleEmbedding(content, model) {
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
