...
class VectorDatabase {
   ...
   * @private
   */
  async _initializeMetadataDB() {
    return new Promise((resolve, reject) => {
      this.metadataDb = new sqlite3.Database(this.config.metadataPath, (err) => {
        if (err) {
          reject(err);
          return;
        }
   ...
   * @private
   */
  async _createIndex(contentType) {
    try {
      let _index;
   ...
   * @private
   */
  async _insertMetadata(vectorId, metadata) {
    return new Promise((resolve, reject) => {
      const insertSQL = `
   ...
   * @private
   */
  async _rankAndFilterResults(results, options) {
