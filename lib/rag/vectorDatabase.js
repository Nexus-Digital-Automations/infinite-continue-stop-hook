...
class VectorDatabase {
  ...
  async addVectors(vectors, metadata) {
    ...
    try {
      ...
      const addPromises = Array.from(groupedData.entries()).map(async ([contentType, data]) => {
        const { vectors: typeVectors, metadata: typeMetadata } = data;
        return await this._addVectorsToIndex(contentType, typeVectors, typeMetadata);
      });
  ...
  async _addVectorsToIndex(contentType, vectors, metadata) {
    ...
    for (let i = 0; i < vectors.length; i += this.config.batchSize) {
      ...
      const metadataPromises = batchVectors.map(async (_, j) => {
      ...
      const batchVectorIds = await Promise.all(metadataPromises);
