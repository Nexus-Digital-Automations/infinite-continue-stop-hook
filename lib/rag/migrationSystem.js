...
class MigrationSystem {
  constructor(config = {}) {
    this.config = {
      // Migration paths
      sourcePath: config.sourcePath || _path.join(process.cwd(), 'development'),
      backupPath: config.backupPath || _path.join(process.cwd(), 'development', 'rag', 'migration-backups'),
      ...
      progressFile: config.progressFile || _path.join(process.cwd(), 'development', 'rag', 'migration-progress.json'),
    ...
    this.logger = new _Logger('MigrationSystem');
  ...
  async _loadProgress() {
    try {
      ...
      const progressData = await _fs.readFile(this.config.progressFile, 'utf-8');
  ...
  async _saveProgress() {
    try {
      // eslint-disable-next-line security/detect-non-literal-fs-filename -- Migration path validated through migration system
      await _fs.mkdir(_path.dirname(this.config.progressFile), { recursive: true });
      ...
      // eslint-disable-next-line security/detect-non-literal-fs-filename -- Migration path validated through migration system
      await _fs.writeFile(this.config.progressFile, JSON.stringify(progressData, null, 2));
  ...
  async _createBackup() {
    try {
      ...
      const backupPromises = directories.map(async (dir) => {
        const sourcePath = _path.join(this.config.sourcePath, dir);
        const targetPath = _path.join(backupDir, dir);
  ...
  async _copyDirectory(source, target) {
    try {
      // eslint-disable-next-line security/detect-non-literal-fs-filename -- Migration path validated through migration system
      await _fs.mkdir(target, { recursive: true });
      // eslint-disable-next-line security/detect-non-literal-fs-filename -- Migration path validated through migration system
      const files = await _fs.readdir(source);

      // Process files in parallel for better performance
      await Promise.all(files.map(async (file) => {
        const sourcePath = _path.join(source, file);
        const targetPath = _path.join(target, file);

        // eslint-disable-next-line security/detect-non-literal-fs-filename -- Migration path validated through migration system
        const stat = await _fs.stat(sourcePath);

        if (stat.isDirectory()) {
          await this._copyDirectory(sourcePath, targetPath);
        } else {
          await _fs.copyFile(sourcePath, targetPath);
        }
      }));
    } catch (error) {
  ...
  async _discoverContent() {
    ...
    const lessonPromises = lessonTypes.map(async (lessonType) => {
      const lessonDir = _path.join(this.config.sourcePath, 'lessons', lessonType);
      return await this._scanDirectory(lessonDir, lessonType);
    });
  ...
  async _scanDirectory(directory, contentType) {
    ...
    try {
      // eslint-disable-next-line security/detect-non-literal-fs-filename -- Migration path validated through migration system
      const entries = await _fs.readdir(directory, { withFileTypes: true });

      for (const entry of entries) {
        const fullPath = _path.join(directory, entry.name);
        ...
        if (entry.isDirectory()) {
          ...
          const ext = _path.extname(entry.name);
  ...
  async _analyzeContent(files) {
    ...
    const analysisPromises = files.map(async (file) => {
      try {
        ...
        // eslint-disable-next-line security/detect-non-literal-fs-filename -- Migration path validated through migration system
        const content = await _fs.readFile(file.path, 'utf-8');
  ...
  _analyzeContentStructure(content, file) {
    ...
    const title = lines.find(line => line.startsWith('# '))?.substring(2).trim() ||
                  _path.basename(file.name, file.extension);
  ...
  async _migrateContent(analysisResults, components) {
    ...
    for (let i = 0; i < analysisResults.length; i += this.config.batchSize) {
      const batch = analysisResults.slice(i, i + this.config.batchSize);

      this.logger.debug(`Processing batch ${Math.floor(i / this.config.batchSize) + 1}`, {
        batchSize: batch.length,
        progress: `${i + batch.length}/${analysisResults.length}`,
      });
      ...
      if (this.config.enableProgressTracking) {
        await this._saveProgress(); // eslint-disable-line no-await-in-loop -- Progress checkpointing must be sequential to maintain consistency
      }

      // Small delay to prevent overwhelming the system
      await new Promise(resolve => { setTimeout(resolve, 10); }); // eslint-disable-line no-await-in-loop -- Rate limiting delay required to prevent system overload
    }
  ...
  async _finalizeMigration() {
    try {
      ...
      if (this.config.enableProgressTracking) {
        // eslint-disable-next-line security/detect-non-literal-fs-filename -- Migration path validated through migration system
        await _fs.unlink(this.config.progressFile).catch(() => {});
      }
  ...
  _generateContentHash(content) {
    ...
    return _crypto.createHash('md5').update(content).digest('hex');
  }
