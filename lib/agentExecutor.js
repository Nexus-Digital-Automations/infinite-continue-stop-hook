...
class AgentExecutor {
    ...
    async buildPrompt(task, mode, todoData, taskManager = null) {
        // Check if linter feedback should be the complete response
        const linterFeedback = await this.buildLinterFeedback(taskManager);
        if (linterFeedback) {
            // When linter feedback is available, provide it with clear stopping instructions
            return this.buildCompleteLinterFeedbackPrompt(linterFeedback, task, mode, todoData);
        }
        
        // Create minimal prompt that instructs reading files (original flow when no linter feedback)
        ...
        fullPrompt += `READ OR REVIEW IF ALREADY READ all files listed below:\n\n`;
        ...
        developmentFiles.forEach(file => {
            fullPrompt += `${fileIndex}. \`${file}\`\n`;
            fileIndex++;
        });
        
        // Add task-specific file reading instructions
        const taskFileInstructions = this.buildTaskFileInstructions(task, fileIndex);
        if (taskFileInstructions.trim()) {
            fullPrompt += taskFileInstructions;
        } else {
            fullPrompt += `\n`;
        }
    ...
    discoverDevelopmentFiles(workingDir, mode) {
        ...
        try {
            ...
            // Add mode files from modes subdirectory
            const modesDir = path.join(developmentDir, 'modes');
            if (fs.existsSync(modesDir)) {
                if (mode.toLowerCase().replace('_', '-') === 'task-creation') {
                    // For TASK_CREATION mode, scan and read ALL mode files
                    try {
                        const modeFiles = fs.readdirSync(modesDir);
                        modeFiles.forEach(modeFile => {
                            const modePath = path.join(modesDir, modeFile);
                            const modeStat = fs.statSync(modePath);
                            
                            if (modeStat.isFile() && modeFile.endsWith('.md')) {
                                files.push(`./development/modes/${modeFile}`);
                            }
                        });
                    } catch (error) {
                        console.error(`Warning: Error reading modes directory: ${error.message}`);
                    }
                } else {
                    // For other modes, add only the specific mode file
                    const modeFileName = `${mode.toLowerCase().replace('_', '-')}.md`;
                    const modeFile = path.join(modesDir, modeFileName);
    ...
    buildTaskContext(task, mode, todoData) {
        ...
        if (mode === 'TASK_CREATION') {
            ...
            context += `**CORE PRINCIPLE**: Create only necessary tasks that address specific problems or opportunities. Always be proactive in identifying real issues and gaps, but never create tasks just to meet arbitrary numbers.\n\n`;
            ...
            context += `\n**🔍 MODE FILES SCAN REQUIRED**: All mode files in development/modes/ directory are included below for comprehensive understanding of available task modes and their purposes. Read ALL mode files to understand:\n`;
            context += `- Available work modes and their specific use cases\n`;
            context += `- Mode-specific workflows and requirements\n`;
            context += `- How to structure tasks for each mode effectively\n`;
            context += `- Quality standards and success criteria per mode\n\n`;
...
- No skipped tests`
    ...
    buildTaskFileInstructions(task, startingIndex = 3) {
            ...
            enhancedImportantFiles.forEach(file => {
                if (file && file.trim()) {
                    ...
                    if (file.includes('research-report-') && (task.mode === 'RESEARCH' || task.mode === 'research')) {
                        if (this.researchReportExists(task.id)) {
                            instructions += `${fileIndex}. \`${file.trim()}\` (research report - contains previous research findings)\n`;
                        } else {
                            instructions += `${fileIndex}. \`${file.trim()}\` (create research report following standardized template)\n`;
                        }
                    } else {
                        instructions += `${fileIndex}. \`${file.trim()}\`\n`;
                    }
                    fileIndex++;
                }
            });
        ...
        if (task.dependencies && task.dependencies.length > 0) {
            task.dependencies.forEach(dep => {
                if (dep && dep.trim()) {
                    instructions += `${fileIndex}. \`${dep.trim()}\`\n`;
                    fileIndex++;
                }
            });
        }
    ...
    /**
     * Build complete linter feedback prompt - this becomes the ENTIRE feedback when available
     */
    buildCompleteLinterFeedbackPrompt(linterFeedback, task, mode, todoData) {
        // Start with a focused header emphasizing this is the complete feedback
        let completePrompt = '# LINTER RECOMMENDATION - COMPLETE FEEDBACK\n\n';
        
        // Add the linter feedback as the primary content
        completePrompt += linterFeedback;
        
        // Add clear stopping instructions
        completePrompt += '\n\n---\n\n';
        completePrompt += `## 🛑 HOW TO STOP THE LINTER FEEDBACK LOOP\n\n`;
        completePrompt += `Once you have addressed all linter issues, you MUST clear the linter feedback to continue:\n\n`;
        completePrompt += `**Option 1 - Shell Script (Recommended):**\n`;
        completePrompt += `\`\`\`bash\n`;
        completePrompt += `./scripts/taskmanager/taskmanager.sh linter-clear\n`;
        completePrompt += `\`\`\`\n\n`;
        completePrompt += `**Option 2 - Manual Testing:**\n`;
        completePrompt += `1. Fix all linter errors shown above\n`;
        completePrompt += `2. Run: \`npm run lint\` to verify no errors remain\n`;
        completePrompt += `3. If clean, use Option 1 above to clear feedback\n\n`;
        
        // Add minimal context about the current system state
        completePrompt += '\n\n---\n\n';
        completePrompt += `## Current System Context\n\n`;
        completePrompt += `**Current Task:** ${task.title || task.description}\n`;
        completePrompt += `**Mode:** ${mode}\n`;
        completePrompt += `**Project:** ${todoData.project}\n`;
        
        const summary = this.getTaskSummary(todoData);
        completePrompt += `**Progress:** ${summary.completed}/${summary.total} tasks (${summary.percentage}%)\n\n`;
        
        // Emphasize that this linter recommendation is the complete guidance
        completePrompt += '**⚠️ IMPORTANT: This linter feedback will keep showing until you clear it using one of the commands above.**\n';
        completePrompt += 'Address the linting issues, then clear the feedback to proceed with other tasks.\n';
        
        return completePrompt;
    }

    /**
     * Build linter feedback section for the prompt
     */
    async buildLinterFeedback(taskManager) {
        if (!taskManager) {
            return null;
        }

        try {
            const linterMessage = await taskManager.generateLinterFeedbackMessage();
            if (!linterMessage) {
                return null;
            }

            let feedback = `## ${linterMessage.title}\n\n`;
            feedback += linterMessage.message + '\n';
            
            return feedback;
        } catch (error) {
            console.warn(`Failed to generate linter feedback: ${error.message}`);
            return null;
        }
    }
