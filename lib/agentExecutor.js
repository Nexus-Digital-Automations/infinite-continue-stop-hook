...
class AgentExecutor {
    ...
    buildPrompt(task, mode, todoData) {
        ...
        // Add instruction to read mode-specific file from project
        const modeFileName = `${mode.toLowerCase().replace('_', '-')}.md`;
        fullPrompt += `## Mode-Specific Instructions\n\n`;
        fullPrompt += `IMPORTANT: Read the mode-specific instructions from:\n`;
        fullPrompt += `\`./development/modes/${modeFileName}\`\n\n`;
        fullPrompt += `This file contains detailed guidance for ${mode.replace('_', ' ')} mode.\n\n`;
        fullPrompt += `---\n\n`;
