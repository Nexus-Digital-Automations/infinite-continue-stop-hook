...
class LockSecurityHelper {
  static validateLockPath(lockPath, baseDir = process.cwd()) {
    ...
    const resolvedPath = _path.resolve(lockPath);
...
class DistributedLockManager {
  ...
  releaseLock(filePath, agentId) {
    ...
    try {
      ...
      if (_fs.existsSync(validatedLockFile)) {
        _fs.unlinkSync(validatedLockFile);
      }
      ...
      return {
        success: false,
        error: `Lock release failed: ${error.message}`,
      };
    }
  }
  ...
  attemptLockAcquisition(lockFile, agentId, lockId) {
    try {
      ...
      if (_fs.existsSync(validatedLockFile)) {
        ...
        const lockData = JSON.parse(_fs.readFileSync(validatedLockFile, 'utf8'));
      ...
      // Use atomic write operation with validated paths
      const tempFile = validatedLockFile + '.tmp.' + Date.now();
      const validatedTempFile = LockSecurityHelper.validateLockPath(tempFile);
      /* eslint-disable security/detect-non-literal-fs-filename */
      _fs.writeFileSync(validatedTempFile, JSON.stringify(lockData, null, 2));
      _fs.renameSync(validatedTempFile, validatedLockFile);
      /* eslint-enable security/detect-non-literal-fs-filename */
  ...
  getLockFilePath(lockId) {
    return _path.join(this.options.lockDirectory, `${lockId}.lock`);
  }
  ...
  ensureLockDirectory() {
    try {
      ...
      /* eslint-disable security/detect-non-literal-fs-filename */
      if (!_fs.existsSync(validatedLockDir)) {
        _fs.mkdirSync(validatedLockDir, { recursive: true });
      }
      /* eslint-enable security/detect-non-literal-fs-filename */
    } catch (error) {
      // Log the error but don't throw - this allows testing invalid directories
      _logger.warn('Warning: Could not create lock directory', {
        lockDirectory: this.options.lockDirectory,
        error: error.message,
      });
  ...
  /**
     ...
     * @param {string} agentId - Agent ID to check for deadlocks
     * @returns {Promise<Array|null>} Deadlock chain if detected, null otherwise
     */
  detectDeadlock(agentId) {
  ...
  cleanupStaleLocks() {
    ...
    const cleanupPromises = staleLocks.map((lockId) => {
      ...
      if (lockInfo) {
        try {
          ...
          _logger.warn('Failed to cleanup stale lock', { lockId, error: error.message });
        }
      }
    });
    ...
    if (staleLocks.length > 0) {
      _logger.info('Cleaned up stale locks', { count: staleLocks.length });
    }
  }
    ...
    Array.from(this.activeLocks.keys()).forEach((lockId) => {
      ...
      if (lockInfo) {
        try {
          ...
          _logger.warn('Failed to cleanup lock on exit', { error: error.message });
        }
      }
    });
