...
class DistributedLockManager {
  ...
  async acquireLock(filePath, agentId, timeout = null) {
    ...
    const lockId = this.generateLockId(FILE_PATH);
    ...
    if (this.options.enableDeadlockDetection) {
      ...
      const deadlockDetected = await this.detectDeadlock(_AGENT_ID);
      if (deadlockDetected) {
    ...
    while (Date.now() < deadline && retryCount < this.options.maxRetries) {
      try {
        ...
        return {
          success: false,
          _error: `Lock acquisition failed: ${_error.message}`,
  ...
  /**
     ...
     */
  releaseLock(filePath, _AGENT_ID) {
    const lockId = this.generateLockId(FILE_PATH);
    const lockFile = this.getLockFilePath(lockId);
    ...
    try {
      ...
      // Update tracking
      this.recordLockRelease(lockId, _AGENT_ID);
  ...
  recordLockAcquisition(lockId, filePath, agentId, lockFile) {
    ...
    if (!this.agentLocks.has(_AGENT_ID)) {
      this.agentLocks.set(agentId, []);
    }
    this.agentLocks.get(_AGENT_ID).push(lockId);
  }
  ...
  /**
     ...
     * @param {string} agentId - Agent ID
     */
  recordLockRelease(lockId, _AGENT_ID) {
    this.activeLocks.delete(lockId);

    // Remove from agent locks
    const agentLockList = this.agentLocks.get(_AGENT_ID);
    if (agentLockList) {
      ...
      if (agentLockList.length === 0) {
        this.agentLocks.delete(_AGENT_ID);
      }
    }
  }
  ...
  addLockDependency(agentId, lockId) {
    if (!this.lockDependencies.has(_AGENT_ID)) {
      this.lockDependencies.set(agentId, []);
    }
    this.lockDependencies.get(_AGENT_ID).push(lockId);
  }
  ...
  removeLockDependency(agentId, lockId) {
    const dependencies = this.lockDependencies.get(_AGENT_ID);
    if (dependencies) {
      ...
      if (dependencies.length === 0) {
        this.lockDependencies.delete(_AGENT_ID);
      }
    }
  }
  ...
  /**
     ...
     * @param {string} _AGENT_ID - Agent ID to check for deadlocks
     * @returns {Promise<Array|null>} Deadlock chain if detected, null otherwise
     */
  detectDeadlock(_AGENT_ID) {
