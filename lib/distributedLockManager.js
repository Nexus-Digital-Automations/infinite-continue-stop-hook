...
class DistributedLockManager {
  ...
  attemptLockAcquisition(lockFile, agentId, lockId) {
    try {
      ...
      if (_fs.existsSync(lockFile)) {
        ...
        const lockData = JSON.parse(_fs.readFileSync(lockFile, 'utf8'));
      ...
      const tempFile = lockFile + '.tmp.' + Date.now();
      _fs.writeFileSync(tempFile, JSON.stringify(lockData, null, 2));
      _fs.renameSync(tempFile, lockFile);
  ...
  ensureLockDirectory() {
    try {
      if (!_fs.existsSync(this.options.lockDirectory)) {
        _fs.mkdirSync(this.options.lockDirectory, { recursive: true });
      }
    } catch (error) {
  ...
  cleanupStaleLocks() {
    ...
    const cleanupPromises = staleLocks.map((lockId) => {
      ...
      if (lockInfo) {
        try {
          if (_fs.existsSync(lockInfo.lockFile)) {
            _fs.unlinkSync(lockInfo.lockFile);
          }
  ...
  cleanup() {
    ...
    Array.from(this.activeLocks.keys()).forEach((lockId) => {
      const lockInfo = this.activeLocks.get(lockId);
      if (lockInfo && _fs.existsSync(lockInfo.lockFile)) {
        try {
          _fs.unlinkSync(lockInfo.lockFile);
        } catch (error) {
