...
class DistributedLockManager {
  ...
  async acquireLock(filePath, agentId, timeout = null) {
    ...
    if (this.options.enableDeadlockDetection) {
      ...
      const deadlockDetected = await this.detectDeadlock(_agentId);
      if (deadlockDetected) {
  ...
  recordLockAcquisition(lockId, filePath, agentId, lockFile) {
    ...
    if (!this.agentLocks.has(_agentId)) {
      this.agentLocks.set(agentId, []);
    }
    this.agentLocks.get(_agentId).push(lockId);
  }
  ...
  recordLockRelease(lockId, agentId) {
    ...
    const agentLockList = this.agentLocks.get(_agentId);
    if (agentLockList) {
      ...
      if (agentLockList.length === 0) {
        this.agentLocks.delete(_agentId);
      }
    }
  }
  ...
  addLockDependency(agentId, lockId) {
    if (!this.lockDependencies.has(_agentId)) {
      this.lockDependencies.set(agentId, []);
    }
    this.lockDependencies.get(_agentId).push(lockId);
  }
  ...
  removeLockDependency(agentId, lockId) {
    const dependencies = this.lockDependencies.get(_agentId);
    if (dependencies) {
      ...
      if (dependencies.length === 0) {
        this.lockDependencies.delete(_agentId);
      }
    }
  }
