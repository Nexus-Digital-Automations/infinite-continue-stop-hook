  ...
  async loadFAISSIndexes() {
    ...
    try {
      ...
      const loadPromises = Object.keys(this.indexes).map(async entityType => {
        // eslint-disable-next-line security/detect-object-injection -- entityType from Object.keys of validated indexes
        const indexPath = _path.join(this.cacheDir, `${entityType}_index.faiss`);
  ...
  async getStatistics() {
    try {
      ...
      for (const [entityType, index] of Object.entries(this.indexes)) {
        // eslint-disable-next-line security/detect-object-injection -- entityType from Object.entries of validated indexes
        if (index) {
