...
class VectorEmbeddingManager {
  ...
  initialize() {
    try {
      ...
      throw new Error(`Vector embedding initialization failed: ${_error.message}`);
    }
  }
  ...
  generateEmbedding(text, options = {}) {
    try {
      ...
      throw new Error(`Embedding generation failed: ${_error.message}`);
    }
  }
   ...
   */
  storeEmbedding(entityType, entityId, text, metadata = {}) {
    try {
   ...
   */
  findSimilar(text, entityType, options = {}) {
    try {
   ...
   */
  searchWithFAISS(entityType, queryVector, limit, threshold) {
    try {
      // Validate entityType to prevent object injection
      if (!this.isValidEntityType(entityType)) {
        throw new Error(`Invalid entity type: ${entityType}`);
   ...
   */
  searchWithDatabase(entityType, queryVector, limit, threshold, _includeScores) {
    try {
  ...
  cleanupCache() {
    ...
    try {
      ...
      const cleanupPromises = cacheFiles.map(async (file) => {
        try {
          const _filePath = path.join(this.cacheDir, file);
          // eslint-disable-next-line security/detect-non-literal-fs-filename -- _filePath constructed from validated directory listing
          const stats = await FS.stat(__filename);
          const isExpired = Date.now() - stats.mtime.getTime() > this.cacheMaxAge;

          if (isExpired) {
            // eslint-disable-next-line security/detect-non-literal-fs-filename -- FILE_PATH constructed from validated directory listing
