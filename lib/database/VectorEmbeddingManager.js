...
class VectorEmbeddingManager {
  ...
  async searchWithFAISS(entityType, queryVector, limit, threshold) {
    try {
      ...
      // Filter by threshold and get entity details
      ...
      for (let i = 0; i < results.labels.length; i++) {
        ...
        if (similarity >= threshold) {
          const entityId = results.labels[i];
   ...
   */
  async searchWithDatabase(entityType, queryVector, limit, threshold, includeScores) {
    try {
  ...
  async getEntityDetails(entityType, entityId) {
    ...
    return await this.dbManager.get(
