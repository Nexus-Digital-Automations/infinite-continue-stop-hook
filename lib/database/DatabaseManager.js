...
class DatabaseManager {
  ...
  createConnection() {
    return new Promise((resolve, reject) => {
      this.db = new SQLITE3.Database(this.dbPath, (_error) => {
        if (error) {
  ...
  prepare(sql) {
    return new Promise((resolve, reject) => {
      const stmt = this.db.prepare(sql, (_error) => {
        if (error) {
          reject(error);
        } else {
          resolve(stmt);
  ...
  async backup(backupPath) {
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        backup.step(-1, (_error) => {
          if (error) {
            reject(error);
          } else {
            backup.finish((_error) => {
              if (error) {
                reject(error);
              } else {
