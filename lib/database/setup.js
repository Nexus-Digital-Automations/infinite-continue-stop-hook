...
class RAGDatabaseSetup {
  ...
  async setup() {
    try {
      ...
      this.log(`❌ Setup failed: ${_error.message}`);
      throw _error;
    }
  }
  ...
  async insertSampleData() {
    ...
    for (const lesson of sampleLessons) {
      ...
      const _result = await this.dbManager.run(
        `INSERT INTO lessons
         (title, _category, content, context, tags, code_patterns, confidence_score, effectiveness_score, project_id, agent_id)
...
async function main(_category = 'general') {
  ...
  try {
    const _result = await setup.setup();
