...
class RAGDatabaseSetup {
  constructor(options = {}) {
    ...
    this.backupPath = options.backupPath || _path.join(__dirname, '../../data/backups');
  ...
  async createInitialBackup() {
    ...
    const backupFile = _path.join(this.backupPath, `rag_learning_initial_${timestamp}.db`);
  ...
  log(message) {
    if (this.verbose) {
      ...
      // eslint-disable-next-line no-console -- Setup script logging for verbose mode output
...
async function main() {
  ...
  try {
    ...
    // eslint-disable-next-line no-console -- Setup script success output for CLI usage
    console.log('\n✅ Setup completed successfully!');
    // eslint-disable-next-line no-console -- Setup script result output for CLI usage
    console.log(JSON.stringify(result, null, 2));
    return result;
  } catch (error) {
    // eslint-disable-next-line no-console -- Setup script error output for CLI usage
    console.error('\n❌ Setup failed:', error.message);
    // eslint-disable-next-line no-console -- Setup script error stack output for debugging
