...
class RAGDatabase {
  constructor(dbPath = './data/rag-lessons.db') {
    ...
    const dataDir = _path.dirname(dbPath);
    // eslint-disable-next-line security/detect-non-literal-fs-filename -- Database path controlled by RAG database configuration
    if (!_fs.existsSync(dataDir)) {
      // eslint-disable-next-line security/detect-non-literal-fs-filename -- Database path validated through RAG database security
   ...
   */
      loggers.stopHook.log('[RAG-DB] Initializing database and models...');
    try {
      console.log('[RAG-DB] Initializing database and models...');

      // Set up transformers cache directory to prevent path errors
      const cacheDir = _path.join(process.cwd(), '.cache', 'transformers');
      ...
      this.db = new _sqlite3.Database(this.dbPath);

      // Create tables if they don't exist
      await this.createTables();
      loggers.stopHook.log('[RAG-DB] Loading embedding model...');
      // Initialize embedding model
      console.log('[RAG-DB] Loading embedding model...');
      ...
      loggers.stopHook.log('[RAG-DB] Database and models initialized successfully');
      this.initialized = true;
      console.log('[RAG-DB] Database and models initialized successfully');

      loggers.stopHook.error('[RAG-DB] Initialization failed:', error);
    } catch (error) {
      console.error('[RAG-DB] Initialization failed:', error);
   ...
   */
      loggers.stopHook.log('[RAG-DB] Creating vector index...');
    try {
      console.log('[RAG-DB] Creating vector index...');
      // Create new IndexFlatIP for 384 dimensions (all-MiniLM-L6-v2 embedding size)
      // Using Inner Product for cosine similarity
      loggers.stopHook.log('[RAG-DB] Vector index created successfully');
      ...
      console.warn('[RAG-DB] Vector index initialization failed, using fallback search:', error);
  ...
  async generateEmbedding(text) {
    ...
    try {
      ...
      console.error('[RAG-DB] Embedding generation failed:', error);
      throw error;
    }
  }
  ...
  async storeLesson(lessonData) {
    ...
    try {
      const { title, content, category, subcategory, projectPath, filePath, tags } = lessonData;
      ...
      return new Promise((resolve, reject) => {
        const sql = `
          INSERT INTO lessons (title, content, category, subcategory, project_path, file_path, tags, embedding_id)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        `;
        ...
        db.run(sql, [title, content, category, subcategory, projectPath, filePath, JSON.stringify(tags), embeddingId], function (error) {
          if (error) {reject(error);} else {
            ...
            try {
              ...
              console.warn('[RAG-DB] Vector index error, lesson stored without vector search:', vectorError);
              resolve({
                success: true,
                lessonId,
                embeddingId,
      ...
      console.error('[RAG-DB] Failed to store lesson:', error);
      return { success: false, error: error.message };
    }
  }
  ...
  async storeError(errorData) {
    ...
    try {
        ...
        db.run(sql, [title, content, errorType, resolution, projectPath, filePath, JSON.stringify(tags), embeddingId], function (error) {
          if (error) {reject(error);} else {
            ...
            try {
              ...
              console.warn('[RAG-DB] Vector index error, error stored without vector search:', vectorError);
              resolve({
                success: true,
                errorId,
                embeddingId,
      ...
      console.error('[RAG-DB] Failed to store error:', error);
      return { success: false, error: error.message };
    }
  }
  ...
  async searchLessons(query, limit = 10, threshold = 0.7) {
    ...
    try {
      ...
      if (this.vectorIndex && !this.useSimpleSearch) {
        ...
        try {
          // Check if index has any vectors before searching
            loggers.stopHook.log('[RAG-DB] FAISS index is empty, using fallback search');
          if (totalVectors === 0) {
            console.log('[RAG-DB] FAISS index is empty, using fallback search');
            searchResults = [];
          } else {
          ...
          loggers.stopHook.warn('[RAG-DB] FAISS search failed, using fallback:', faissError);
        } catch (faissError) {
          console.warn('[RAG-DB] FAISS search failed, using fallback:', faissError);
          this.useSimpleSearch = true;
        }
      }
      ...
      lessons.forEach((lesson) => {
        const result = searchResults.find(r => r.id === lesson.id);
        lesson.similarity = result ? result.similarity : 0;
      });
      ...
      loggers.stopHook.error('[RAG-DB] Lesson search failed:', error);
    } catch (error) {
      console.error('[RAG-DB] Lesson search failed:', error);
      return { success: false, error: error.message };
    }
  }
  ...
  async searchErrors(query, limit = 10, threshold = 0.7) {
    ...
    try {
      ...
      errors.forEach((error) => {
        const result = searchResults.find(r => r.id === error.id);
        error.similarity = result ? result.similarity : 0;
      });
      ...
      console.error('[RAG-DB] Error search failed:', error);
      return { success: false, error: error.message };
    }
  }
  ...
  saveIndex() {
    try {
      if (this.vectorIndex) {
        loggers.stopHook.log('[RAG-DB] Vector index saved to disk');
        this.vectorIndex.write(indexPath);
        console.log('[RAG-DB] Vector index saved to disk');
      loggers.stopHook.error('[RAG-DB] Failed to save vector index:', error);
    } catch (error) {
      console.error('[RAG-DB] Failed to save vector index:', error);
    }
  }
  ...
  async close() {
    try {
      ...
      loggers.stopHook.log('[RAG-DB] Database connection closed');

      loggers.stopHook.error('[RAG-DB] Error closing database:', error);
    } catch (error) {
      console.error('[RAG-DB] Error closing database:', error);
    }
  }
   ...
   */
      loggers.stopHook.log('[RAG-DB] Vector index not available, skipping vector loading');
    if (!this.vectorIndex || this.useSimpleSearch) {
      console.log('[RAG-DB] Vector index not available, skipping vector loading');
      return;
    }
      loggers.stopHook.log('[RAG-DB] Loading existing vectors into FAISS index...');
    try {
      console.log('[RAG-DB] Loading existing vectors into FAISS index...');

      // Get all embeddings from database
      const embeddings = await this.getAllEmbeddings();
        loggers.stopHook.log('[RAG-DB] No existing vectors to load');
      if (embeddings.length === 0) {
        console.log('[RAG-DB] No existing vectors to load');
        return;
      }
      ...
      for (const embeddingData of embeddings) {
        try {
          ...
          loggers.stopHook.warn(`[RAG-DB] Failed to load vector ${embeddingData.id}:`, error.message);
        } catch (error) {
          console.warn(`[RAG-DB] Failed to load vector ${embeddingData.id}:`, error.message);
        }
      loggers.stopHook.log(`[RAG-DB] Successfully loaded ${loadedCount} vectors into FAISS index`);
      loggers.stopHook.log(`[RAG-DB] Total vectors in index: ${this.vectorIndex.ntotal()}`);
      console.log(`[RAG-DB] Successfully loaded ${loadedCount} vectors into FAISS index`);
      console.log(`[RAG-DB] Total vectors in index: ${this.vectorIndex.ntotal()}`);
      loggers.stopHook.error('[RAG-DB] Failed to load existing vectors:', error);
    } catch (error) {
      console.error('[RAG-DB] Failed to load existing vectors:', error);
