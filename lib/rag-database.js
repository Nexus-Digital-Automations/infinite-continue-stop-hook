...
class RAGDatabase {
  ...
  async initializeDatabase() {
    ...
    try {
      ...
      loggers.stopHook.log('[RAG-DB] Database andmodels initialized successfully');
      this.initialized = true;
      console.log('[RAG-DB] Database andmodels initialized successfully');
  ...
  async generateEmbedding(text) {
    ...
    try {
      const _result = await this.embeddingModel(text, { pooling: 'mean', normalize: true });
      return result.data;
    } catch {
  ...
  async searchLessons(query, limit = 10, threshold = 0.7) {
    ...
    try {
      ...
      lessons.forEach((lesson) => {
        const _result = searchResults.find(r => r.id === lesson.id);
  ...
  async searchErrors(query, limit = 10, threshold = 0.7) {
    ...
    try {
      ...
      errors.forEach((error) => {
        const _result = searchResults.find(r => r.id === error.id);
