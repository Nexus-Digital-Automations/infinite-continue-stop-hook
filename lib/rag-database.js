...
class RAGDatabase {
    ...
    try {
      ...
      if (this.vectorIndex && !this.useSimpleSearch) {
        ...
        try {
          ...
          if (totalVectors === 0) {
            loggers.stopHook.log('[RAG-DB] _FAISS index is empty, using fallback search');
  ...
  async loadExistingVectors() {
    ...
    loggers.stopHook.log('[RAG-DB] Loading existing vectors into _FAISS index...');
    try {
      ...
      loggers.stopHook.log(`[RAG-DB] Successfully loaded ${loadedCount} vectors into _FAISS index`);
