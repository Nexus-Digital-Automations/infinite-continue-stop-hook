...
class RAGDatabase {
  ...
  /**
   * Store lesson in database with embedding (with transaction support)
   */
  async storeLesson(lessonData) {
    ...
    try {
      ...
      try {
        ...
        embeddingId = await this.storeEmbedding(embedding);

        // Store lesson within transaction
        const sql = `
          INSERT INTO lessons (title, content, category, subcategory, project_path, file_path, tags, embedding_id)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        `;

        const result = await this.runQuery(sql, [title, content, category, subcategory, projectPath, filePath, JSON.stringify(tags), embeddingId]);
        lessonId = result.lastID;

        // Commit transaction before vector operations (vector ops are non-critical)
        await this.runQuery('COMMIT');

        // Add to vector index (non-transactional, best-effort)
        try {
          if (this.vectorIndex && !this.useSimpleSearch) {
            this.vectorIndex.add(embedding);
          }
          // Store embedding for fallback search
          this.vectorEmbeddings.push({ id: lessonId, embedding, type: 'lesson' });
        } catch (vectorError) {
      ...
      return { success: false, _error: error.message, lessonId, embeddingId };
    }
  }

  /**
   * Helper method to run SQL queries with Promise wrapper
   */
  runQuery(sql, params = []) {
    return new Promise((resolve, reject) => {
  ...
  async storeError(errorData) {
    ...
    try {
      ...
      try {
        ...
        // Store error within transaction
        const sql = `
          INSERT INTO errors (title, content, error_type, resolution, project_path, file_path, tags, embedding_id)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        `;

        const result = await this.runQuery(sql, [title, content, errorType, resolution, projectPath, filePath, JSON.stringify(tags), embeddingId]);
        ...
        try {
          ...
          // Store embedding for fallback search
          this.vectorEmbeddings.push({ id: errorId, embedding, type: 'error' });
        } catch (vectorError) {
      ...
      return { success: false, _error: error.message, errorId, embeddingId };
    }
  }
