...
class RAGDatabase {
  ...
  storeLesson(lessonData) {
    ...
    try {
      const { title, content, category, subcategory, projectPath, _filePath, tags } = lessonData;
      ...
      return new Promise((resolve, reject) => {
        ...
        db.run(sql, [title, content, category, subcategory, projectPath, _filePath, JSON.stringify(tags), embeddingId], function (error) {
  ...
  storeError(errorData) {
    ...
    try {
      const { title, content, errorType, resolution, projectPath, _filePath, tags } = errorData;
      ...
      return new Promise((resolve, reject) => {
        ...
        db.run(sql, [title, content, errorType, resolution, projectPath, _filePath, JSON.stringify(tags), embeddingId], function (error) {
          if (error) {
   ...
   */
  searchLessons(query, limit = 10, threshold = 0.7) {
    if (!this.initialized) {
      await this.initialize();
    }
      ...
      if (this.vectorIndex && !this.useSimpleSearch) {
        ...
        try {
          ...
          if (totalVectors === 0) {
            ...
            loggers.app.info('[RAG-DB] FAISS index is empty, using fallback search');
            searchResults = [];
          } else {
          ...
          loggers.app.warn('[RAG-DB] FAISS search failed, using fallback:', faissError);
          this.useSimpleSearch = true;
        }
      }
   ...
   */
  searchErrors(query, limit = 10, threshold = 0.7) {
    if (!this.initialized) {
      await this.initialize();
    }
  ...
  loadExistingVectors() {
    ...
    try {
      loggers.app.info('[RAG-DB] Loading existing vectors into FAISS index...');
      ...
      loggers.app.info(`[RAG-DB] Successfully loaded ${loadedCount} vectors into FAISS index`);
