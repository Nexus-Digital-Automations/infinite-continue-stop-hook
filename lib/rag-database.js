  ...
  async searchLessons(query, limit = 10, threshold = 0.7) {
    ...
    try {
      ...
      if (this.vectorIndex && !this.useSimpleSearch) {
        ...
        try {
          const faissResults = this.vectorIndex.search([queryEmbedding], limit);
          for (let i = 0; i < faissResults.distances[0].length; i++) {
            const similarity = faissResults.distances[0][i];
            if (similarity >= threshold) {
              searchResults.push({
                id: faissResults.labels[0][i] + 1,
