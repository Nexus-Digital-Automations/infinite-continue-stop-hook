...
class FilesystemSecurity {
   ...
   */
  static safeResolvePath(basePath, FILE_PATH) {
    if (!this.isValidPath(FILE_PATH)) {
      throw new Error(`Invalid file path detected: ${FILE_PATH}`);
    }
    ...
    if (!resolvedPath.startsWith(resolvedBase)) {
      throw new Error(`Path ${FILE_PATH} is outside project boundaries`);
    }
   ...
   */
  static safeFileExists(basePath, FILE_PATH) {
    try {
      const safePath = this.safeResolvePath(basePath, FILE_PATH);
   ...
   * @throws {Error} If file access is unsafe or fails
   */
  static safeWriteFileSync(basePath, FILE_PATH, content, encoding = 'utf8') {
    const safePath = this.safeResolvePath(basePath, FILE_PATH);
...
class AgentManager {
  ...
  registerAgent(agentConfig) {
    ...
    // Safe assignment after validation - _AGENT_ID has been validated against injection attacks
  ...
  /**
     ...
     */
  unregisterAgent(_AGENT_ID) {
    const todoData = await this.readTodo();

    // Security: Validate _AGENT_ID to prevent object injection
    if (!this.isValidAgentId(_AGENT_ID)) {
      this.logger.logError(new Error('Invalid agent ID'), `unregisterAgent - invalid _AGENT_ID: ${_AGENT_ID}`);
      return false;
    }
    ...
    // Release any assigned tasks - _AGENT_ID has been validated
    /* eslint-disable-next-line security/detect-object-injection */
    ...
    // Remove agent - validated safe OPERATION- _AGENT_ID has been validated
    /* eslint-disable-next-line security/detect-object-injection */
  ...
  /**
     ...
     * @param {string} _AGENT_ID - Agent ID
     * @returns {boolean} Success status
     */
  updateAgentHeartbeat(_AGENT_ID) {
    try {
      const todoData = await this.readTodo();

      // Security: Validate _AGENT_ID to prevent object injection
      if (!this.isValidAgentId(_AGENT_ID)) {
        this.logger.logError(new Error('Invalid agent ID'), `updateAgentHeartbeat - invalid _AGENT_ID: ${_AGENT_ID}`);
        return false;
      }
  ...
  reinitializeAgent(agentId, config = {}) {
    try {
      ...
      this.logger.addFlow(`Reinitialized agent ${_AGENT_ID} - heartbeat renewed, timeout reset`);
  ...
  getAgent(_AGENT_ID) {
    ...
    // Security: Validate _AGENT_ID to prevent object injection
    if (!this.isValidAgentId(_AGENT_ID)) {
      this.logger.logError(new Error('Invalid agent ID'), `getAgent - invalid _AGENT_ID: ${_AGENT_ID}`);
      return null;
    }

    if (todoData.agents && Object.prototype.hasOwnProperty.call(todoData.agents, _AGENT_ID)) {
      /* eslint-disable-next-line security/detect-object-injection */
      return { _AGENT_ID, ...todoData.agents[_AGENT_ID] };
    }
  ...
  updateAgentWorkload(agentId, workloadDelta) {
    ...
    if (todoData.agents && Object.prototype.hasOwnProperty.call(todoData.agents, _AGENT_ID)) {
      /* eslint-disable security/detect-object-injection */
      todoData.agents[_AGENT_ID].workload = Math.max(0,
        (todoData.agents[_AGENT_ID].workload || 0) + workloadDelta,
      );
      /* eslint-enable security/detect-object-injection */
      await this.writeTodo(todoData);
      return true;
  ...
  /**
     ...
     */
  releaseAllTasksFromAgent(_AGENT_ID) {
    const todoData = await this.readTodo();
    const releasedTasks = [];

    // Security: Validate _AGENT_ID to prevent object injection
    if (!this.isValidAgentId(_AGENT_ID)) {
      this.logger.logError(new Error('Invalid agent ID'), `releaseAllTasksFromAgent - invalid _AGENT_ID: ${_AGENT_ID}`);
      return [];
    }
  ...
  /**
     ...
     */
  markAgentsAsInactive(agentIds) {
    const todoData = await this.readTodo();

    // Process agent inactivation in parallel for better performance
    const inactivationPromises = agentIds.map(async (_AGENT_ID) => {
      // Security: Validate agentId to prevent object injection
      if (!this.isValidAgentId(_AGENT_ID)) {
  ...
  isValidAgentId(_AGENT_ID) {
    ...
    if (!agentIdPattern.test(_AGENT_ID)) {
      return false;
    }
