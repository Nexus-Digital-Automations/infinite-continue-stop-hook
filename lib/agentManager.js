...
class FilesystemSecurity {
  ...
  static safeWriteFileSync(basePath, filePath, content, encoding = 'utf8') {
    const safePath = this.safeResolvePath(basePath, filePath);
    // eslint-disable-next-line security/detect-non-literal-fs-filename -- safePath is validated and sanitized
    _fs.writeFileSync(safePath, content, encoding);
  }
}
...
class AgentManager {
  ...
  async registerAgent(agentConfig) {
    ...
    _todoData.agents[agentId] = agent;
    await this.writeTodo(_todoData);
  ...
  async markAgentsAsInactive(agentIds) {
    ...
    const inactivationPromises = agentIds.map(async (agentId) => {
      // Security: Validate agentId to prevent object injection
      if (!this.isValidAgentId(agentId)) {
