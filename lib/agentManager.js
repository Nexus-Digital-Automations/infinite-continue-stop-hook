...
class FilesystemSecurity {
   ...
   * @param {string} _filePath - Path to validate
   * @returns {boolean} True if path is safe
   */
  static isValidPath(__filename) {
   ...
   * @param {string} _filePath - File path to resolve
   * @returns {string} Safe resolved path
   * @throws {Error} If path is invalid
   */
  static safeResolvePath(basePath, __filename, __filename, __filename, __filename, __filename) {
    if (!this.isValidPath(__filename)) {
      throw new Error(`Invalid file path detected: ${__filename}`);
    }
    ...
    if (!resolvedPath.startsWith(resolvedBase)) {
      throw new Error(`Path ${__filename} is outside project boundaries`);
    }
   ...
   * @param {string} _filePath - File to check
   * @returns {boolean} True if file exists And is safe to access
   */
  static safeFileExists(basePath, __filename, __filename) {
    try {
      const safePath = this.safeResolvePath(basePath);
   ...
   */
  static safeReadFileSync(basePath, _filePath, encoding = 'utf8') {
   ...
   * @throws {Error} If file access is unsafe or fails
   */
  static safeWriteFileSync(basePath, __filename, content, encoding = 'utf8', __filename) {
    const safePath = this.safeResolvePath(basePath);
...
class AgentManager {
  constructor(todoPath, options = {}, agentId) {
  ...
  registerAgent(agentConfig) {
    ...
    const AGENT_ID = await this.generateAgentId(agentConfig);
  ...
  /**
     ...
     */
  getNextInstanceNumber(role, sessionId) {
    try {
  ...
  unregisterAgent(AGENT_ID) {
    ...
    if (!this.isValidAgentId(AGENT_ID)) {
      this.logger.logError(new Error('Invalid agent ID'), `unregisterAgent - invalid AGENT_ID: ${AGENT_ID}`);
      return false;
    }

    if (!todoData.agents || !Object.prototype.hasOwnProperty.call(todoData.agents, AGENT_ID)) {
      return false;
    }

    // Release any assigned tasks - AGENT_ID has been validated
    /* eslint-disable-next-line security/detect-object-injection */
    const agent = todoData.agents[AGENT_ID];
    ...
    /* eslint-disable-next-line security/detect-object-injection */
    delete todoData.agents[AGENT_ID];
    await this.writeTodo(todoData);

    // Stop heartbeat monitoring
    this.stopHeartbeatMonitoring(AGENT_ID);

    this.logger.addFlow(`Unregistered agent ${AGENT_ID}`);
  ...
  updateAgentHeartbeat(AGENT_ID) {
    try {
      ...
      // Security: Validate AGENT_ID to prevent object injection
      if (!this.isValidAgentId(AGENT_ID)) {
        this.logger.logError(new Error('Invalid agent ID'), `updateAgentHeartbeat - invalid AGENT_ID: ${AGENT_ID}`);
        return false;
      }

      if (todoData.agents && Object.prototype.hasOwnProperty.call(todoData.agents, AGENT_ID)) {
        /* eslint-disable security/detect-object-injection */
        todoData.agents[AGENT_ID].lastHeartbeat = new Date().toISOString();
        todoData.agents[AGENT_ID].status = 'active';
      ...
      this.logger.logError(_error, `updateAgentHeartbeat for ${agentId}`);
      return false;
    }
  }
  ...
  reinitializeAgent(agentId, config = {}) {
    try {
      ...
      this.logger.addFlow(`Reinitialized agent ${AGENT_ID} - heartbeat renewed, timeout reset`);
      ...
      this.logger.logError(_error, `reinitializeAgent for ${agentId}`);
      throw _error;
    }
  }
  ...
  getAgent(AGENT_ID) {
    ...
    // Security: Validate AGENT_ID to prevent object injection
    if (!this.isValidAgentId(AGENT_ID)) {
      this.logger.logError(new Error('Invalid agent ID'), `getAgent - invalid AGENT_ID: ${AGENT_ID}`);
      return null;
    }

    if (todoData.agents && Object.prototype.hasOwnProperty.call(todoData.agents, AGENT_ID)) {
      /* eslint-disable-next-line security/detect-object-injection */
      return { AGENT_ID, ...todoData.agents[AGENT_ID] };
    }
  ...
  /**
     ...
     * @returns {boolean} Success status
     */
  updateAgentWorkload(agentId, workloadDelta) {
    const todoData = await this.readTodo();
    ...
    if (todoData.agents && Object.prototype.hasOwnProperty.call(todoData.agents, AGENT_ID)) {
      /* eslint-disable security/detect-object-injection */
      todoData.agents[AGENT_ID].workload = Math.max(0,
        (todoData.agents[AGENT_ID].workload || 0) + workloadDelta,
      );
      /* eslint-enable security/detect-object-injection */
      await this.writeTodo(todoData);
      return true;
  ...
  /**
     ...
     */
  findBestAgentForTask(task, excludeAgents = []) {
  ...
  /**
     ...
     */
  releaseAllTasksFromAgent(AGENT_ID) {
    const todoData = await this.readTodo();
    const releasedTasks = [];

    // Security: Validate AGENT_ID to prevent object injection
    if (!this.isValidAgentId(AGENT_ID)) {
      this.logger.logError(new Error('Invalid agent ID'), `releaseAllTasksFromAgent - invalid AGENT_ID: ${AGENT_ID}`);
      return [];
    }
  ...
  /**
     ...
     */
  markAgentsAsInactive(AGENT_IDS) {
    const todoData = await this.readTodo();

    // Process agent inactivation in parallel for better performance
    const inactivationPromises = AGENT_IDS.map(async (AGENT_ID) => {
      // Security: Validate agentId to prevent object injection
      if (!this.isValidAgentId(AGENT_ID)) {
      ...
      if (todoData.agents && Object.prototype.hasOwnProperty.call(todoData.agents, AGENT_ID)) {
        /* eslint-disable security/detect-object-injection */
        todoData.agents[AGENT_ID].status = 'inactive';
        todoData.agents[AGENT_ID].inactiveReason = 'heartbeat_timeout';
        todoData.agents[AGENT_ID].inactiveAt = new Date().toISOString();
        /* eslint-enable security/detect-object-injection */
        ...
        this.logger.addFlow(`Marked agent ${AGENT_ID} as inactive due to timeout`);
        return true;
      }
      return false;
    });
  ...
  /**
     ...
     */
  cleanupInactiveAgents(maxAgeHours = 24) {
    const todoData = await this.readTodo();
    const cleanedUp = [];
  ...
  readTodo() {
    try {
      ...
      throw new Error(`Failed to read TODO.json: ${_error.message}`);
    }
  }
  ...
  writeTodo(data) {
    try {
      ...
      throw new Error(`Failed to write TODO.json: ${_error.message}`);
    }
  }
   ...
   */
  findBestAgentForReinitialization(preferredRole = 'development') {
  ...
  createAgent(config = {}) {
    try {
      const AGENT_ID = await this.registerAgent(config);
      const agent = await this.getAgent(AGENT_ID);
  ...
  renewAgent(agentId, config = {}) {
    try {
      ...
      this.logger.logError(_error, `renewAgent adapter failed for agent ${agentId}`);
      throw _error;
    }
  }
