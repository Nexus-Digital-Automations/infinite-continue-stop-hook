...
class ContaminationResolver {
    constructor(projectRoot = process.cwd()) {
        ...
        this.criticalFiles = [
            ...
            'node_modules/istanbul-lib-coverage/lib/collection.js',
    ...
    async restoreContaminatedFiles() {
        // Set restoration context to allow writes
        ...
        return { restored: restored.length, files: restored };
            ...
            // Always clear restoration context
            this.isRestoring = false;
        }
    }
...
`;
    ...
    setupJestExitProtection() {
        ...
        if (isCoverageRun) {
            ...
            this.coverageCleanupInterval = coverageCleanupInterval;
            
            // Clean up interval when process exits - use once to avoid memory leaks
            const cleanup = () => {
                if (this.coverageCleanupInterval) {
                    clearInterval(this.coverageCleanupInterval);
                    this.coverageCleanupInterval = null;
                }
                global.jestDone = true;
            };
    ...
    setupProactiveWriteBlocking() {
        ...
        fs.writeFileSync = (filePath, data, options) => {
            ...
            if (this.isCriticalFile(resolvedPath)) {
                ...
                // Block all writes to node_modules JavaScript files during tests (unless we're restoring)
                if (this.isNodeModulesJSFile(resolvedPath) && !this.isRestoring) {
                    console.warn(`🛡️ BLOCKED: node_modules JavaScript file write prevented: ${filePath}`);
                    throw new Error(`CRITICAL: Complete test isolation - node_modules write blocked: ${filePath}`);
                }
            }
