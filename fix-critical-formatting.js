...
jsFiles.forEach((filePath) => {
  try {
    ...
    content = content.replace(/\bcatch\s*\(\s*([a-zA-Z_$][a-zA-Z0-9_$]*)\s*\)/g, 'catch (_$1)');

    // Fix 2: Fix undefined _error variables in catch blocks
    content = content.replace(/catch\s*\(\s*_\s*\)\s*{[^}]*_error[^}]*}/g, (match) => {
    ...
    content = content.replace(/\b(const|let|var)\s+([a-zA-Z_$][a-zA-Z0-9_$]*)\s*=/g, (match, keyword, varName) => {
      if (varName === 'RESULT' || varName === 'OUTPUT' || varName === 'LINT_OUTPUT' || varName === 'LINT_RESULT') {
    ...
    content = content.replace(/function\s+[a-zA-Z_$][a-zA-Z0-9_$]*\s*\(\s*([a-zA-Z_$][a-zA-Z0-9_$]*)\s*\)/g, (match, param) => {
