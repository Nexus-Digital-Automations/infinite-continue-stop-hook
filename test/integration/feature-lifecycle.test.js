...
describe('Feature Lifecycle Integration Tests', () => {
  ...
  describe('Feature Suggestion Workflow', () => {
    test('should handle complete feature suggestion process', async () => {
      ...
      const listResult = await execAPI('list-features', [], { projectRoot: testDir });
      expect(listResult.success).toBe(true);
      expect(listResult.features).toHaveLength(6);
      expect(listResult.features.every(f => f.status === 'suggested')).toBe(true);
      ...
      // 5. Verify statistics
      const statsResult = await execAPI('feature-stats', [], { projectRoot: testDir });
    ...
    test('should validate feature data requirements', async () => {
      ...
      const incompleteFeatures = [
        { description: 'Missing title', business_value: 'Test', category: 'enhancement' },
        { title: 'Missing description', business_value: 'Test', category: 'enhancement' },
        { title: 'Missing business value', description: 'Test', category: 'enhancement' },
        { title: 'Missing category', description: 'Test', business_value: 'Test' },
      ];

      for (const incompleteFeature of incompleteFeatures) {
        const result = await execAPI('suggest-feature', [JSON.stringify(incompleteFeature)], {
          projectRoot: testDir,
        });

        expect(result.success).toBe(false);
        expect(result.error).toBeDefined();
        expect(result.error).toContain('Required field') || expect(result.error).toContain('missing');
      }
      ...
      const invalidResult = await execAPI('suggest-feature', [JSON.stringify(invalidCategoryFeature)], {
        projectRoot: testDir,
      });

      expect(invalidResult.success).toBe(false);
      expect(invalidResult.error).toContain('Invalid category') || expect(invalidResult.error).toContain('category');
      ...
      const shortResult = await execAPI('suggest-feature', [JSON.stringify(tooShortFeature)], {
        projectRoot: testDir,
      });
      ...
      const longResult = await execAPI('suggest-feature', [JSON.stringify(tooLongFeature)], {
        projectRoot: testDir,
      });
    ...
    test('should handle concurrent feature suggestions', async () => {
      ...
      const finalListResult = await execAPI('list-features', [], { projectRoot: testDir });
  ...
  describe('Feature Approval Workflow', () => {
    test('should handle individual feature approval process', async () => {
      ...
      const testFeatures = [
        generateTestFeature({ title: 'High Priority Feature', category: 'enhancement' }),
        generateTestFeature({ title: 'Critical Bug Fix', category: 'bug-fix' }),
        generateTestFeature({ title: 'Security Enhancement', category: 'security' }),
      ];
      ...
      const approvalData = [
        { approved_by: 'product-manager', notes: 'High business value, approved for next sprint' },
        { approved_by: 'technical-lead', notes: 'Critical issue affecting users' },
        { approved_by: 'security-team', notes: 'Essential security improvement' },
      ];

      for (let i = 0; i < featureIds.length; i++) {
        const approveResult = await execAPI('approve-feature', [
          featureIds[i],
          JSON.stringify(approvalData[i]),
        ], { projectRoot: testDir });

        expect(approveResult.success).toBe(true);
        expect(approveResult.feature.status).toBe('approved');
        expect(approveResult.feature.approved_by).toBe(approvalData[i].approved_by);
        expect(approveResult.feature.approval_notes).toBe(approvalData[i].notes);
      ...
      expect(approvedResult.features.every(f => f.status === 'approved')).toBe(true);
    ...
    test('should handle bulk feature approval process', async () => {
      ...
      const bulkResult = await execAPI('bulk-approve-features', [
        JSON.stringify(bulkIds),
        JSON.stringify(bulkApprovalData),
      ], { projectRoot: testDir });
      ...
      listApprovedResult.features.forEach(feature => {
        ...
        expect(feature.approval_notes).toBe('Batch approved for Sprint 2024-Q1');
      });
      ...
      for (const featureId of featureIds.slice(10)) {
        const approveResult = await execAPI('approve-feature', [
          featureId,
          JSON.stringify({ approved_by: 'individual-approver' }),
        ], { projectRoot: testDir });
    ...
    test('should prevent invalid approval transitions', async () => {
      ...
      const approveResult = await execAPI('approve-feature', [suggestResult.feature.id], {
        projectRoot: testDir,
      });
      expect(approveResult.success).toBe(true);

      // 2. Try to approve already approved feature
      const secondApproveResult = await execAPI('approve-feature', [suggestResult.feature.id], {
        projectRoot: testDir,
      });
      ...
      const rejectResult = await execAPI('reject-feature', [suggestResult.feature.id], {
        projectRoot: testDir,
      });
  ...
  describe('Feature Rejection Workflow', () => {
    test('should handle feature rejection process', async () => {
      ...
      const testFeatures = [
        generateTestFeature({ title: 'Low Priority Feature', category: 'enhancement' }),
        generateTestFeature({ title: 'Out of Scope Feature', category: 'new-feature' }),
        generateTestFeature({ title: 'Duplicate Feature', category: 'bug-fix' }),
      ];
      ...
      const rejectionReasons = [
        { rejected_by: 'product-manager', reason: 'Low business value, not aligned with roadmap' },
        { rejected_by: 'architect', reason: 'Out of current project scope' },
        { rejected_by: 'team-lead', reason: 'Duplicate of existing feature #123' },
      ];

      for (let i = 0; i < featureIds.length; i++) {
        const rejectResult = await execAPI('reject-feature', [
          featureIds[i],
          JSON.stringify(rejectionReasons[i]),
        ], { projectRoot: testDir });

        expect(rejectResult.success).toBe(true);
        expect(rejectResult.feature.status).toBe('rejected');
        expect(rejectResult.feature.rejected_by).toBe(rejectionReasons[i].rejected_by);
        expect(rejectResult.feature.rejection_reason).toBe(rejectionReasons[i].reason);
      ...
      expect(rejectedResult.features.every(f => f.status === 'rejected')).toBe(true);
    ...
    test('should prevent operations on rejected features', async () => {
      ...
      const secondRejectResult = await execAPI('reject-feature', [suggestResult.feature.id], {
        projectRoot: testDir,
      });
  ...
  describe('Feature Filtering and Search', () => {
    beforeEach(async () => {
      ...
      const testFeatures = [
        { title: 'Authentication System', category: 'enhancement', status: 'suggested' },
        { title: 'Login Bug Fix', category: 'bug-fix', status: 'suggested' },
        { title: 'Performance Optimization', category: 'performance', status: 'suggested' },
        { title: 'Security Audit', category: 'security', status: 'suggested' },
        { title: 'Documentation Update', category: 'documentation', status: 'suggested' },
        { title: 'New Dashboard', category: 'new-feature', status: 'suggested' },
      ];

      for (const feature of testFeatures) {
        const featureData = generateTestFeature(feature);
        await execAPI('suggest-feature', [JSON.stringify(featureData)], { projectRoot: testDir });
      }
      ...
      for (let i = 0; i < 3; i++) {
        await execAPI('approve-feature', [features[i].id], { projectRoot: testDir });
      }

      // Reject next 2 features
      for (let i = 3; i < 5; i++) {
        await execAPI('reject-feature', [
          features[i].id,
          JSON.stringify({ reason: 'Test rejection' }),
        ], { projectRoot: testDir });
      }
    });
    ...
    test('should filter features by status', async () => {
      ...
      expect(suggestedResult.features.every(f => f.status === 'suggested')).toBe(true);
    ...
    test('should filter features by category', async () => {
      const categories = ['enhancement', 'bug-fix', 'performance', 'security', 'documentation', 'new-feature'];
  ...
  describe('Complete Lifecycle Scenarios', () => {
    test('should handle realistic product development workflow', async () => {
      ...
      const sprintFeatures = [
        generateTestFeature({ title: 'User Profile Page', category: 'new-feature' }),
        generateTestFeature({ title: 'Fix Login Validation', category: 'bug-fix' }),
        generateTestFeature({ title: 'Optimize Database Queries', category: 'performance' }),
        generateTestFeature({ title: 'Add Input Sanitization', category: 'security' }),
        generateTestFeature({ title: 'Update API Documentation', category: 'documentation' }),
        generateTestFeature({ title: 'Improve Button Styling', category: 'enhancement' }),
      ];
      ...
      const bulkApproveResult = await execAPI('bulk-approve-features', [
        JSON.stringify(highPriorityIds),
        JSON.stringify({ approved_by: 'product-team', notes: 'Sprint 2024-Q1 approved' }),
      ], { projectRoot: testDir });
      ...
      for (const featureId of featureIds.slice(4)) {
        const rejectResult = await execAPI('reject-feature', [
          featureId,
          JSON.stringify({ rejected_by: 'product-team', reason: 'Deferred to next sprint' }),
        ], { projectRoot: testDir });
      ...
      const approvalEntries = featuresData.metadata.approval_history.filter(h => h.action === 'approved');
      expect(approvalEntries).toHaveLength(4);
      expect(approvalEntries.every(e => e.approved_by === 'product-team')).toBe(true);

      // Check that low priority features have rejection entries
      const rejectionEntries = featuresData.metadata.approval_history.filter(h => h.action === 'rejected');
      expect(rejectionEntries).toHaveLength(2);
      expect(rejectionEntries.every(e => e.rejected_by === 'product-team')).toBe(true);
    });
    ...
    test('should maintain data integrity throughout complex workflows', async () => {
      ...
      for (let i = 0; i < featureIds.length; i++) {
        if (i % 3 === 0) {
          ...
          await execAPI('reject-feature', [
            featureIds[i],
            JSON.stringify({ rejected_by: 'complex-workflow-test', reason: 'Complex test rejection' }),
          ], { projectRoot: testDir });
        }
      ...
      const finalStatsResult = await execAPI('feature-stats', [], { projectRoot: testDir });
      ...
      const approvalHistoryCount = featuresData.metadata.approval_history.length;
