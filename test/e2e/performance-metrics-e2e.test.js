...
describe('Performance Metrics System E2E Tests', () => {
  ...
  const mockMetricsFile = path.join(mockProjectRoot, '.validation-performance-enhanced.json');
  ...
  function simulateValidationExecutions() {
    ...
    for (let week = 0; week < 3; week++) {
      for (let day = 0; day < 7; day++) {
        for (let validation = 0; validation < 3; validation++) {
          const timestamp = now - ((week * 7 + day) * 24 * 60 * 60 * 1000) + (validation * 2 * 60 * 60 * 1000);
          ...
          criteria.forEach(criterion => {
            ...
            const degradationFactor = criterion === 'build-validation' ? (1 + (week * 0.15)) : 1;
            ...
            const timeOfDayFactor = validation === 0 ? 1.2 : (validation === 1 ? 1.0 : 0.9);

            // Random variation
            const randomFactor = 0.8 + (Math.random() * 0.4);

            const duration = Math.round(baseDuration * degradationFactor * dayOfWeekFactor * timeOfDayFactor * randomFactor);

            // Simulate occasional anomalies
            const isAnomaly = Math.random() > 0.97;
            const anomalyDuration = isAnomaly ? duration * (2 + Math.random() * 3) : duration;
    ...
    metricsData.metrics.sort((a, b) => new Date(b.timing.startTime) - new Date(a.timing.startTime));
  ...
  describe('Complete Performance Metrics Workflow', () => {
    test('should handle full workflow from empty state to comprehensive analysis', () => {
      ...
      let result = executeTaskManagerCommand('get-validation-performance-metrics');
      ...
      result = executeTaskManagerCommand('analyze-performance-trends', '\'{"timeRange":21}\'');
      expect(result.success).toBe(true);
    ...
    test('should maintain data consistency across all endpoints', () => {
      ...
      const basicMetrics = executeTaskManagerCommand('get-validation-performance-metrics');
    ...
    test('should handle filtering and time ranges consistently', () => {
      ...
      expect(linterMetrics.metrics.every(m => m.criterion === 'linter-validation')).toBe(true);
      ...
      expect(recentMetrics.metrics.length).toBeLessThan(linterMetrics.metrics.length);
    ...
    test('should handle large datasets efficiently', () => {
      ...
      for (let day = 0; day < 60; day++) {
        for (let exec = 0; exec < 5; exec++) {
          const timestamp = now - (day * 24 * 60 * 60 * 1000) + (exec * 4 * 60 * 60 * 1000);
      ...
      fs.writeFileSync(mockMetricsFile, JSON.stringify(largeMetricsData, null, 2));

      // Test that analysis completes within reasonable time
      const startTime = Date.now();
      const result = executeTaskManagerCommand('analyze-performance-trends', '\'{"timeRange":60}\'');
  ...
  describe('Data Quality and Validation', () => {
    test('should handle mixed data quality gracefully', () => {
      ...
      fs.writeFileSync(mockMetricsFile, JSON.stringify(mixedQualityData, null, 2));

      // System should handle gracefully and process valid data
      const result = executeTaskManagerCommand('get-validation-performance-metrics');
      expect(result.success).toBe(true);
  ...
  describe('Integration with External Systems', () => {
    test('should maintain compatibility with legacy metrics format', () => {
      ...
      const legacyFile = path.join(mockProjectRoot, '.validation-performance.json');
    ...
    test('should handle version migration scenarios', () => {
      ...
      fs.writeFileSync(path.join(mockProjectRoot, '.validation-performance.json'), JSON.stringify(legacyData, null, 2));
