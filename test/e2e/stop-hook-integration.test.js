...
describe.skip('Stop Hook Integration E2E', () => {
  ...
  describe('Basic Stop Hook Functionality', () => {
    test('Agent stop authorization workflow', async () => {
    ...
    test('Stop hook with feature operations integration', async () => {
      ...
      const featureResult = await FeatureTestHelpers.suggestFeature(environment, {
      ...
      const featureId = featureResult.result.stdout.match(/Feature ID: (\w+)/)[1];
      ...
      console.log(`✅ Stop hook with feature operations integration test passed for agent: ${agentId}`);
    }, E2E_TIMEOUT);
  });

  describe('Infinite Continue Mode', () => {
    test('Infinite continue loop with controlled termination', async () => {
    ...
    test('Multiple concurrent infinite continue streams', async () => {
      ...
      streamResults.forEach((stream, streamIndex) => {
        ...
        console.log(`Stream ${streamIndex}: ${stream.length} iterations completed`);
      });

      const totalIterations = streamResults.reduce((sum, stream) => sum + stream.length, 0);
      expect(totalIterations).toBeGreaterThan(0);

      console.log(`✅ Concurrent infinite continue streams test passed: ${totalIterations} total iterations across ${streamCount} streams`);
    }, E2E_TIMEOUT);
  });

  describe('Stop Hook Conditions and Logic', () => {
    test('Conditional stop authorization based on system state', async () => {
      ...
      const pendingTasksResult = await FeatureTestHelpers.suggestFeature(environment, {
      ...
      console.log(`✅ Conditional stop authorization test completed for agent: ${agentId}`);
    }, E2E_TIMEOUT);

    test('Stop hook error recovery scenarios', async () => {
      ...
      console.log(`✅ Stop hook error recovery test passed for agent: ${agentId}`);
    }, E2E_TIMEOUT);
  });

  describe('Advanced Stop Hook Integration', () => {
    test('Stop hook with feature lifecycle integration', async () => {
      ...
      const initialFeature = await FeatureTestHelpers.suggestFeature(environment, {
      ...
      console.log(`✅ Stop hook lifecycle integration test passed for feature: ${featureId}`);
    }, E2E_TIMEOUT);
    ...
    test('Stop hook performance under load', async () => {
      ...
      console.log(`✅ Stop hook performance test passed: ${successfulRequests}/${requestCount} requests in ${duration}ms (avg: ${averageResponseTime}ms)`);
    }, E2E_TIMEOUT);

    test('Stop hook system recovery after failure', async () => {
      ...
      const preFailureFeature = await FeatureTestHelpers.suggestFeature(environment, {
      ...
      const recoveryFeature = await FeatureTestHelpers.suggestFeature(environment, {
      ...
      expect(features.features.find(f => f.id === preFailureId)).toBeTruthy();
      expect(features.features.find(f => f.id === postRecoveryId)).toBeTruthy();

      console.log(`✅ Stop hook system recovery test passed for agent: ${agentId}`);
    }, E2E_TIMEOUT * 2); // Extended timeout for recovery testing
  });
});
