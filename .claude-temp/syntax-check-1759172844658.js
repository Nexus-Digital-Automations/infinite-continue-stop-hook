...
class AgentManager {
  ...
  async registerAgent(agentConfig) {
    ...
    const AGENT_ID = this.generateAgentId(agentConfig);
    ...
    const agent = {
      ...
      workload: 0,
      maxConcurrentTasks:
        agentConfig.maxConcurrentTasks || this.options.maxConcurrentTasks,
  ...
  async generateAgentId(agentConfig) {
    ...
    const instance = this.getNextInstanceNumber(role, sessionId);
  ...
  async getNextInstanceNumber(role, sessionId) {
    try {
      ...
      const existingAgents = Object.keys(todoData.agents).filter((agentId) =>
        agentId.startsWith(`${role}_${sessionId}`)
      );
  ...
  getCapabilitiesForRole(role) {
    ...
    if (!role || typeof role !== 'string') {
      this.logger.addFlow(
        `Invalid role provided: ${role}, using development default`
      );
  ...
  async unregisterAgent(AGENT_ID) {
    ...
    if (!this.isValidAgentId(AGENT_ID)) {
      this.logger.logError(
        ...
        `unregisterAgent - invalid AGENT_ID: ${AGENT_ID}`
      );
      return false;
    }
    ...
    if (
      !todoData.agents ||
      !Object.prototype.hasOwnProperty.call(todoData.agents, AGENT_ID)
    ) {
      return false;
    }
  ...
  async updateAgentHeartbeat(AGENT_ID) {
    try {
      ...
      if (!this.isValidAgentId(AGENT_ID)) {
        this.logger.logError(
          new Error('Invalid agent ID'),
          `updateAgentHeartbeat - invalid AGENT_ID: ${AGENT_ID}`
        );
        return false;
      }
      ...
      if (
        ...
        this.agentHeartbeats.set(AGENT_ID, Date.now());
      ...
      this.logger.logError(error, `updateAgentHeartbeat for ${AGENT_ID}`);
      return false;
    }
  }
  ...
  async reinitializeAgent(agentId, config = {}) {
    try {
      ...
      if (
        !todoData.agents ||
        !Object.prototype.hasOwnProperty.call(todoData.agents, _agentId)
      ) {
      ...
      Object.assign(agent, {
        ...
        metadata: {
          ...
          renewalReason:
            config.renewalReason || 'Agent reinitialization requested',
        },
      });
      ...
      this.logger.addFlow(
        `Reinitialized agent ${AGENT_ID} - heartbeat renewed, timeout reset`
      );
  ...
  async getAgent(AGENT_ID) {
    ...
    if (!this.isValidAgentId(AGENT_ID)) {
      this.logger.logError(
        new Error('Invalid agent ID'),
        `getAgent - invalid AGENT_ID: ${AGENT_ID}`
      );
      return null;
    }
  ...
  async updateAgentWorkload(agentId, workloadDelta) {
    ...
    if (!this.isValidAgentId(AGENT_ID)) {
      this.logger.logError(
        new Error('Invalid agent ID'),
        `updateAgentWorkload - invalid agentId: ${agentId}`
      );
      return false;
    }
    ...
    if (
      ...
      /* eslint-disable security/detect-object-injection */
      todoData.agents[AGENT_ID].workload = Math.max(
        0,
  ...
  async releaseAllTasksFromAgent(AGENT_ID) {
    ...
    if (!this.isValidAgentId(AGENT_ID)) {
      this.logger.logError(
        new Error('Invalid agent ID'),
        `releaseAllTasksFromAgent - invalid AGENT_ID: ${AGENT_ID}`
      );
      return [];
    }
    ...
    this.logger.addFlow(
      `Released ${releasedTasks.length} tasks from agent ${agentId}`
    );
  ...
  async markAgentsAsInactive(AGENT_IDS) {
    ...
    const inactivationPromises = AGENT_IDS.map(async (AGENT_ID) => {
      ...
      if (!this.isValidAgentId(AGENT_ID)) {
        this.logger.logError(
          new Error('Invalid agent ID'),
          `markAgentsAsInactive - invalid agentId: ${agentId}`
        );
        return false;
      }
      ...
      if (
        ...
        this.logger.addFlow(
          `Marked agent ${AGENT_ID} as inactive due to timeout`
        );
        return true;
      }
      return false;
  ...
  async getAgentStatistics() {
    ...
    agents.forEach((agent) => {
      ...
      const role =
        typeof agent.role === 'string' && agent.role ? agent.role : 'unknown';
      /* eslint-disable-next-line security/detect-object-injection */
      ...
      if (agent.status === 'inactive' && agent.inactiveAt) {
        ...
        if (inactiveTime < cutoffTime) {
          ...
          if (!this.isValidAgentId(AGENT_ID)) {
            this.logger.logError(
              new Error('Invalid agent ID'),
              `cleanupInactiveAgents - invalid agentId: ${agentId}`
            );
            continue;
          }
  ...
  async readTodo() {
    try {
      ...
      const content = FilesystemSecurity.safeReadFileSync(
        projectRoot,
        filename,
        'utf8'
      );
  ...
  async writeTodo(data) {
    try {
      ...
      const projectRoot = path.dirname(this.todoPath);
      const filename = path.basename(this.todoPath);
      FilesystemSecurity.safeWriteFileSync(
        projectRoot,
        filename,
  ...
  isValidAgentId(AGENT_ID) {
    ...
    if (AGENT_ID.length < 10 || AGENT_ID.length > 200) {
      return false;
    }

    // Check format: role_sessionId_instance_specialization_hash;
    const AGENT_ID_PATTERN =
      /^[a-zA-Z_]+_session_\d+_\d+_[a-zA-Z0-9_]+_[a-f0-9]{8}$/;
    ...
    const lowerAgentId = AGENT_ID.toLowerCase();
    for (const pattern of suspiciousPatterns) {
      if (lowerAgentId.includes(pattern)) {
        this.logger.logError(
          new Error('Suspicious agent ID pattern detected'),
          `agentId: ${AGENT_ID}, pattern: ${pattern}`
        );
        return false;
      }
    }
  ...
  async getAllActiveAgents() {
    ...
    return activeAgents.sort(
      (a, b) => a.timeSinceLastHeartbeat - b.timeSinceLastHeartbeat
    );
  }
  ...
  async findBestAgentForReinitialization(preferredRole = 'development') {
    ...
    const roleMatches = activeAgents.filter(
      (a) => a.agent.role === preferredRole
    );
  ...
  async cleanupStaleAgents() {
    ...
    Object.entries(todoData.agents).forEach(([agentId, agent]) => {
      ...
      if (timeSinceLastHeartbeat > this.options.agentTimeout) {
        ...
        this.logger.addFlow(
          `Cleaning up stale agent ${agentId} (inactive for ${Math.round(timeSinceLastHeartbeat / 1000 / 60)} minutes)`
        );
      } else {
  ...
  async renewAgent(agentId, config = {}) {
    try {
      ...
      this.logger.logError(
        error,
        `renewAgent adapter failed for agent ${agentId}`
      );
      throw error;
    }
  }
