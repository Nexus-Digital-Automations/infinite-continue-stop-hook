...
class SecurityManager {
  constructor(options = {}, _agentId) {
    ...
    this.validator = new _SecurityValidator(
      this.logger,
      this.config.validatorOptions
    );
    this.middleware = new _SecurityMiddleware(
      this.logger,
      this.config.middlewareOptions
    );
  ...
  async initializeSecuritySystem(taskManagerInstance, expressApp = null) {
    try {
      ...
      switch (this.config.integrationMode) {
        ...
        default:
          throw new Error(
            `Unknown integration mode: ${this.config.integrationMode}`
          );
      }
  ...
  async fullSecurityIntegration() {
    ...
    if (this.expressApp && this.config.enableRateLimiting) {
      this.expressApp.use(
        '/api',
        this.middleware.createSecurityMiddleware({
  ...
  integrateWithTaskManager() {
    ...
    if (this.taskManager.createTask) {
      originalMethods.createTask = this.taskManager.createTask.bind(
        this.taskManager
      );
      this.taskManager.createTask = (taskData, agentId = null) => {
        return this.secureTaskOperation(
          'create',
          originalMethods.createTask,
          taskData,
          AGENT_ID
        );
      };
    ...
    if (this.taskManager.updateTask) {
      originalMethods.updateTask = this.taskManager.updateTask.bind(
        this.taskManager
      );
      this.taskManager.updateTask = (taskId, updateData, AGENT_ID = null) => {
        return this.secureTaskOperation(
          'update',
          originalMethods.updateTask,
          updateData,
          AGENT_ID,
          taskId
        );
    ...
    if (this.taskManager.completeTask) {
      originalMethods.completeTask = this.taskManager.completeTask.bind(
        this.taskManager
      );
      this.taskManager.completeTask = (taskId, completionData, AGENT_ID) => {
        return this.secureTaskOperation(
          'complete',
          originalMethods.completeTask,
          completionData,
          AGENT_ID,
          taskId
        );
    ...
    if (this.taskManager.claimTask) {
      originalMethods.claimTask = this.taskManager.claimTask.bind(
        this.taskManager
      );
    ...
    try {
      ...
      if (this.config.enableAuthorization && agentId) {
        ...
        if (!authResult.authorized) {
          const error = new Error(
            `Authorization failed for ${operation}: ${authResult.error}`
          );
      ...
      if (this.config.enableInputValidation && data) {
        ...
        if (!validationResult.valid) {
          const error = new Error(
            `Input validation failed for ${operation}: ${validationResult.error}`
          );
    ...
    try {
      const metrics = {
        ...
        middleware: this.middleware
          ? this.middleware.getSecurityMetrics()
          : null,
      };
