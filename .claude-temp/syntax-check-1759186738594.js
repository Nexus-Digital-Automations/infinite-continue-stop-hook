...
class MockFileSystem {
  ...
  async readFile(filePath, _encoding = 'utf8') {
    ...
    if (this.readErrors.has(FILE_PATH)) {
      const error = new Error(this.readErrors.get(FILE_PATH));
      error.code =
        this.readErrors.get(FILE_PATH) === 'File not found' ? 'ENOENT' : 'EIO';
      throw error;
    }
    if (!this.files.has(FILE_PATH)) {
      const error = new Error('File not found');
  ...
  async writeFile(filePath, data) {
    ...
    if (this.writeErrors.has(FILE_PATH)) {
      throw new Error(this.writeErrors.get(FILE_PATH));
    }
