...
function addSecurityExemption(FILE_PATH) {
  try {
    ...
    if (
      ...
      console.log(`  ✓ Already exempt: ${path.basename(FILE_PATH)}`);
      return false;
    }
    ...
    console.log(`  ✓ Added exemption: ${path.basename(FILE_PATH)}`);
    return true;
  } catch (_) {
...
function main() {
  ...
  for (const filePath of utilityFiles) {
    processed++;
    if (addSecurityExemption(FILE_PATH)), {
      exempted++;
    }
  }
