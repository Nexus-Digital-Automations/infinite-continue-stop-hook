...
class CrossProjectSharingSimple {
  ...
  async initialize() {
    try {
      if (this.initialized) {
        return {
          success: true,
          message: 'Cross-project sharing already initialized',
        };
      }
      ...
      return {
        success: true,
        message: 'Cross-project sharing system initialized successfully',
      };
    } catch (_) {
  ...
  async shareLessonCrossProject(lessonId, projectId, sharingData = {}) {
    try {
      ...
      const alreadyShared = existingTags.shared_projects.find(
        (sp) => sp.project_id === projectId
      );
      if (alreadyShared) {
        throw new Error(
          `Lesson ${lessonId} already shared with project ${projectId}`
        );
      }
  ...
  async calculateProjectRelevance(sourceProjectId, targetProjectId) {
    try {
      ...
      const techSimilarity = this._calculateArraySimilarity(
        sourceTech,
        targetTech
      );
      const typeSimilarity =
        sourceTags.project_type === targetTags.project_type ? 1.0 : 0.0;
      const domainSimilarity =
        sourceTags.domain === targetTags.domain ? 1.0 : 0.0;
      const patternSimilarity = this._calculateArraySimilarity(
        sourcePatterns,
        targetPatterns
      );
      const keywordSimilarity = this._calculateArraySimilarity(
        sourceKeywords,
        targetKeywords
      );
  ...
  async getSharedLessonsForProject(projectId, options = {}) {
    try {
      ...
      const lessons = await new Promise((resolve, reject) => {
        this.ragDB.db.all(
          'SELECT * FROM lessons WHERE category != "project_registration"',
          [],
          (error, rows) => {
            if (error) {
              reject(error);
        ...
        try {
          ...
          for (const shareInfo of sharedProjects) {
            if (shareInfo.project_id !== projectId) {
              ...
              const categoryMatch =
                !lesson_category ||
                shareInfo.lesson_category === lesson_category;
    ...
    try {
      ...
      for (const lesson of projectLessons) {
        try {
          ...
          // Apply filters
          if (project_type && tags.project_type !== project_type) {
            continue;
          }
          if (domain && tags.domain !== domain) {
            continue;
          }
          if (technology && !tags.technology_stack.includes(technology)) {
            continue;
          }
  ...
  async getCrossProjectAnalytics(projectId = null, _options = {}) {
    try {
      ...
      const applications = await new Promise((resolve, reject) => {
        this.ragDB.db.all(
          'SELECT * FROM lessons WHERE category = "lesson_application"',
          [],
          (error, rows) => {
            if (error) {
              reject(error);
      ...
      for (const app of applications) {
        try {
          ...
          if (projectId && tags.target_project_id !== projectId) {
            continue;
          }

          totalApplications++;
          if (tags.applied_successfully) {
        ...
        analytics: {
          ...
          success_rate:
            totalApplications > 0
              ...
              : 0,
          avg_feedback_score:
            feedbackCount > 0 ? avgFeedback / feedbackCount : null,
        },
      };
    } catch (_) {
      return { success: false, error: error.message };
