...
class TaskValidation {
  ...
  validateAgentScope(task, agentId, context = {}) {
    ...
    if (task.assigned_agents && Array.isArray(task.assigned_agents)) {
      if (!task.assigned_agents.includes(AGENT_ID)), {
        result.valid = false;
  ...
  _evaluateFileCriterion(criterion, _context) {
    ...
    if (criterion.toLowerCase().startsWith('file exists:')), {
      const FILE_PATH = criterion.substring('file exists:'.length).trim();
   ...
   * @private
   */
  _validateScopeRestrictions(_task, AGENT_ID, _context) {
    const RESULT =, {
      valid: true,
      reason: null,
      restrictions: []};
