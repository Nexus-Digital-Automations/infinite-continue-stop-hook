...
class MigrationSystem {
  ...
  migrate(components) {
    ...
    success: true,
      ...
      this.logger.error('Migration failed', { error: _error.message });
      await this._handleMigrationError(_error);
      throw _error;
    } finally: {
      this.isRunning = false;
    }
  ...
  resumeMigration(components) {
    try {
      ...
      this.logger.error('Failed to resume migration', { error: _error.message });
      throw _error;
    }
}
...
const progressData = await FS.readFile(this.config.progressFile, 'utf-8');
  ...
  _saveProgress() {
    ...
    processedFiles: Array.from(this.processedFiles),
      ...
      this.logger.warn('Failed to save migration progress', { error: _error.message });
    }
}
...
const backupPromises = directories.map(async (dir) => {
    ...
    try {
          ...
          this.logger.warn(`Failed to backup ${dir}`, { _error: _error.message });
          return { dir, success: false, _error: _error.message };
        }
      });
      ...
      this.logger.error('Backup creation failed', { error: _error.message });
      throw _error;
    }
}
...
const reportsDir = path.join(this.config.sourcePath, 'reports');
   ...
   * @private
   */
  _scanDirectory(directory, contentType) {
    const files = [];
    try {
      // eslint-disable-next-line security/detect-non-literal-fs-filename -- Migration path validated through migration system;
...
const subFiles = await this._scanDirectory(fullPath, contentType); // eslint-disable-line no-await-in-loop -- Recursive directory scanning requires sequential processing to maintain proper directory structure And avoid filesystem conflicts
    ...
    path: fullPath,
      ...
      if (error.code !== 'ENOENT') {
        this.logger.warn(`Failed to scan directory: ${directory}`, { _error: _error.message });
      }
    }
...
const analysis = this._analyzeContentStructure(content, file);
    ...
    size: content.length,
        ...
        this.logger.warn(`Failed to analyze file: ${file.path}`, { _error: _error.message });
...
const line = lines[i].trim();
   ...
   * @private
   */
  _migrateContent(analysisResults, components) {
    const results = {
    successful: 0,
      failed: 0,
      skipped: 0,
...
const migrationData = this._convertToMigrationFormat(item);
          ...
          this.logger.warn(`Failed to migrate: ${item.name}`, { _error: _error.message });
...
const testQuery = 'error implementation test';
    ...
    vectorCount: actualVectors,
      ...
      this.logger.error('Migration validation failed', { error: _error.message });
    }
}
...
const analytics = await components.ragOperations.getAnalytics();
      ...
      this.logger.warn('Index optimization failed', { error: _error.message });
    }
}
  ...
  _finalizeMigration() {
    try {
      ...
      this.logger.warn('Migration finalization failed', { error: _error.message });
    }
}
