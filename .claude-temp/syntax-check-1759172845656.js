...
class TaskManager {
  ...
  constructor(todoPath, options = {}, _agentId) {
    ...
    this.logger?.logInfo?.(
      'Modular TaskManager system initialized successfully'
    );
  }
  ...
  get lockManager() {
    if (!this._lockManager && this.options.enableMultiAgent) {
      this._lockManager = new DISTRIBUTED_LOCK_MANAGER(
        this._lockManagerOptions
      );
    }
  ...
  /**
   * Get tasks by task.category
   * @param {string} task.category - Task task.category
   * @returns {Array} Tasks in task.category
   */
  async getTasksByCategory(_category) {
    const todoData = await this.readTodo();
  ...
  async claimTask(taskId, agentId, priority = 'normal', options = {}) {
    const startTime = Date.now();
    this.logger?.logInfo?.(
      `[claimTask] Starting task claim: ${taskId} by agent ${agentId}`
    );
    ...
    if (
      ...
      this.logger?.logWarning?.(
        `[claimTask] Task not available for claiming: ${taskId} (status: ${task.status})`
      );
      return {
        success: false,
    ...
    if (!lockResult.success) {
      this.logger?.logError?.(
        `[claimTask] Failed to acquire lock for task ${taskId}:`,
        lockResult.error
      );
      return {
        success: false,
    ...
    try {
      ...
      this.logger?.logInfo?.(
        `[claimTask] Task claimed successfully: ${taskId} by agent ${agentId}`
      );
      ...
      await this.lockManager.releaseLock(this.todoPath);
    }
  }
  ...
  _validateTaskClaiming(taskId, todoData, options = {}, agentId = null) {
    ...
    if (agentId && task && task.category === 'audit') {
      ...
      if (
        ...
        task.audit_metadata?.original_implementer === agentId
      ) {
        return {
          valid: false,
          errorResult: {
            ...
            message:
              'Audit tasks must be performed by different agents to ensure objectivity',
            original_implementer:
              task.original_implementer ||
              task.audit_metadata?.original_implementer,
            current_agent: agentId,
            suggestion:
              'This audit task should be assigned to a different agent who did not implement the original feature',
          },
        };
      }
    }
  ...
  async listTasks(filter = {}) {
    ...
    if (filter.assigned_agent) {
      tasks = tasks.filter(
        (task) => task.assigned_agent === filter.assigned_agent
      );
    }
    ...
    // Record performance metrics;
    const duration = Date.now() - startTime;
    this.performance.recordQueryTime('listTasks', duration, {
  ...
  async getTask(taskId) {
    ...
    const task = index
      ? index.byId.get(taskId)
      : todoData.tasks?.find((t) => t.id === taskId);
  ...
  async updateTask(taskId, updates) {
    ...
    const validationResult = this.validation.validateTask(updatedTask, {
      todoData,
    });
    if (!validationResult.isValid) {
      throw new Error(
  ...
  async completeTask(taskId, completionData = {}) {
    ...
    const validationResult = this.validation.validateCompletionData(
      completionData,
      task
    );
    if (!validationResult.isValid) {
      throw new Error(
        `Completion data validation failed: ${validationResult.errors.join(', ')}`
      );
    }
    ...
    if (this.options.enableArchiving) {
      try {
        ...
        this.logger?.logWarning?.(
          `Failed to archive task ${taskId}: ${_error.message}`
        );
      }
    }
  ...
  async autoFix(filePath) {
    ...
    const RESULT = await this.autoFixer.autoFix(filePath);
    return result;
  }
