...
class LessonVersioning {
   ...
   */
  createVersioningTables(category = 'general') {
  ...
  findLessonByIdentifier(lessonData) {
    return new Promise((resolve, reject) => {
      ...
      this.ragDB.db.get(sql, [lessonData.title, lessonData.category], (error, row) => {
        if (error) {
          reject(error);
        } else {
          resolve(row || null);
  ...
  async updateExistingLessonVersion(existingLesson, newLessonData, versionInfo) {
    ...
    try {
      ...
      throw new Error(`Failed to update lesson version: ${error.message}`);
    }
}
  ...
  async createNewVersionedLesson(lessonData, versionInfo) {
    ...
    try {
      ...
      throw new Error(`Failed to create new versioned lesson: ${error.message}`);
    }
}
    ...
    return new Promise((resolve, reject) => {
      const sql = `
        INSERT INTO lesson_versions (
          lesson_id, version_number, title, content, category, subcategory,
  ...
  createVersionedLessonRecord(lessonData, version, versionHash, embeddingId, versionInfo) {
    return new Promise((resolve, reject) => {
      const sql = `
        INSERT INTO lessons (
          title, content, category, subcategory, project_path, file_path, tags,
