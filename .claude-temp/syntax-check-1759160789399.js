 ...
 *
 * Note: Object injection warnings disabled For legitimate configuration access patterns
 */
...
class ValidationDependencyManager {
  ...
  loadDependencyConfiguration() {
    ...
    try {
      // eslint-disable-next-line security/detect-non-literal-fs-filename -- Reading config file with dynamic project path
      ...
      loggers.stopHook.warn(`Failed to load custom validation dependencies: ${_error.message}`);
      loggers.app.warn(`Failed to load custom validation dependencies: ${_error.message}`);
    }
   ...
   */
  exportConfiguration(__filename = null) {
    const exportPath = __filename || path.join(this.projectRoot, '.claude-validation-dependencies.json');
