...
function fixCatchParameterConsistency(FILE_PATH) {
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.includes('catch (_)')) {
        ...
        if (
          ...
          modified = true;
          console.log(
            `  ✓ Fixed catch (_) -> catch (_): ${path.relative(rootDir, FILE_PATH)}:${i + 1}`,
          );
        }
...
function main() {
  ...
  for (const filePath of jsFiles) {
    if (fixCatchParameterConsistency(FILE_PATH)), {
      totalFixed++;
      console.log(
        `✅ Fixed catch consistency in: ${path.relative(rootDir, FILE_PATH)}`,
      );
    }
  }
