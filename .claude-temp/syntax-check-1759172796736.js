...
class TaskStorage {
  ...
  readTodoSync() {
    ...
    if (!FS.existsSync(this.todoPath)) {
      this.logger?.logWarning?.(
        'TODO.json does not exist, returning default structure'
      );
      return {
        tasks: [],
        completed_tasks: [],
    ...
    if (this._cache.enabled) {
      try {
        ...
        this.logger?.logError?.(
          error,
          'Error in fast read, falling back to uncached read'
        );
  ...
  async readTodo(skipValidation = false) {
    ...
    if (!FS.existsSync(this.todoPath)) {
      this.logger?.logInfo?.(
        'TODO.json does not exist, creating default structure'
      );
      const defaultData = {
        tasks: [],
        completed_tasks: [],
    ...
    try {
      ...
      if (backupPath && FS.existsSync(backupPath)) {
        try {
          ...
          this.logger?.logWarning?.(
            cleanupError,
            'Failed to cleanup temporary backup'
          );
        }
      }
    }
  ...
  async restoreFromBackup(backupFile = null) {
    ...
    try {
      ...
      if (FS.existsSync(this.todoPath)) {
        ...
        this.logger?.logInfo?.(
          `Created pre-restore backup: ${preRestoreBackup}`
        );
      }
      ...
      this.logger?.logInfo?.(
        `Successfully restored from backup: ${selectedBackup.filename}`
      );
      return {
        success: true,
  ...
  cleanupLegacyBackups(keepCount = 5) {
    ...
    for (const backup of toRemove) {
      try {
        ...
        this.logger?.logWarning?.(
          error,
          `Failed to remove backup: ${backup.filename}`
        );
      }
    }

    this.logger?.logInfo?.(
      `Cleanup completed, removed ${removedCount} old backups`
    );
    return {
      success: true,
    ...
    try {
      ...
      if (FS.existsSync(this.donePath)) {
        try {
          ...
          this.logger?.logWarning?.(
            'Failed to read DONE.json, creating new structure:',
            error.message
          );
      ...
      for (const task of tasks) {
        try {
          ...
          const existingTask = doneData.completed_tasks.find(
            (t) => t.id === task.id
          );
          if (existingTask) {
            results.skipped++;
          ...
          this.logger?.logError?.(
            error,
            `Failed to prepare task for archiving: ${task?.id}`
          );
        }
      }
      ...
      this.logger?.logInfo?.(
        `Bulk archive completed: ${results.archived} archived, ${results.skipped} skipped, ${results.errors} errors`
      );
  ...
  async readDone() {
    ...
    if (!FS.existsSync(this.donePath)) {
      this.logger?.logInfo?.(
        'DONE.json does not exist, creating default structure'
      );
      ...
      try {
        ...
        this.logger?.logWarning?.(
          error,
          'Failed to create DONE.json, returning default structure'
        );
        return defaultData;
      }
    }
