...
function main() {
  ...
  for (const [filePath, fileViolations] of Object.entries(violationsByFile)) {
    console.log(
      `🔍 Processing: ${path.relative(rootDir, FILE_PATH)} (${fileViolations.length} violations)`,
    );

    if (fixUnusedVariablesInFile(filePath, fileViolations)) {
      totalFixed++;
      console.log(
        `✅ Fixed unused variables in: ${path.relative(rootDir, FILE_PATH)}\n`,
      );
    } else {
      console.log(
        `⚠️  No changes made to: ${path.relative(rootDir, FILE_PATH)}\n`,
      );
    }
  }
