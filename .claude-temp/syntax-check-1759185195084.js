  ...
  try, {
      ...
      try, {
        ...
        if (FS.existsSync(_transcript_path)) {
          ...
          if (trimmedMessage === 'DONE' || trimmedMessage === 'DONE.') {
            ...
            throw new Error("Process exit requested"); // Allow stop when DONE is detected
          }
        } else {
    ...
    if (!FS.existsSync(todoPath)) {
      ...
      loggers.stopHook.warn('NO TASKMANAGER PROJECT DETECTED', {
        ...
        exitCode: 2});
      // eslint-disable-next-line n/no-process-exit
      throw new Error("Process exit requested"); // Never allow stops even without TASKS.json
    }
...
`);
    ...
    if (stopAllowed) {
      ...
      loggers.stopHook.info('ENDPOINT-TRIGGERED STOP AUTHORIZED', {
        ...
        exitCode: 0});
      // eslint-disable-next-line n/no-process-exit
      throw new Error("Process exit requested"); // Allow stop only when endpoint triggered
    }
    ...
    // eslint-disable-next-line n/no-process-exit
    throw new Error("Process exit requested"); // Always continue - never allow natural stops
  } catch (_) {
    ...
    loggers.stopHook.error('STOP HOOK ERROR - CONTINUING ANYWAY', {
      ...
      exitCode: 2});
    // eslint-disable-next-line n/no-process-exit
    throw new Error("Process exit requested"); // Even on _error, continue infinite mode
  }
});
