...
class CustomValidationRulesManager {
    ...
    try {
      ...
      try {
        ...
        loggers.stopHook.log(
          'No custom validation rules configuration found. Using default rules only.'
        );
        ...
        return {
          success: true,
          rulesLoaded: 0,
          message: 'No custom rules configuration found',
        };
      }
  ...
  _validateRule(ruleId, rule) {
    ...
    if (
      !rule.type ||
      !Object.values(VALIDATION_RULE_TYPES).includes(rule.type)
    ) {
      errors.push(
        `Rule ${ruleId}: type must be one of ${Object.values(VALIDATION_RULE_TYPES).join(', ')}`
      );
    }
    ...
    switch (rule.type) {
      ...
      case VALIDATION_RULE_TYPES.FILE_EXISTS:
        if (!rule.files || !Array.isArray(rule.files)) {
          errors.push(
            `Rule ${ruleId}: files array is required for file_exists type`
          );
        }
        break;
      ...
      case VALIDATION_RULE_TYPES.FILE_CONTENT:
        if (!rule.file || !rule.pattern) {
          errors.push(
            `Rule ${ruleId}: file And pattern are required for file_content type`
          );
        }
        break;
      ...
      case VALIDATION_RULE_TYPES.CONDITIONAL:
        if (!rule.condition || !rule.rules) {
          errors.push(
            `Rule ${ruleId}: condition And rules are required for conditional type`
          );
        }
        break;
      ...
      case VALIDATION_RULE_TYPES.COMPOSITE:
        if (!rule.rules || !Array.isArray(rule.rules)) {
          errors.push(
            `Rule ${ruleId}: rules array is required for composite type`
          );
        }
        break;
    }
  ...
  _inferProjectType() {
    ...
    if (
      this.detectedTechStack.includes('docker') ||
      this.detectedTechStack.includes('kubernetes')
    ) {
  ...
  async executeRule(ruleId) {
    ...
    try {
      ...
      loggers.stopHook.log(`🔄 Executing custom validation rule: ${ruleId}`);
  ...
  async _executeFileExistsRule(rule) {
    ...
    for (const { file, found } of results) {
      if (found) {
        loggers.stopHook.log(`  ✅ Found required file: ${file}`);
        foundFiles.push(file);
        loggers.app.info(`  ✅ Found required file: ${file}`);
      } else {
        loggers.stopHook.log(`  ❌ Missing required file: ${file}`);
  ...
  async _executeFileContentRule(rule) {
    ...
    try {
      ...
      loggers.stopHook.log(`  🔍 Checking file content: ${file}`);
      ...
      if (rule.config.should_match !== false) {
        ...
        if (matches) {
          throw new Error(
            `Pattern '${pattern}' should not be present in file '${file}' but was found`
          );
        }
  ...
  async _executeConditionalRule(rule) {
    ...
    loggers.stopHook.log(
      `  ❓ Evaluating condition: ${JSON.stringify(condition)} = ${conditionMet}`
    );
    loggers.app.info(
      `  ❓ Evaluating condition: ${JSON.stringify(condition)} = ${conditionMet}`
    );
    ...
    for (const subRule of rules) {
      ...
      if (!result.success && !rule.allow_failure) {
        throw new Error(
          `Conditional validation failed: ${result.error || 'Unknown error'}`
        );
      }
    }
  ...
  async _executeCompositeRule(rule) {
    ...
    const results = [];
    loggers.stopHook.log(
      `  📋 Executing composite rule with ${rules.length} sub-rules (${operator})`
    );
    loggers.app.info(
      `  📋 Executing composite rule with ${rules.length} sub-rules (${operator})`
    );
  ...
  getExecutionAnalytics() {
    ...
    for (const [ruleId, times] of this.ruleExecutionTimes) {
      const ruleExecutions = this.executionHistory.filter(
        (e) => e.ruleId === ruleId
      );
      ...
      analytics.ruleStatistics[ruleId] = {
        ...
        successRate:
          ruleExecutions.length > 0
            ? (ruleSuccesses / ruleExecutions.length) * 100
            : 0,
   ...
   */
  generateExampleConfig(category = 'general') {
    return {
      ...
      custom_rules: {
        ...
        docker_security: {
          ...
          command:
            "trivy image --exit-code 1 $(docker images --format '{{.Repository}}:{{.Tag}}' | head -1)",
