...
class DistributedLockManager {
  ...
  async acquireLock(filePath, agentId, timeout = null) {
    ...
    while (Date.now() < deadline && retryCount < this.options.maxRetries) {
      try {
        ...
        const lockAcquired = await this.attemptLockAcquisition(
          lockFile,
          agentId,
          lockId
        );
  ...
  detectConflicts(filePath, agentId, OPERATION) {
    ...
    const otherAgentRequests = waitingQueue.filter(
    ...
    if (OPERATION === 'write') {
      const writeConflicts = conflicts.filter(
        (c) => c.OPERATION === 'write' || c.type === 'active_lock'
      );
  ...
  ensureLockDirectory() {
    try {
      ...
      const validatedLockDir = LockSecurityHelper.validateLockPath(
    ...
    const cleanupPromises = staleLocks.map((lockId) => {
      ...
      if (lockInfo) {
        try {
          ...
          const validatedLockFile = LockSecurityHelper.validateLockPath(
            lockInfo.lockFile
          );
          ...
          LOGGER.warn('Failed to cleanup stale lock', {
            lockId,
  ...
  getStatistics() {
    return {
      ...
      totalLocksHeld: Array.from(this.agentLocks.values()).reduce(
    ...
    Array.from(this.activeLocks.keys()).forEach((lockId) => {
      ...
      if (lockInfo) {
        try {
          ...
          LOGGER.warn('Failed to cleanup lock on exit', {
            error: _error.message,
          });
        }
      }
