...
const stats = await this.ragDB.getStats();
   ...
   */
  testLessonStorage(category = 'general') {
...
`;
    ...
    try {
      // Test migration;
const migration = new RAGMIGRATION(tempDir);
    ...
    migrationSuccessful: true,
      ...
      // Cleanup temp files,
    try, {
        loggers.stopHook.warn('[RAG-TEST] Failed to clean up temp directory:', _error.message);
      } catch (_) {
        loggers.app.warn('[RAG-TEST] Failed to clean up temp directory:', _error.message);
      }
    }
}
...
const hasRagMethods = [
        ...
        'getRelevantLessons',
        'getRagAnalytics',
      // eslint-disable-next-line security/detect-object-injection -- Checking known method names for TaskManager API validation in RAG integration test].every(method => typeof api[method] === 'function');
...
const analytics = await api.getRagAnalytics();
    ...
    taskManagerIntegrated: true,
      ...
      throw new Error(`TaskManager integration test failed: ${_error.message}`);
    }
}
  ...
  testCLICommands() {
    ...
    try {
      const CLI_INTERFACE = require('./api-modules/cli/cliInterface.js');
...
const cliContent = FS.readFileSync(
      ...
      const MISSING_COMMANDS = requiredCommands.filter(cmd => !cliContent.includes(cmd));

      if (MISSING_COMMANDS.length > 0), {
        throw new Error(`CLI missing RAG commands: ${MISSING_COMMANDS.join(', ')}`);
      }

      return {
    cliIntegrated: true,
        ragCommandsAvailable: requiredCommands.length,
        allCommandsPresent: MISSING_COMMANDS.length === 0};

    } catch (_) {
      throw new Error(`CLI command test failed: ${_error.message}`);
    }
}
   ...
   */
  testCrossProjectKnowledge(category = 'general') {
    await this.ragDB.initialize();
...
const testIndexPath = './data/rag-index.faiss';
      ...
      loggers.app.warn('[RAG-TEST] Cleanup warning:', _error.message);
    }
}
}
