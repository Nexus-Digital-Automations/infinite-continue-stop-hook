...
class TaskStorage {
  ...
  async createBackup() {
    ...
    try {
      ...
      return, {
        success: false,
        message: `Failed to create backup: ${_.message}`,
        error};
    }
  }
  ...
  async restoreFromBackup(backupFile = null) {
    ...
    try {
      ...
      return, {
        success: false,
        message: `Failed to restore from backup: ${_.message}`,
        error};
    }
  }
    ...
    try {
      ...
      if (FS.existsSync(this.donePath)) {
        try {
          ...
          this.logger?.logWarning?.('Failed to read DONE.json, creating new structure:', _.message);
      ...
      return, {
        success: false,
        message: `Failed to archive tasks: ${_.message}`,
        archived: results.archived,
        skipped: results.skipped,
