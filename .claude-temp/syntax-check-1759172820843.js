...
class ValidationDependencyManager {
  ...
  addDependency(criterion, dependencyConfig) {
    ...
    const config = {
      ...
      metadata: {
        description:
          dependencyConfig.description || `Validation for ${criterion}`,
        estimatedDuration: dependencyConfig.estimatedDuration || 10000,
        parallelizable: dependencyConfig.parallelizable !== false,
        resourceRequirements: dependencyConfig.resourceRequirements || [
          'filesystem',
        ],
      },
    };
    ...
    for (const dep of config.dependencies) {
      if (!dep.criterion || !dep.type) {
        throw new Error(
          `Invalid dependency specification for ${criterion}: ${JSON.stringify(dep)}`
        );
      }
      if (!Object.values(DEPENDENCY_TYPES).includes(dep.type)) {
        throw new Error(
          `Invalid dependency type '${dep.type}' for ${criterion}. Must be one of: ${Object.values(DEPENDENCY_TYPES).join(', ')}`
        );
      }
    }
  ...
  _rebuildExecutionGraph() {
    ...
    for (const [criterion, config] of this.dependencyConfig.entries()) {
      this.executionGraph.set(criterion, {
        criterion,
        strictDependencies: config.dependencies
          .filter((d) => d.type === DEPENDENCY_TYPES.STRICT)
          .map((d) => d.criterion),
        weakDependencies: config.dependencies
          .filter((d) => d.type === DEPENDENCY_TYPES.WEAK)
          .map((d) => d.criterion),
        optionalDependencies: config.dependencies
          .filter((d) => d.type === DEPENDENCY_TYPES.OPTIONAL)
          .map((d) => d.criterion),
        dependents: [],
    ...
    for (const [criterion, node] of this.executionGraph.entries()) {
      for (const dep of [
        ...node.strictDependencies,
        ...node.weakDependencies,
  ...
  getExecutionOrder(criteria = null) {
    ...
    while (completed.size < targetCriteria.length) {
      ...
      const readyCriteria = targetCriteria.filter(
        (criterion) =>
      ...
      if (readyCriteria.length === 0) {
        ...
        const blockedCriteria = targetCriteria.filter(
          (criterion) => !completed.has(criterion) && !inProgress.has(criterion)
        );

        if (blockedCriteria.length > 0) {
          // Force execution of highest priority blocked criterion;
          const forcedCriterion = blockedCriteria.sort(
        ...
        const prioritizedCriteria = readyCriteria.sort(
          (a, b) => getExecutionPriority(b) - getExecutionPriority(a)
        );
  ...
  generateParallelExecutionPlan(criteria = null, maxConcurrency = 4) {
    ...
    const executionPlan = [];
    const completed = new Set();
    const RESOURCE_ALLOCATION = new Map();
    ...
    const resourcePools = {
      ...
      cpu: {
        maxConcurrent: Math.max(2, Math.floor(maxConcurrency * 0.75)),
        current: 0,
      },
      memory: {
        maxConcurrent: Math.max(2, Math.floor(maxConcurrency * 0.5)),
        current: 0,
      },
    ...
    const hasResourceAvailability = (criterion, currentWave) => {
      ...
      for (const resource of node.metadata.resourceRequirements) {
        if (
          ...
          currentUsage[resource].current >= currentUsage[resource].maxConcurrent
        ) {
          return false;
        }
      }
    ...
    const canStart = (criterion, currentWave = []) => {
      ...
      const strictDepsReady = node.strictDependencies.every((dep) =>
        completed.has(dep)
      );
      if (!strictDepsReady) {
        return false;
      ...
      const weakDepsReady = node.weakDependencies.every((dep) =>
        completed.has(dep)
      );
      const waitingCount = targetCriteria.filter(
        (c) => !completed.has(c)
      ).length;
      const weakDepOverride =
        !weakDepsReady && waitingCount > targetCriteria.length * 0.6;

      // Check resource availability
      if (!hasResourceAvailability(criterion, currentWave)) {
        return false;
      }

      // Check if parallelizable
      if (!node.metadata.parallelizable && currentWave.length > 0) {
        return false;
      }
    ...
    while (completed.size < targetCriteria.length && waveNumber < maxWaves) {
      ...
      const prioritizedCriteria = remainingCriteria.sort(
      ...
      if (currentWave.length === 0 && remainingCriteria.length > 0) {
        ...
        if (forcedCriterion) {
          ...
          const weakDepBlocked = remainingCriteria.find((criterion) => {
            const node = this.executionGraph.get(criterion);
            return (
              node && node.weakDependencies.some((dep) => !completed.has(dep))
            );
          });
      ...
      executionPlan.push({
        ...
        estimatedDuration: Math.max(
          ...waveEstimations.map((c) => c.estimatedDuration)
        ),
    ...
    const parallelDuration = executionPlan.reduce(
      (sum, wave) => sum + wave.estimatedDuration,
      0
    );
      ...
      efficiency: {
        averageConcurrency:
          executionPlan.length > 0
            ? executionPlan.reduce((sum, wave) => sum + wave.concurrency, 0) /
              executionPlan.length
            : 0,
        resourceUtilization:
          this._calculateOverallResourceUtilization(executionPlan),
        loadBalanceScore: this._calculateOverallLoadBalance(executionPlan),
        criticalPathOptimization:
          this._analyzeCriticalPathOptimization(executionPlan),
      },
  ...
  _calculateCriticalPathImpact(criteria) {
    ...
    for (const criterion of criteria) {
      ...
      if (node) {
        ...
        impact +=
          node.dependents.length * (node.metadata.estimatedDuration || 10000);
      }
    }
    return impact;
  }
  ...
  _analyzeCriticalPathOptimization(executionPlan) {
    const totalImpact = executionPlan.reduce(
      (sum, wave) => sum + (wave.criticalPathImpact || 0),
      0
    );
    const earlyWaves = executionPlan.slice(
      0,
      Math.ceil(executionPlan.length / 2)
    );
    const earlyImpact = earlyWaves.reduce(
      (sum, wave) => sum + (wave.criticalPathImpact || 0),
      0
    );
  ...
  getDependencyVisualization() {
    ...
    for (const [criterion, config] of this.dependencyConfig.entries()) {
      for (const dep of config.dependencies) {
        edges.push({
          ...
          type: dep.type,
          style:
            dep.type === DEPENDENCY_TYPES.STRICT
              ? 'solid'
    ...
    return {
      ...
      statistics: {
        ...
        strictDependencies: edges.filter(
          (e) => e.type === DEPENDENCY_TYPES.STRICT
        ).length,
        weakDependencies: edges.filter((e) => e.type === DEPENDENCY_TYPES.WEAK)
          .length,
        optionalDependencies: edges.filter(
          (e) => e.type === DEPENDENCY_TYPES.OPTIONAL
        ).length,
      },
    };
  }
  ...
  generateInteractiveVisualization(format = 'mermaid') {
    switch (format.toLowerCase()) {
      ...
      default:
        throw new Error(
          `Unsupported visualization format: ${format}. Supported: mermaid, graphviz, json, ascii`
        );
    }
  }
  ...
  _generateMermaidDiagram() {
    ...
    lines.push(
      '  classDef weak stroke:#f39c12,stroke-width:2px,stroke-dasharray: 5 5'
    );
    lines.push(
      '  classDef optional stroke:#95a5a6,stroke-width:1px,stroke-dasharray: 2 2'
    );
    ...
    for (const [criterion, config] of this.dependencyConfig.entries()) {
      ...
      const shape = config.metadata.parallelizable
        ? `${sanitizedId}(["${label}"])`
        : `${sanitizedId}["${label}"]`;
      lines.push(`  ${shape}`);
    ...
    return {
      ...
      diagram: lines.join('\n'),
      instructions:
        'Copy the diagram code to https://mermaid.live/ for interactive viewing',
      metadata: {
        totalNodes: this.dependencyConfig.size,
        totalEdges: Array.from(this.dependencyConfig.values()).reduce(
          (sum, config) => sum + config.dependencies.length,
          0
        ),
      },
    };
  ...
  _generateGraphvizDiagram() {
    ...
    for (const [criterion, config] of this.dependencyConfig.entries()) {
      ...
      lines.push(
        `  ${sanitizedId} [label="${label}", fillcolor="${color}", style="filled"];`
      );
    }
        ...
        const color =
          dep.type === DEPENDENCY_TYPES.STRICT
            ...
            : dep.type === DEPENDENCY_TYPES.WEAK
              ...
              : '#95a5a6';
        lines.push(
          `  ${fromId} -> ${toId} [style="${style}", color="${color}"];`
        );
      }
    }
    ...
    return {
      ...
      diagram: lines.join('\n'),
      instructions:
        'Save as .dot file And render with: dot -Tpng diagram.dot -o output.png',
      metadata: {
        totalNodes: this.dependencyConfig.size,
        totalEdges: Array.from(this.dependencyConfig.values()).reduce(
          (sum, config) => sum + config.dependencies.length,
  ...
  _generateASCIIDiagram() {
    ...
    for (let level = 0; level <= maxLevel; level++) {
      ...
      for (const node of nodesAtLevel) {
        ...
        const durationIndicator =
          node.estimatedDuration > 30000
            ? '🐌'
            : node.estimatedDuration > 10000
              ? '🚶'
              : '🏃';
        lines.push(
          `  ${parallelIndicator} ${durationIndicator} ${node.label} (${node.estimatedDuration}ms)`
        );
        ...
        if (deps.length > 0) {
          const depNames = deps.map((d) => {
            const symbol =
              d.type === DEPENDENCY_TYPES.STRICT
                ? '━━'
    ...
    lines.push(
      '  ━━> = Strict dependency, ┅┅> = Weak dependency, ╌╌> = Optional dependency'
    );
  ...
  _analyzeDependencyChains() {
    ...
    const rootNodes = Array.from(this.dependencyConfig.keys()).filter(
      (criterion) => {
        const config = this.dependencyConfig.get(criterion);
  ...
  _analyzeResourceConflicts() {
      ...
      if (users.length > 1) {
        ...
        if (parallelUsers.length > 1) {
          conflicts.push({
            ...
            totalDuration: parallelUsers.reduce(
              (sum, u) => sum + u.duration,
              0
            ),
            severity:
              resource === 'ports' || resource === 'network'
                ? 'high'
                : 'medium',
          });
        }
    ...
    return conflicts.sort((a, b) => {
      ...
      return (
        b.potentialConcurrency * bSeverityWeight -
        a.potentialConcurrency * severityWeight
      );
    });
  }
  ...
  _analyzeParallelizationOpportunities() {
    ...
    for (let i = 0; i < executionOrder.length; i++) {
      ...
      if (parallelCandidates.length > 0) {
        opportunities.push({
          ...
          parallelCandidates,
          potentialTimeReduction: this._calculatePotentialTimeReduction(
            current.criterion,
            parallelCandidates
          ),
    ...
    return opportunities.sort(
      (a, b) => b.potentialTimeReduction - a.potentialTimeReduction
    );
  }
    ...
    for (const criterion of path) {
      ...
      if (config) {
        ...
        const hasHighResourceUsage =
          config.metadata.resourceRequirements?.length > 2;
  ...
  async saveDependencyConfig(filename = null) {
    const configPath =
      __filename ||
      path.join(this.projectRoot, '.validation-dependencies.json');
  ...
  async loadDependencyConfig(filename = null) {
    const configPath =
      filename || path.join(this.projectRoot, '.validation-dependencies.json');
    try {
      ...
      if (config.dependencies) {
        ...
        for (const [criterion, depConfig] of Object.entries(
          config.dependencies
        )) {
      ...
      if (error.code !== 'ENOENT') {
        throw new Error(
          `Failed to load dependency configuration: ${error.message}`
        );
      }
   ...
   */
  recordExecution(criterion, result, duration, metadata = {}) {
  ...
  getExecutionAnalytics() {
    ...
    const analytics = {
      ...
      successRate:
        (this.executionHistory.filter((e) => e.result === 'success').length /
    ...
    for (const criterion of this.dependencyConfig.keys()) {
      const criterionHistory = this.executionHistory.filter(
      ...
      if (criterionHistory.length > 0) {
        analytics.criteriaStats[criterion] = {
          ...
          successRate:
            (criterionHistory.filter((e) => e.result === 'success').length /
              criterionHistory.length) *
            100,
          averageDuration:
            criterionHistory.reduce((sum, e) => sum + e.duration, 0) /
            criterionHistory.length,
          lastExecution:
            criterionHistory[criterionHistory.length - 1].timestamp,
        };
      }
    }
  ...
  async executeParallelValidationPlan(executionPlan, options = {}) {
    ...
    const TIMEOUT = timeout; // Store timeout for potential future use;
    ...
    try {
      for (const wave of executionPlan.plan) {
        ...
        const wavePromises = wave.criteria.map(async (criterionInfo) => {
          ...
          try {
            await onCriterionStart({
              criterion,
              estimatedDuration: criterionInfo.estimatedDuration,
            });

            // Simulate criterion execution (in real implementation, this would call actual validation)
            const RESULT = await this._executeCriterion(criterion, {
              timeout: criterionInfo.estimatedDuration * 2, // Allow 2x estimated time
              retries: maxRetries,
            });

            const duration = Date.now() - startTime;
            this.recordExecution(criterion, 'success', duration, {
              wave: wave.wave,
            });
            ...
            return { criterion, status: 'success', duration, result };
          } catch (_) {
            const duration = Date.now() - startTime;
        ...
        const waveSuccessCount = waveResults.filter(
        ...
        const criticalFailures = wave.criteria.filter((c) => {
          const RESULT = executionState.results.get(c.criterion);
          return (
            result?.status === 'failed' &&
            this._isCriticalCriterion(c.criterion)
          );
        });

        if (criticalFailures.length > 0) {
          throw new Error(
            `Critical validation failures: ${criticalFailures.map((c) => c.criterion).join(', ')}`
          );
        }
      }
      ...
      const totalCount =
        executionState.completedCriteria.size +
        executionState.failedCriteria.size;

      executionState.metrics = {
        totalDuration,
        successRate: totalCount > 0 ? (successCount / totalCount) * 100 : 0,
        parallelizationActual:
          executionPlan.sequentialDuration > 0
            ? ((executionPlan.sequentialDuration - totalDuration) /
                executionPlan.sequentialDuration) *
              100
            : 0,
        estimatedVsActual:
          executionPlan.estimatedTotalDuration > 0
            ? ((totalDuration - executionPlan.estimatedTotalDuration) /
                executionPlan.estimatedTotalDuration) *
              100
            : 0,
      };
  ...
  async _executeCriterion(criterion, options = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      setTimeout(() => {
        ...
        const node = this.executionGraph.get(criterion);
        const failureRate = node?.metadata.resourceRequirements?.includes(
          'network'
        )
          ? 0.1
          : 0.05;
        ...
        if (Math.random() < failureRate) {
          reject(
            new Error(`Validation failed for ${criterion}: simulated failure`)
          );
        } else {
          resolve({
            criterion,
  ...
  generateAdaptiveExecutionPlan(criteria = null, systemInfo = {}) {
    ...
    const memoryConcurrency = Math.max(
      1,
      Math.floor(availableMemory / (1024 * 1024 * 500))
    ); // 500MB per task;
    ...
    const basePlan = this.generateParallelExecutionPlan(
      criteria,
      optimalConcurrency
    );
    ...
    basePlan.adaptiveOptimizations = {
      ...
      resourceScheduling: this._optimizeResourceScheduling(
        basePlan.plan,
        SYSTEM_INFO
      ),
      executionTiming: this._optimizeExecutionTiming(
        basePlan.plan,
        SYSTEM_INFO
      ),
    };
   ...
   */
  _optimizeResourceScheduling(executionPlan, systemInfo) {
    const { diskIOLoad = 0.5, networkLatency = 10 } = systemInfo;
    ...
    if (diskIOLoad > 0.7) {
      optimizations.push({
        ...
        description:
          'High disk I/O detected - staggering filesystem-intensive validations',
        recommendation:
          'Run linter And build validations sequentially instead of parallel',
      });
    }
    ...
    if (networkLatency > 100) {
      optimizations.push({
        ...
        description:
          'High network latency detected - prioritizing local validations',
        recommendation:
          'Run network-dependent validations (security scans) after local validations complete',
      });
    }
  ...
  _optimizeExecutionTiming(executionPlan, _systemInfo) {
    ...
    for (const wave of executionPlan) {
      const longestTask = Math.max(
        ...wave.criteria.map((c) => c.estimatedDuration)
      );
      const shortestTask = Math.min(
        ...wave.criteria.map((c) => c.estimatedDuration)
      );
      ...
      if (timingRatio > 3) {
        recommendations.push({
          ...
          recommendation:
            'Consider splitting long tasks or running them in separate waves',
        });
      }
    }
