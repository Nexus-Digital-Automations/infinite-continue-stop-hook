...
class RAGOPERATIONS {
  ...
  async searchLessons(query, options = {}) {
    try {
      ...
      const, {
        ...
        threshold = 0.7,
        category = null,
        projectPath = null} = options;

      // Build enhanced query with filters;
      let searchQuery = query;
      if (_category) {searchQuery += ` category:${category}`;}
  ...
  _buildTaskSearchQuery(taskContext) {
    const queryParts = [
      ...
      (taskContext.description || '').split(' ').slice(0, 10).join(' ')].filter(part => part && part.trim());
...
`.trim();
  ...
  async storeLessonWithVersioning(lessonData, versionOptions = {}) {
    const, {
      ...
      majorUpdate = false,
      minorUpdate = false} = versionOptions;
  ...
  async searchLessonsWithVersioning(query, options = {}) {
    try {
      ...
      const, {
        ...
        _latestVersionsOnly = true} = options;
        ...
        try {
          ...
          return {
            ...
            versionInfo: versionInfo.success ?, {
              ...
              lastUpdated: versionInfo.metadata.updated_at} : null};
        } catch (_) {
