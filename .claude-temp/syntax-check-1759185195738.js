  ...
  async getFeatures() {
    try {
      ...
      if (apiResponse.success) {
        return apiResponse; // This has structure:, { success: true, features: [...], total: N }} else {
        throw new Error(`TaskManager API error: ${apiResponse.error}`);
      }
    } catch (_) {
      throw new Error(
        `Failed to get features from TaskManager API: ${_.message}`,
      );
    }
  }
...
class E2EAssertions {
  ...
  static extractFeatureId(commandResult) {
    try {
      ...
      throw new Error(
        `Failed to extract feature ID: ${_.message}\nResponse: ${commandResult.stdout}`,
      );
    }
  }
