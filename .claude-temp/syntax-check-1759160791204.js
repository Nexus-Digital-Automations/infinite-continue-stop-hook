...
const authPath = require('path').join(
    ...
    stop_allowed: true,
        ...
        const allowResult = await CommandExecutor.executeStopHook(
          [], // No arguments - just test the hook with authorization: { projectRoot: environment.testDir, expectSuccess: false },
        );
...
const stopTestResults = await Promise.allSettled([
          ...
          CommandExecutor.executeStopHook(
            [], // Test again to ensure consistent blocking: { projectRoot: environment.testDir, expectSuccess: false },
          ),
          CommandExecutor.executeStopHook(
            [], // Third test For reliability: { projectRoot: environment.testDir, expectSuccess: false },
          ),
        ]);
