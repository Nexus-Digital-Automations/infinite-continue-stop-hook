...
class RAGMigration {
  ...
  async migrate() {
    loggers.app.info(
      '[RAG-MIGRATION] Starting migration from development/ structure to RAG database...'
    );
    try {
   ...
   */
  async migrateLessons(category = 'general') {
  ...
  async migrateLessonCategory(categoryPath, _category) {
    ...
    try {
      ...
      this.log(
        'INFO',
        `Found ${markdownFiles.length} lesson files in ${category} category`
      );
      ...
      const filePromises = markdownFiles.map(async (file) => {
        ...
        try {
          ...
          if (migrationResult.success) {
            ...
            this.log(
              'ERROR',
              `Failed to migrate lesson ${filePath}: ${migrationResult.error}`
            );
            return { type: 'error' };
          }
        } catch (_) {
          this.log(
            'ERROR',
            `Error processing lesson file ${filePath}: ${error.message}`
          );
          return { type: 'error' };
        }
      });
      ...
      this.log(
        'ERROR',
        `Error reading category directory ${categoryPath}: ${error.message}`
      );
      results.errors++;
    }
  ...
  async migrateErrors() {
    ...
    try {
      ...
      const filePromises = markdownFiles.map(async (file) => {
        ...
        try {
          ...
          if (await this.isFileMigrated(filePath)) {
            this.log(
              'INFO',
              `Skipping already migrated error file: ${filePath}`
            );
            return { type: 'skipped' };
          }
          ...
          if (migrationResult.success) {
            ...
            this.log(
              'ERROR',
              `Failed to migrate error ${filePath}: ${migrationResult.error}`
            );
            return { type: 'error' };
          }
        } catch (_) {
          this.log(
            'ERROR',
            `Error processing error file ${filePath}: ${error.message}`
          );
          return { type: 'error' };
        }
      });
      ...
      this.log(
        'ERROR',
        `Error reading errors directory ${errorsPath}: ${error.message}`
      );
      results.errors++;
    }
  ...
  extractTags(fileName, content) {
    ...
    const techKeywords =
      content.match(
        /\b(javascript|typescript|react|node|express|mongo|postgres|redis|docker|aws|api|database|linter|eslint|jest|test|build|deploy)\b/gi
      ) || [];
  ...
  extractSubcategory(fileName, content) {
    ...
    if (content.includes('build') || content.includes('compilation')) {
      return 'build';
    }
  ...
  extractErrorType(fileName, content) {
    ...
    if (fileName.includes('linter') || content.includes('ESLint')) {
      return 'linter';
    }
    if (fileName.includes('build') || content.includes('compilation')) {
      return 'build';
    }
    if (fileName.includes('runtime') || content.includes('runtime')) {
      return 'runtime';
    }
    if (fileName.includes('api') || content.includes('API')) {
      return 'api';
    }
    ...
    if (fileName.includes('performance') || content.includes('performance')) {
      return 'performance';
    }
  ...
  extractResolution(content) {
    ...
    const lastThird = lines
      .slice(Math.floor((lines.length * 2) / 3))
      .join('\n');
  ...
  async isFileMigrated(filePath) {
    return new Promise((resolve, reject) => {
      const sql =
        'SELECT COUNT(*) as count FROM migrations WHERE file_path = ?';
  ...
  async markFileMigrated(filePath) {
    return new Promise((resolve, reject) => {
      const sql =
        'INSERT INTO migrations (file_path, migration_status) VALUES (?, ?)';
