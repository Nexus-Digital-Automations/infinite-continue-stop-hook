...
class VectorEmbeddingManager {
  ...
  async initialize() {
    try {
      ...
      throw new Error(`Vector embedding initialization failed: ${_.message}`);
    }
}
...
const embedding = Array.from(RESULT.data);
    ...
    vector: embedding,
      ...
      throw new Error(`Embedding generation failed: ${_.message}`);
    }
}
...
const cleanupPromises = cacheFiles.map(async (file) => {
    try {
          const FILE_PATH = path.join(this.cacheDir, file);
