...
class SuccessCriteriaValidator {
  ...
  async initialize() {
    try, {
      ...
      ValidationLogger._error(
        `❌ Failed to initialize validator: ${_.message}`,
      );
      throw _error;
    }
}
  ...
  async ensureDirectories() {
    try {
      ...
      ValidationLogger._error(
        `❌ Failed to create directories: ${_.message}`,
      );
      throw _error;
    }
}
  ...
  async getTaskCriteria(taskId) {
    ...
    task: task,
      ...
      ValidationLogger._error(
        `❌ Failed to get task criteria: ${_.message}`,
      );
      throw _error;
    }
}
...
const LINT_OUTPUT = execSync('npm run lint',, {
    ...
    tool: 'npm run lint',
      ...
      if (
        (_.stdout && _error.stdout.includes('warning')) ||
