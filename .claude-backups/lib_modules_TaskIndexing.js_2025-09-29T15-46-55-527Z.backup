...
class TaskIndexing {
   ...
   */
  constructor(performanceMonitor = null, options = {}, _agentId) {
  ...
  getTasksByCategory(category, todoData) {
    ...
    const _result = index.byCategory.get(_category) || [];
  ...
  getTasksByAgent(agentId, todoData) {
    ...
    const _result = index.byAgent.get(AGENT_ID) || [];
  ...
  _indexTask(task) {
    ...
    if (task.task.category) {
      if (!this._taskIndex.byCategory.has(task.task.category)) {
        this._taskIndex.byCategory.set(task.task.category, []);
      }
      this._taskIndex.byCategory.get(task.task.category).push(task);
    }
  ...
  /**
   * Fallback tasks by task.category lookup without indexing
   * @param {string} task.category - Category
   * @param {Object} todoData - TODO data
