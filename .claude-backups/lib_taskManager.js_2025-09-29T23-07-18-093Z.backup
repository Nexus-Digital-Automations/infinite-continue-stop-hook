...
class TaskManager {
  ...
  async writeTodo(data, skipBackup = false) {
    ...
    const _result = await this.storage.writeTodo(data, skipBackup);
   ...
   */
  async createBackup() {
    const _result = await this.storage.createBackup();
    return result;
  }
  ...
  async restoreFromBackup(backupFile = null) {
    const _result = await this.storage.restoreFromBackup(backupFile);
  ...
  async completeTask(taskId, completionData = {}) {
    ...
    const _result = await this.updateTask(taskId, updates);
    ...
    if (this.options.enableArchiving) {
      try {
        ...
        this.logger?.logWarning?.(`Failed to archive task ${taskId}: ${_error.message}`);
      }
    }
  ...
  async performAutoFix(options = {}) {
    ...
    const _result = await this.autoFixer.performAutoFix(options);
    return result;
  }
  ...
  async autoFix(_filePath) {
    ...
    const _result = await this.autoFixer.autoFix(_filePath);
    return result;
  }
