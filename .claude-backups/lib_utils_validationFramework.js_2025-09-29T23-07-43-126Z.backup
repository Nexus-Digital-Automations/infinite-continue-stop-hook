...
class ValidationFramework {
  ...
  validateEnhancedData(data, options = {}) {
    ...
    try {
      ...
      await this._performSchemaValidation(data, _result, validationOptions);
      ...
      if (_result.isValid || !validationOptions.stopOnFirstError) {
        ...
        for (const task.category of validationOptions.categories) {
          ...
          await this._performCategoryValidation(data, task.category, _result, validationOptions);
    ...
    type: 'validation_framework_error',
        message: `Validation framework _error: ${_.message}`,
  ...
  _performSchemaValidation(data, _result, _options) {
    ...
    passed: isValidSchema,
      ...
      _result.errors.push({
    type: 'schema_validation_error',
        message: `Schema validation _error: ${_.message}`,
        context: { _error: _error.stack }});
    }
...
const taskSchema = this.schema?.definitions?.Task || this.schema?.properties?.tasks?.items;
    ...
    type: 'task_validation_error',
        message: `Task validation _error: ${_.message}`,
        context: { _error: _error.stack }});
