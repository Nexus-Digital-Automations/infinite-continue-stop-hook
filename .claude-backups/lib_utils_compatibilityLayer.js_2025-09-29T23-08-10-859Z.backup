...
class CompatibilityLayer {
  ...
  readTodoData(__filename) {
    try {
      ...
      throw new Error(`Failed to read TODO data: ${_.message}`);
    }
}
   ...
   */
  writeTodoData(filePath, data, options = {}) {
    try {
      let writeResults = {};
...
const legacyBackupPath = filePath.replace('.json', '.legacy.json');
    ...
    enhanced: { success: true, format: 'enhanced', path: filePath },
      ...
      throw new Error(`Failed to write TODO data: ${_.message}`);
    }
}
  ...
  convertLegacyToEnhanced(legacyData) {
    const _result = await this._convertLegacyToEnhanced(legacyData);
    return _result;
}
   ...
   * @private
   */
  _convertLegacyToEnhanced(legacyData) {
...
const indexResult = await this.indexGenerator.generateIndexes(enhancedData);
  ...
  _migrateTasks(legacyTasks) {
    ...
    try {
        ...
        this.logger._error(`❌ Failed to migrate task ${legacyTask.id}:`, _error);
...
const uniqueCriteria = [];
   ...
   * @private
   */
  _normalizeCategoryField(_category) {
