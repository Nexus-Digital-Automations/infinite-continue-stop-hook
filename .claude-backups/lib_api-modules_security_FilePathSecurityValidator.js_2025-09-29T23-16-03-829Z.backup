...
class FilePathSecurityValidator {
   ...
   * @returns {Object} Validation _result with sanitized path
   */
  validateFilePath(filePath, OPERATION= 'read', options = {}) {
    ...
    try {
      this.log('debug', 'Starting file path validation', {
        validationId,,,
    filePath: this.sanitizePathForLogging(_filePath), operation,
        options});

      // Step 1: Basic input validation;
const inputResult = this.validateInput(_filePath);
...
const finalPath = this.performFinalValidation(normalizedPath, OPERATION;
  ...
  validateDirectoryPath(dirPath, OPERATION= 'read', options = {}) {
    const _result = this.validateFilePath(dirPath, OPERATION: { ...options, allowDirectory: true });
    if (_result.valid && !this.isDirectory(_result.path)) {
    return {
    valid: false,
        error: 'Path is not a directory',
        validationId: _result.validationId,
        path: null};
    }
    return _result;
}
